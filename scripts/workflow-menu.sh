#!/bin/bash
# workflow-menu.sh - Quick workflow helper for BookedBarber
# Called by /workflow slash command
...
echo -e "${PURPLE}🚀 BookedBarber Workflow Helper${NC}"
echo -e "${PURPLE}===============================${NC}\n"
...
echo -e "${GREEN}1${NC} ) 🚀 Deploy my feature to staging"
echo -e "${GREEN}2${NC} ) 🎯 Deploy staging to production" 
echo -e "${GREEN}3${NC} ) 🧹 Clean up after merge"
echo -e "${GREEN}4${NC} ) 📊 Check workflow status"
echo -e "${GREEN}5${NC} ) 🆕 Start new feature branch"
echo -e "${GREEN}6${NC} ) 💾 Quick commit changes"
...
case $choice in
    ...
    5)
        ...
        echo -e "${BLUE}Start coding! When done, run ${GREEN}/workflow${NC} again${NC}"
        ;;
    6)
        ...
        echo -e "${BLUE}Run ${GREEN}/workflow${NC} and choose option 1 to deploy${NC}"
        ;;
    0)
        ...
        ;;
    *)
        echo -e "\n${RED}Invalid choice. Run ${GREEN}/workflow${NC} again.${NC}"
        exit 1
        ;;
esac