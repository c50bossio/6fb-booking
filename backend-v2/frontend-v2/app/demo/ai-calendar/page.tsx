      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h3 className="font-semibold text-blue-900">Touch-First Design</h3>
            </div>
            <p className="text-sm text-blue-800">
              44px+ touch targets, haptic feedback, and gesture-based navigation optimized for mobile devices.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <span className="text-2xl">👈👉</span>
              <h3 className="font-semibold text-green-900">Swipe Navigation</h3>
            </div>
            <p className="text-sm text-green-800">
              Swipe left/right for dates, up/down for barbers. Natural mobile interaction patterns.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-purple-50 border-purple-200">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h3 className="font-semibold text-purple-900">AI Insights Drawer</h3>
            </div>
            <p className="text-sm text-purple-800">
              Bottom drawer with AI insights, analytics, and smart suggestions optimized for mobile viewing.
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <Card className="mt-6">
            ...
            <CardContent>
              <div className="space-y-4">
                {analytics.serviceProfitability.slice(0, 5).map((service, index) => (
                  <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      <h4 className="font-semibold">{service.service_name}</h4>
                      <p className="text-sm text-gray-600">
                        {service.total_bookings} bookings • ${service.average_price} avg price
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-green-700">
                        ${service.revenue_per_minute.toFixed(2)}/min
                      </p>
                      <Badge 
                        variant={service.six_fb_alignment > 75 ? 'default' : 'secondary'}
                        className="mt-1"
                      >
                        {service.six_fb_alignment}% 6FB Aligned
                      </Badge>
                    </div>
                  </div>
                ))}
  ...
  const renderAnalyticsDemo = () => (
    <div className="space-y-6">
                ...
                <div key={index} className="p-4 border border-gray-200 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="font-medium ml-1">{profile.booking_patterns.average_frequency_days}d</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Stage:</span> 
                      <span className="font-medium ml-1 capitalize">{profile.client_journey_stage.stage}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(aiTestScenarios).map(([key, scenario]) => (
              <Card 
                key={key} 
                className={`cursor-pointer transition-all ${
                  selectedScenario === key ? 'ring-2 ring-blue-500' : 'hover:shadow-md'
                }`}
                onClick={() => setSelectedScenario(key as keyof typeof aiTestScenarios)}
              >
                <CardContent className="p-4">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    {Object.entries(scenario).map(([prop, value]) => (
                      <div key={prop} className="flex justify-between">
                        <span className="capitalize">{prop.replace(/([A-Z])/g, ' $1').trim()}:</span>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs value={activeDemo} onValueChange={(value) => setActiveDemo(value as any)}>
          ...
          <TabsContent value="mobile">{renderMobileDemo()}</TabsContent>
          <TabsContent value="suggestions">{renderSuggestionsDemo()}</TabsContent>
          <TabsContent value="insights">{renderInsightsDemo()}</TabsContent>
          <TabsContent value="analytics">{renderAnalyticsDemo()}</TabsContent>
        </Tabs>
