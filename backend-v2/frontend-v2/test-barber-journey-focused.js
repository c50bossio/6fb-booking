...
class FocusedBarberTester {
  ...
  async init() {
    console.log('🚀 Starting Focused Barber Testing...');
    ...
    try {
      ...
      const premiumFeatures = await this.page.evaluate(() => {
        const features = {
          ...
          visualEffects: {
            coloredEvents: document.querySelectorAll('.fc-event[style*="color"], .event-colored').length,
            animatedElements: document.querySelectorAll('.animate-', '[class*="animate"]').length
          },
          advancedControls: {
            viewSwitcher: document.querySelectorAll('.fc-toolbar button, .view-switcher button').length,
            timeSlots: document.querySelectorAll('.fc-timegrid-slot, .time-slot').length
          },
          interactivity: {
            clickableSlots: document.querySelectorAll('.fc-daygrid-day, .fc-timegrid-slot').length,
      ...
      if (premiumFeatures.dragAndDrop.draggableElements > 0) {
        dragDropResult = await this.testDragAndDrop();
      }
      
      const premiumWorking = (
        premiumFeatures.dragAndDrop.draggableElements > 0 ||
  ...
  async generateReport() {
    ...
    // Save report
    const reportPath = path.join(__dirname, `barber-focused-test-report-${Date.now()}.json`);
  ...
  async runFullTest() {
    try {
      ...
      console.log('🎯 Starting focused barber journey testing...');
...
if (require.main === module) {
  const tester = new FocusedBarberTester();
...
module.exports = FocusedBarberTester;