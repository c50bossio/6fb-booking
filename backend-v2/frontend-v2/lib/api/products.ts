...
export const productsAPI = {
  ...
  async getProducts(filters?: ProductCatalogFilter): Promise<ProductCatalogResponse> {
    ...
    return fetchAPI(`/api/v2/products?${params}`)
  },

  async getProduct(productId: number): Promise<Product> {
    return fetchAPI(`/api/v2/products/${productId}`)
  },

  async createProduct(product: ProductCreate): Promise<Product> {
    return fetchAPI('/api/v2/products', {
      method: 'POST',
  ...
  async uploadProductImage(productId: number, file: File): Promise<{ image_url: string }> {
    ...
    return fetchAPI(`/api/v2/products/${productId}/images`, {
      method: 'POST',
      body: formData
    })
  },
  ...
  async createVariant(productId: number, variant: Omit<ProductVariant, 'id' | 'created_at' | 'updated_at'>): Promise<ProductVariant> {
    return fetchAPI(`/api/v2/products/${productId}/variants`, {
      method: 'POST',
  ...
  async updateVariant(productId: number, variantId: number, updates: Partial<ProductVariant>): Promise<ProductVariant> {
    return fetchAPI(`/api/v2/products/${productId}/variants/${variantId}`, {
      method: 'PUT',
      body: JSON.stringify(updates)
    })
  },
  ...
  async adjustInventory(adjustments: InventoryAdjustment[]): Promise<{ message: string; updated: number }> {
    return fetchAPI('/api/v2/products/inventory/adjust', {
      method: 'POST',
  ...
  async getInventoryReport(locationId: number): Promise<InventoryReport> {
    return fetchAPI(`/api/v2/products/inventory/report?location_id=${locationId}`)
  },
  ...
  async getOrders(filters?: {
    ...
    return fetchAPI(`/api/v2/products/orders?${params}`)
  },

  async getOrder(orderId: number): Promise<Order> {
    return fetchAPI(`/api/v2/products/orders/${orderId}`)
  },

  async createOrder(order: OrderCreate): Promise<Order> {
    return fetchAPI('/api/v2/products/orders', {
      method: 'POST',
  ...
  async cancelOrder(orderId: number, reason?: string): Promise<Order> {
    return fetchAPI(`/api/v2/products/orders/${orderId}/cancel`, {
      method: 'POST',
  ...
  async fulfillOrder(orderId: number, items?: Array<{ item_id: number; quantity: number }>): Promise<Order> {
    return fetchAPI(`/api/v2/products/orders/${orderId}/fulfill`, {
      method: 'POST',
  ...
  async createPOSTransaction(transaction: Omit<POSTransaction, 'id' | 'created_at' | 'updated_at'>): Promise<POSTransaction> {
    return fetchAPI('/api/v2/products/pos/transactions', {
      method: 'POST',
  ...
  async voidPOSTransaction(transactionId: number, reason: string): Promise<POSTransaction> {
    return fetchAPI(`/api/v2/products/pos/transactions/${transactionId}/void`, {
      method: 'POST',
      body: JSON.stringify({ reason })
    })
  },
  ...
  async getSalesReport(params: {
    ...
    return fetchAPI(`/api/v2/products/reports/sales?${queryParams}`)
  },
  ...
  async getCommissionReport(params: {
    ...
    const queryParams = new URLSearchParams(params as any)
    return fetchAPI(`/api/v2/products/reports/commissions?${queryParams}`)
  },

  // Shopify integration
  async syncShopifyProducts(request: ShopifyProductSyncRequest): Promise<ShopifyProductSyncResponse> {
    return fetchAPI('/api/v2/products/shopify/sync', {
      method: 'POST',
  ...
  async getShopifySyncStatus(integrationId: number): Promise<{
    ...
    return fetchAPI(`/api/v2/products/shopify/status?integration_id=${integrationId}`)
  },

  async triggerShopifyWebhookResync(integrationId: number): Promise<{ message: string }> {
    return fetchAPI(`/api/v2/products/shopify/webhooks/resync`, {
      method: 'POST',
