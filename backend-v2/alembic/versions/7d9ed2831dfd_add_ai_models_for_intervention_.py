"""Add AI models for intervention campaigns and intelligence features

Revision ID: 7d9ed2831dfd
Revises: 958456e0ff67
Create Date: 2025-07-21 21:46:01.288176
...
revision: str = '7d9ed2831dfd'
down_revision: Union[str, Sequence[str], None] = '958456e0ff67'
...
def upgrade() -> None:
    """Upgrade schema."""
    # Create enum types for AI models
    op.execute("CREATE TYPE interventiontype AS ENUM ('proactive_reminder', 'personalized_offer', 'follow_up_sequence', 'loyalty_reward', 'cancellation_prevention')")
    op.execute("CREATE TYPE interventionstatus AS ENUM ('pending', 'active', 'completed', 'paused', 'cancelled')")
    op.execute("CREATE TYPE interventionoutcome AS ENUM ('successful', 'failed', 'partially_successful', 'no_response', 'appointment_kept', 'appointment_cancelled', 'appointment_rescheduled')")
    op.execute("CREATE TYPE clienttier AS ENUM ('vip', 'platinum', 'gold', 'silver', 'bronze', 'new')")
    op.execute("CREATE TYPE weathercondition AS ENUM ('clear', 'cloudy', 'rain', 'snow', 'storm', 'fog', 'extreme_heat', 'extreme_cold')")
    op.execute("CREATE TYPE conversationintent AS ENUM ('confirm_appointment', 'cancel_appointment', 'reschedule_request', 'general_question', 'complaint_issue', 'location_inquiry', 'service_question', 'pricing_inquiry', 'emergency_request', 'unclear_intent')")
    ...
    op.create_table('ai_intervention_campaigns',
        ...
        sa.Column('campaign_type', sa.Enum('proactive_reminder', 'personalized_offer', 'follow_up_sequence', 'loyalty_reward', 'cancellation_prevention', name='interventiontype'), nullable=False),
        sa.Column('status', sa.Enum('pending', 'active', 'completed', 'paused', 'cancelled', name='interventionstatus'), nullable=True),
        ...
        sa.Column('outcome', sa.Enum('successful', 'failed', 'partially_successful', 'no_response', 'appointment_kept', 'appointment_cancelled', 'appointment_rescheduled', name='interventionoutcome'), nullable=True),
    ...
    op.create_table('weather_data',
        ...
        sa.Column('condition', sa.Enum('clear', 'cloudy', 'rain', 'snow', 'storm', 'fog', 'extreme_heat', 'extreme_cold', name='weathercondition'), nullable=False),
    ...
    op.create_table('client_tier_data',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('current_tier', sa.Enum('vip', 'platinum', 'gold', 'silver', 'bronze', 'new', name='clienttier'), nullable=False),
        sa.Column('previous_tier', sa.Enum('vip', 'platinum', 'gold', 'silver', 'bronze', 'new', name='clienttier'), nullable=True),
    ...
    op.create_table('message_personalizations',
        ...
        sa.Column('client_tier', sa.Enum('vip', 'platinum', 'gold', 'silver', 'bronze', 'new', name='clienttier'), nullable=True),
    ...
    op.create_table('client_sentiments',
        ...
        sa.Column('detected_intent', sa.Enum('confirm_appointment', 'cancel_appointment', 'reschedule_request', 'general_question', 'complaint_issue', 'location_inquiry', 'service_question', 'pricing_inquiry', 'emergency_request', 'unclear_intent', name='conversationintent'), nullable=True),
    ...
    op.create_table('ai_intervention_learning',
        ...
        sa.Column('appointment_type', sa.String(length=100), nullable=True),
        sa.Column('risk_level', sa.String(length=50), nullable=True),
        sa.Column('intervention_type', sa.Enum('proactive_reminder', 'personalized_offer', 'follow_up_sequence', 'loyalty_reward', 'cancellation_prevention', name='interventiontype'), nullable=False),
        ...
        sa.Column('outcome_type', sa.Enum('successful', 'failed', 'partially_successful', 'no_response', 'appointment_kept', 'appointment_cancelled', 'appointment_rescheduled', name='interventionoutcome'), nullable=False),
        sa.Column('engagement_score', sa.Float(), nullable=True),
        sa.Column('conversion_rate', sa.Float(), nullable=True),
        sa.Column('weather_condition', sa.String(length=50), nullable=True),
        sa.Column('day_of_week', sa.String(length=20), nullable=True),
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS clienttier")
    op.execute("DROP TYPE IF EXISTS interventionoutcome")
