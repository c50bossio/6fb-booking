...
class PerformanceMonitor:
    ...
    async def measure_api_endpoint(self, session: aiohttp.ClientSession, 
                                  endpoint: Dict) -> Dict:
        ...
        successful_times = [m['response_time'] for m in measurements 
        ...
        result = {
            ...
            'statistics': {
                ...
                'median': statistics.median(successful_times) if successful_times else None,
                'p95': statistics.quantiles(successful_times, n=20)[18] if len(successful_times) > 1 else None,
                'success_rate': sum(1 for m in measurements if m['success']) / len(measurements) * 100
            },
            'expected_time': endpoint['expected_time'],
            'within_sla': (statistics.mean(successful_times) <= endpoint['expected_time']) if successful_times else False
        }
        
        return result
    
    async def measure_frontend_performance(self, session: aiohttp.ClientSession,
                                         page: Dict) -> Dict:
        ...
        # Calculate statistics
        successful_times = [m['html_load_time'] for m in measurements 
        ...
        result = {
            ...
            'within_sla': (statistics.mean(successful_times) <= page['expected_load_time']) if successful_times else False
        }
    ...
    def save_metrics(self, metrics: Dict):
        ...
        detailed_file = self.metrics_dir / f"performance_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        rolling_data['api'] = [m for m in rolling_data['api'] 
                              if m['timestamp'] > cutoff]
        rolling_data['frontend'] = [m for m in rolling_data['frontend'] 
                                   if m['timestamp'] > cutoff]
    ...
    def generate_summary(self, metrics: Dict) -> Dict:
        ...
        for result in metrics['api_results']:
            if result['statistics']['mean']:
                api_summary['average_response_times'][result['endpoint']] = {
                    ...
                    'p95': round(result['statistics']['p95'], 2) if result['statistics']['p95'] else None,
        ...
        frontend_summary = {
            ...
            'within_sla': sum(1 for r in metrics['frontend_results'] if r['within_sla']),
            'average_load_times': {}
        }
        ...
        return {
            ...
            'overall_health': {
                'api_sla_compliance': (api_summary['within_sla'] / api_summary['total_endpoints'] * 100),
                'frontend_sla_compliance': (frontend_summary['within_sla'] / frontend_summary['total_pages'] * 100)
            }
        }
...
async def main():
    ...
    while True:
        try:
            ...
            logger.info(f"Performance Summary: API SLA {summary['overall_health']['api_sla_compliance']:.1f}%, "
            ...
            for slow in summary['api_performance']['slowest_endpoints']:
                logger.warning(f"Slow endpoint: {slow['endpoint']} - "
