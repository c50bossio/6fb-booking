...
class DeploymentMonitor:
    def __init__(self):
        self.deployment_state_file = Path('/var/log/6fb-monitoring/deployment_state.json')
    ...
    def save_state(self):
        ...
        try:
            ...
            with open(self.deployment_state_file, 'w') as f:
                json.dump({
                    'history': self.deployment_history[-100:],  # Keep last 100 deployments
...
"""
        ...
        if os.getenv('ENABLE_DEPLOYMENT_EMAILS', 'true').lower() == 'true':
            self.send_email_alert(f"Deployment {deployment['status']}: {service_name}", message)
        
        # Send webhook alert
        webhook_url = DEPLOYMENT_SERVICES[deployment['service']].get('webhook_url')
        if webhook_url:
            self.send_webhook_alert(webhook_url, {
            ...
            payload = {
                "attachments": [{
                    ...
                    "fields": [
                        ...
                        {"title": "Commit", "value": deployment['commit'][:8] if deployment['commit'] else 'N/A', "short": True},
                        {"title": "Health Check", "value": "Passed" if success else "Failed", "short": True}
                    ],
    ...
    def monitor_deployments(self):
        ...
        for service_key, config in DEPLOYMENT_SERVICES.items():
            ...
            if deployment:
                ...
                if (deployment_hash != self.last_deployment_hash.get(service_key) and
                    deployment['status'] in ['live', 'READY', 'success']):
