...
class ErrorAggregator:
    ...
    def load_error_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            "database_connection": re.compile(
                r"(connection|connect).*?(failed|error|refused)", re.I
            ),
            "authentication": re.compile(
                r"(auth|authentication|unauthorized|401)", re.I
            ),
    ...
    async def collect_backend_logs(self, session: aiohttp.ClientSession):
        ...
        try:
            ...
            async with session.get(
                ...
                headers={
                    "Authorization": f'Bearer {os.getenv("MONITORING_TOKEN", "")}'
                },
    ...
    async def collect_frontend_errors(self, session: aiohttp.ClientSession):
        ...
        try:
            ...
            async with session.get(
                frontend_url, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
    ...
    def generate_fingerprint(self, error: ErrorEvent) -> str:
        ...
        normalized_message = re.sub(
            r"[a-f0-9]{8,}", "ID", normalized_message
        )  # Replace IDs
    ...
    def aggregate_errors(self, errors: List[ErrorEvent]):
        ...
        for error in errors:
            ...
            if fingerprint in self.unique_errors:
                ...
                self.unique_errors[fingerprint].timestamp = (
                    error.timestamp
                )  # Update to latest
            else:
                # New unique error
    ...
    def save_errors(self):
        ...
        errors_file = (
            self.errors_dir / f"errors_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def update_rolling_errors(self):
        ...
        rolling_data["errors"] = [
            e for e in rolling_data.get("errors", []) if e.get("timestamp", "") > cutoff
        ]
    ...
    def generate_alert_summary(self) -> Optional[Dict]:
        ...
        critical_errors = [
            e
            for e in self.unique_errors.values()
        ...
        return {
            ...
            "top_errors": [
                ...
                for e in sorted(critical_errors, key=lambda x: x.count, reverse=True)[
                    :5
                ]
            ],
    ...
    async def run_collection_cycle(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            total_errors = sum(e.count for e in self.unique_errors.values())
            logger.info(
                f"Collection cycle complete. Total errors: {total_errors}, "
    ...
    async def send_critical_alerts(self, alert_summary: Dict):
        ...
        # For now, just log
        logger.critical(
            f"ALERT: {alert_summary['critical_error_count']} critical errors detected"
        )
