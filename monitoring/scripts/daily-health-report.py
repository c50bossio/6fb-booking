...
class DailyHealthReport:
            ...
            if metrics_file.exists():
                ...
                for service, status in data.get('status', {}).items():
                    ...
                    if failures == 0:
                        ...
                        uptime = max(0, 100 - (downtime_minutes / 1440 * 100))  # 1440 minutes in a day
                ...
                if uptime_data['service_uptime']:
                    uptime_values = [s['uptime_percentage'] for s in uptime_data['service_uptime'].values()]
                    uptime_data['overall_uptime'] = round(statistics.mean(uptime_values), 2)
    ...
    def calculate_performance_metrics(self, date: datetime.date) -> Dict:
        ...
        try:
            ...
            if rolling_file.exists():
                ...
                if api_times:
                    performance_data['api_performance']['average_response_time'] = round(statistics.mean(api_times), 2)
                    performance_data['api_performance']['p95_response_time'] = round(statistics.quantiles(api_times, n=20)[18], 2) if len(api_times) > 1 else api_times[0]
                    ...
                    performance_data['api_performance']['slowest_endpoints'] = [
                        {'endpoint': ep, 'avg_time': round(time, 2)} for ep, time in slowest
                    ]
                    
                    # Calculate SLA compliance (assuming 1000ms SLA)
                    within_sla = sum(1 for t in api_times if t <= 1000)
                    performance_data['api_performance']['sla_compliance'] = round(within_sla / len(api_times) * 100, 2)
                ...
                if frontend_times:
                    performance_data['frontend_performance']['average_load_time'] = round(statistics.mean(frontend_times), 2)
                    performance_data['frontend_performance']['p95_load_time'] = round(statistics.quantiles(frontend_times, n=20)[18], 2) if len(frontend_times) > 1 else frontend_times[0]
                    
                    # Find slowest pages
                    slowest = sorted(
                        [(page, statistics.mean(times)) for page, times in page_times.items()],
                        key=lambda x: x[1],
                        reverse=True
                    )[:3]
                    
                    performance_data['frontend_performance']['slowest_pages'] = [
                        {'page': page, 'avg_time': round(time, 2)} for page, time in slowest
                    ]
                    
                    # Calculate SLA compliance (assuming 3000ms SLA)
                    within_sla = sum(1 for t in frontend_times if t <= 3000)
                    performance_data['frontend_performance']['sla_compliance'] = round(within_sla / len(frontend_times) * 100, 2)
        ...
        try:
            ...
            if deployment_file.exists():
                ...
                for deployment in data.get('history', []):
                    detected_at = datetime.fromisoformat(deployment.get('detected_at', ''))
                    ...
                    if start_of_day <= detected_at < end_of_day:
                        ...
                        deployment_summary['deployments_by_service'][service]['total'] += 1
                        
                        if deployment.get('health_check_passed', True):
                            deployment_summary['successful_deployments'] += 1
                            deployment_summary['deployments_by_service'][service]['successful'] += 1
                        else:
                            deployment_summary['failed_deployments'] += 1
                            deployment_summary['deployments_by_service'][service]['failed'] += 1
    ...
    def generate_executive_summary(self, report: Dict) -> Dict:
        ...
        frontend_sla = report['performance_metrics']['frontend_performance']['sla_compliance']
        
        # Determine overall health
        if uptime >= 99.9 and critical_errors == 0 and api_sla >= 95 and frontend_sla >= 95:
            health_status = 'Excellent'
            health_color = 'green'
        elif uptime >= 99 and critical_errors <= 5 and api_sla >= 90 and frontend_sla >= 90:
            health_status = 'Good'
    ...
    def generate_recommendations(self, report: Dict) -> List[Dict]:
        ...
        if uptime < 99.9:
            recommendations.append({
                ...
                'affected_services': [
                    service for service, data in report['uptime_metrics']['service_uptime'].items()
        ...
        if api_p95 > 1000:
            recommendations.append({
                ...
                'affected_endpoints': report['performance_metrics']['api_performance']['slowest_endpoints']
            })
        ...
        if failed_deployments > 0:
            recommendations.append({
                ...
                'affected_services': [
                    service for service, data in report['deployment_summary']['deployments_by_service'].items()
                    if data['failed'] > 0
                ]
            })
    ...
    def generate_charts(self, report: Dict) -> Dict[str, BytesIO]:
        ...
        uptimes = [data['uptime_percentage'] for data in report['uptime_metrics']['service_uptime'].values()]
    ...
    def format_html_report(self, report: Dict, charts: Dict[str, BytesIO]) -> str:
        ...
        """.format(
            api_avg=report['performance_metrics']['api_performance']['average_response_time'],
            api_p95=report['performance_metrics']['api_performance']['p95_response_time'],
            api_sla=report['performance_metrics']['api_performance']['sla_compliance'],
            fe_avg=report['performance_metrics']['frontend_performance']['average_load_time'],
            fe_p95=report['performance_metrics']['frontend_performance']['p95_load_time'],
            fe_sla=report['performance_metrics']['frontend_performance']['sla_compliance']
        )
    ...
    def send_report(self, report: Dict, recipients: List[str]):
        ...
        try:
            ...
            # Send email
            with smtplib.SMTP(os.getenv('SMTP_HOST', 'smtp.sendgrid.net'), 
                             int(os.getenv('SMTP_PORT', '587'))) as server:
                server.starttls()
                server.login(os.getenv('SMTP_USER', 'apikey'), 
