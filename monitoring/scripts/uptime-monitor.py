...
class ServiceMonitor:
    ...
    def send_email_alert(self, service_name: str, check_result: Dict):
        ...
        try:
            msg = MIMEMultipart()
            msg["Subject"] = (
                f'[6FB Monitoring] {service_name} is {check_result["status"]}'
            )
            ...
            with smtplib.SMTP(
                ALERT_CONFIG["email"]["smtp_host"], ALERT_CONFIG["email"]["smtp_port"]
            ) as server:
                server.starttls()
                server.login(
                    ALERT_CONFIG["email"]["smtp_user"],
    ...
    def send_webhook_alert(self, service_name: str, check_result: Dict):
        ...
        try:
            ...
            response = requests.post(
                ALERT_CONFIG["webhook"]["url"], json=payload, timeout=10
            )
    ...
    def get_summary(self) -> Dict:
        ...
        healthy = sum(
            1 for s in self.service_status.values() if s.get("status") == "healthy"
        )
    ...
    while True:
        try:
            ...
            if datetime.utcnow().minute == 0:
                ...
                logger.info(
                    f"Monitoring Summary: {summary['healthy_services']}/{summary['total_services']} "
