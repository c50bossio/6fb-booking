...
class AuthenticationTester:
    ...
    def test_user_login(self) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if all(key in token_response for key in ["access_token", "token_type", "user"]):
                    self.log_result(
    ...
    def test_logout_functionality(self) -> bool:
        ...
        if not self.test_users.get("barber", {}).get("token"):
            self.log_result("Logout functionality", False, "No authenticated user available")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_result(
                    "Logout endpoint",
                    True,
                    "Successfully called logout endpoint"
                )
    ...
    def test_additional_auth_features(self) -> bool:
        ...
        if self.test_users.get("barber", {}).get("token"):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.test_users["barber"]["password"] = change_password_data["new_password"]
        ...
        try:
            forgot_password_data = {
                "email": self.test_users.get("barber", {}).get("email", "test@example.com")
            }
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/auth/send-magic-link",
    ...
    def generate_summary_report(self):
        ...
        with open(report_filename, 'w') as f:
            json.dump({
                ...
                "test_users": {k: v.get("email") for k, v in self.test_users.items()}
            }, f, indent=2)
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                print(f"Please start the server with: cd backend && uvicorn main:app --reload")
                return False
        except Exception as e:
