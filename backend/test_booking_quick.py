...
def test_booking_flow():
    ...
    if resp.status_code == 200:
        ...
        if barbers:
            ...
            barber_id = barber["id"]
            print(
                f"   Using barber: {barber['first_name']} {barber['last_name']} (ID: {barber_id})"
            )
            print(
                f"   Rating: {barber.get('average_rating', 'N/A')} ({barber.get('total_reviews', 0)} reviews)"
            )
        else:
    ...
    resp = requests.get(
        f"{BASE_URL}/booking/public/barbers/{barber_id}/availability", params=params
    )
    ...
    if resp.status_code == 200:
        ...
        for date_str, date_slots in list(slots_by_date.items())[
            :3
        ]:  # Show first 3 days
            print(f"   📅 {date_str}:")
    ...
    resp = requests.post(
        f"{BASE_URL}/booking/public/bookings/create", json=booking_data
    )
    ...
    if resp.status_code == 200:
        ...
        print(
            f"   Appointment: {confirmation['appointment']['service']} on {confirmation['appointment']['date']} at {confirmation['appointment']['time']}"
        )
        print(f"   Barber: {confirmation['barber']['name']}")
        print(
            f"   Client: {confirmation['client']['name']} ({confirmation['client']['email']})"
        )
    ...
    resp = requests.post(
        f"{BASE_URL}/booking/public/bookings/create", json=past_booking
    )
