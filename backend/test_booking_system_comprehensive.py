...
class BookingSystemTester:
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if resp.status_code == 200:
                    self.log_result("Create Admin User", "passed", {"email": TEST_USER["email"]})
            ...
            try:
                ...
                if resp.status_code == 200:
                    self.log_result("Create Barber User", "passed", {"email": TEST_BARBER_USER["email"]})
            except:
                pass
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                if locations:
                    ...
                    self.log_result("Get Locations", "passed", {
                        "count": len(locations),
            ...
            if self.location_id:
                resp = await client.get(f"{self.api_url}/booking/public/shops/{self.location_id}/barbers")
                if resp.status_code == 200:
                    barbers = resp.json()
                    if barbers:
                        self.barber_id = barbers[0]["id"]
                        self.log_result("Get Shop Barbers", "passed", {
                            "count": len(barbers),
                            "first_barber": f"{barbers[0]['first_name']} {barbers[0]['last_name']}"
                        })
                    else:
                        self.log_result("Get Shop Barbers", "failed", "No barbers found")
                else:
                    self.log_result("Get Shop Barbers", "failed", resp.json())
            
            # 3. Get services for a barber
            if self.barber_id:
                resp = await client.get(f"{self.api_url}/booking/public/barbers/{self.barber_id}/services")
                if resp.status_code == 200:
                    services = resp.json()
                    if services:
                        self.service_id = services[0]["id"]
                        self.log_result("Get Barber Services", "passed", {
                            "count": len(services),
                            "categories": list(set(s["category_name"] for s in services)),
                            "first_service": services[0]["name"],
                            "price": services[0]["base_price"]
                        })
                    else:
                        self.log_result("Get Barber Services", "failed", "No services found")
                else:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    self.log_result("Get Service Categories", "passed", {
    ...
    async def test_availability(self):
        ...
        if not self.barber_id or not self.service_id:
            self.log_result("Availability Test", "skipped", "Missing barber or service ID")
            return
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                unavailable_slots = [slot for slot in availability["slots"] if not slot["available"]]
                
                self.log_result("Get Availability", "passed", {
                    "total_slots": len(availability["slots"]),
                    "available": len(available_slots),
                    "unavailable": len(unavailable_slots),
                    "reasons": list(set(s.get("reason", "N/A") for s in unavailable_slots if s.get("reason")))
                })
    ...
    async def test_create_booking(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                self.log_result("Create Booking", "passed", {
    ...
    async def test_booking_rules_enforcement(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 422:  # Validation error expected
                self.log_result("Prevent Past Booking", "passed", "Correctly rejected past date")
            else:
                self.log_result("Prevent Past Booking", "failed", 
            ...
            if hasattr(self, 'available_slot'):
                ...
                if resp.status_code == 400:
                    self.log_result("Prevent Double Booking", "passed", 
                        "Correctly rejected double booking")
                else:
                    self.log_result("Prevent Double Booking", "failed",
    ...
    async def test_authenticated_endpoints(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                calendar = resp.json()
                self.log_result("Get Booking Calendar", "passed", {
    ...
    async def test_barber_schedule_management(self):
        ...
        if not self.barber_token or not self.barber_id:
            self.log_result("Schedule Management", "skipped", "No barber token/ID available")
            return
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                schedule = resp.json()
                self.log_result("Get Barber Schedule", "passed", {
    ...
    async def test_review_system(self):
        ...
        if not self.token or not self.appointment_id:
            self.log_result("Review System", "skipped", "No token or appointment available")
            return
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 201:
                ...
                self.log_result("Create Review", "passed", 
