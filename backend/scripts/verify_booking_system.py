...
from models.booking import (
    ServiceCategory, Service, BookingRule, Review, 
    BarberAvailability, BookingSlot, WaitList
)
...
def test_relationships(db: Session):
    ...
    try:
        ...
        if service:
            print(f"✓ Service '{service.name}' belongs to category '{service.category.name}'")
        
        # Test Barber -> BarberAvailability relationship
        barber = db.query(Barber).first()
        if barber:
            availability = db.query(BarberAvailability).filter_by(barber_id=barber.id).first()
            if availability:
    ...
    for cat in categories:
        if cat.services:
            ...
            for service in cat.services:
                ...
                print(f"    - {service.name}: ${service.base_price} / {service.duration_minutes}min{deposit_info}")
    
    # Booking Rules
    print("\nBooking Rules:")
    rules = db.query(BookingRule).filter_by(is_active=True).order_by(BookingRule.priority).all()
    for rule in rules:
        ...
        for barber_id, schedules in barber_schedules.items():
            ...
            if barber:
                ...
                for schedule in schedules:
                    ...
                    print(f"    - {schedule.day_of_week.name}: {schedule.start_time.strftime('%I:%M %p')} - {schedule.end_time.strftime('%I:%M %p')}{break_info}")
    else:
