...
class EnvironmentValidator:
            ...
            if "postgresql" in db_url or "postgres" in db_url:
                if settings.DB_POOL_SIZE < 5:
                    self.warnings.append(
                        "DB_POOL_SIZE is very low, consider increasing"
                    )
        ...
        if settings.payment_enabled:
            ...
            if settings.STRIPE_SECRET_KEY:
                if (
                    ...
                    and not settings.STRIPE_SECRET_KEY.startswith("sk_live_")
                ):
            ...
            if settings.is_production:
                self.errors.append(
                    "Payment processing must be configured for production"
                )
            else:
    ...
    def validate_monitoring_config(self):
        ...
        if settings.monitoring_enabled:
            ...
            if settings.is_production:
                self.warnings.append(
                    "Monitoring (Sentry) not configured for production"
                )
            else:
