...
class PaymentEndpointTester:
    ...
    def test_payout_endpoints(self):
        ...
        self.print_test(
            ...
            f"Total pending: ${summary_response.json().get('total_pending_payouts', 0):.2f}" if summary_response.status_code == 200 else f"Status: {summary_response.status_code}"
        )
    ...
    def test_webhook_endpoints(self):
        ...
        self.print_test(
            ...
            "Correctly rejected unsigned webhook" if response.status_code == 400 else f"Status: {response.status_code}"
        )
    ...
    def test_payment_method_endpoints(self):
        ...
        self.print_test(
            ...
            f"Methods available" if methods_response.status_code == 200 else f"Status: {methods_response.status_code}"
        )
    ...
    def test_refund_endpoints(self):
        ...
        if self.test_data.get("payment_intent_id"):
            ...
            self.print_test(
                ...
                expected_failure,
                "Correctly rejected refund for incomplete payment" if expected_failure else f"Unexpected status: {response.status_code}"
            )
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            print(f"{Fore.RED}❌ Cannot connect to server at {BASE_URL}{Style.RESET_ALL}")
