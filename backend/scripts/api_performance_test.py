...
class APIPerformanceTest:
    ...
    def check_server_and_authenticate(self) -> bool:
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/api/v1/auth/token", data=login_data)
    ...
    def test_api_endpoints(self) -> Dict[str, Any]:
        ...
        for method, endpoint, description, requires_auth in endpoints:
            try:
                ...
                if method == "GET":
                    ...
                    response = requests.request(method, f"{self.base_url}{endpoint}", headers=headers, timeout=10)
        ...
        print(f"   📊 Tested {results['endpoints_tested']} endpoints: {results['successful_endpoints']} successful, {results['failed_endpoints']} failed")
    ...
    def test_booking_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"{self.base_url}/api/v1/services", headers=self.auth_headers)
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ Services retrieved ({step_time:.3f}s) - {len(services) if isinstance(services, list) else 0} services")
            else:
                results["workflow_steps"]["get_services"] = {
                    "success": False,
                    "status_code": response.status_code,
            ...
            step_start = time.time()
            response = requests.get(f"{self.base_url}/api/v1/clients", headers=self.auth_headers)
            step_time = time.time() - step_start
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ Clients retrieved ({step_time:.3f}s) - {len(clients) if isinstance(clients, list) else 0} clients")
            else:
                results["workflow_steps"]["get_clients"] = {
                    "success": False,
                    "status_code": response.status_code,
            ...
            step_start = time.time()
            response = requests.get(f"{self.base_url}/api/v1/barbers", headers=self.auth_headers)
            step_time = time.time() - step_start
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ Barbers retrieved ({step_time:.3f}s) - {len(barbers) if isinstance(barbers, list) else 0} barbers")
            else:
                results["workflow_steps"]["get_barbers"] = {
                    "success": False,
                    "status_code": response.status_code,
            ...
            if clients and barbers and services:
                ...
                booking_data = {
                    ...
                    "scheduled_at": (datetime.now() + timedelta(days=1, hours=14)).isoformat(),
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(f"   ✅ Booking created ({step_time:.3f}s) - ID: {booking.get('id')}")
                else:
                    results["workflow_steps"]["create_booking"] = {
                        "success": False,
                        "status_code": response.status_code,
    ...
    def test_security_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   JWT Protection - Invalid token: {'✅' if results['jwt_protection']['invalid_token']['properly_rejected'] else '❌'}")
            print(f"   JWT Protection - No token: {'✅' if results['jwt_protection']['no_token']['properly_rejected'] else '❌'}")
            ...
            forgot_status = "✅ Implemented" if results["forgot_password"]["implemented"] else f"⚠️ Status: {response.status_code}"
            ...
            response = requests.post(f"{self.base_url}/api/v1/auth/token", data=malicious_login)
            ...
            print(f"   Input Validation - SQL injection handled: {'✅' if results['input_validation']['sql_injection']['properly_handled'] else '❌'}")
            ...
            results["security_score"] = (sum(security_checks) / len(security_checks)) * 100
    ...
    def test_load_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            def make_api_request(endpoint):
                ...
                try:
                    if endpoint.startswith("/api/v1/auth/"):
                        response = requests.get(f"{self.base_url}{endpoint}", headers=self.auth_headers, timeout=10)
                    else:
            ...
            for concurrent_users in [5, 10]:
                ...
                success_rate = results["concurrent_requests"][f"{concurrent_users}_concurrent"]["success_rate"]
                print(f"     Success rate: {success_rate:.1f}% ({len(successful_requests)}/{total_requests})")
                print(f"     Average response time: {avg_response_time:.3f}s")
                print(f"     Requests per second: {results['concurrent_requests'][f'{concurrent_users}_concurrent']['requests_per_second']:.1f}")
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if api_results.get("endpoints_tested", 0) > 0:
                endpoint_success_rate = (api_results.get("successful_endpoints", 0) / api_results.get("endpoints_tested", 1)) * 100
                scores.append(endpoint_success_rate)
                summary["api_functionality"] = "good" if endpoint_success_rate >= 80 else "needs_work"
            ...
            if concurrent_results:
                ...
                summary["performance_status"] = "good" if avg_success_rate >= 90 else "acceptable"
            else:
            ...
            if summary["booking_workflow"] == "issues":
                summary["critical_issues"].append("End-to-end booking workflow has failures")
            
            if security_score < 75:
                summary["critical_issues"].append("Security configuration needs improvement")
            
            if summary["performance_status"] not in ["good", "acceptable"]:
                summary["recommendations"].append("Improve API response times and reliability")
            
            # Key metrics
            summary["key_metrics"] = {
                "avg_api_response_time": api_results.get("average_response_time", 0),
                "api_success_rate": endpoint_success_rate if 'endpoint_success_rate' in locals() else 0,
    ...
    def run_tests(self):
        ...
        print(f"\n📈 Component Status:")
        print(f"   • API Functionality: {summary['api_functionality'].replace('_', ' ').title()}")
        print(f"   • Booking Workflow: {summary['booking_workflow'].replace('_', ' ').title()}")
        print(f"   • Security Status: {summary['security_status'].replace('_', ' ').title()}")
        print(f"   • Performance Status: {summary['performance_status'].replace('_', ' ').title()}")
        
        # Key metrics
        metrics = summary["key_metrics"]
        print(f"\n📊 Key Metrics:")
        print(f"   • Average API Response Time: {metrics['avg_api_response_time']:.3f}s")
