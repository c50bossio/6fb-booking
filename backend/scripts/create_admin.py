...
def create_admin_user(email=None, first_name=None, last_name=None, username=None, password=None):
    ...
    try:
        ...
        if existing_user:
            ...
            if existing_user.role == "super_admin":
                ...
                update = input(f"Current role is '{existing_user.role}'. Update to super_admin? (y/n): ")
                if update.lower() == 'y':
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create an admin user for the 6FB Platform")
