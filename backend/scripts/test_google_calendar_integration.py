...
def test_environment_setup():
    ...
    redirect_uri = getattr(settings, "GOOGLE_CALENDAR_REDIRECT_URI", None) or os.getenv(
...
def test_database_tables():
    ...
    try:
        ...
        BaseModel.metadata.create_all(
            bind=engine,
            tables=[GoogleCalendarSettings.__table__, GoogleCalendarSyncLog.__table__],
        )
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                and oauth_config["web"]["client_id"] != "your_google_client_id_here"
            ):
        ...
        for endpoint in endpoints_to_test:
            try:
                ...
                if response.status_code == 401:  # Unauthorized is expected
                    print_success(
                        f"Endpoint {endpoint} responds correctly (requires auth)"
                    )
...
def test_sync_logging():
    ...
    try:
        ...
        logs = (
            ...
            .filter(GoogleCalendarSyncLog.barber_id == test_data["barber_id"])
            .all()
        )
