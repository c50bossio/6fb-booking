...
from models.training import (
    TrainingModule, Certification, TrainingEnrollment,
    UserCertification, TrainingPath
)
...
class DataSeeder:
    ...
    def seed_clients(self, barbers, locations):
        ...
        first_names = ["James", "Robert", "John", "Michael", "William", "David", 
                       "Richard", "Joseph", "Thomas", "Christopher", "Maria", "Jennifer",
        ...
        for i in range(50):  # Create 50 clients
            client_data = {
                ...
                "customer_type": random.choice(["new", "returning", "vip"]) if i % 5 != 0 else "new",
        ...
        for days_ago in range(60):
            ...
            for _ in range(num_appointments):
                ...
                if status == "completed":
                    ...
                    appointment_data["tip_amount"] = random.choice([0, 5, 10, 15, 20]) if random.random() > 0.2 else 0
                    appointment_data["product_revenue"] = random.choice([0, 0, 0, 15, 25, 40]) if random.random() > 0.7 else 0
    ...
    def seed_metrics(self, barbers, appointments):
        ...
        for barber in barbers:
            ...
            completed_appointments = [a for a in barber_appointments if a.status == "completed"]
            ...
            retention_score = (returning_clients / total_clients * 100) if total_clients > 0 else 0
            avg_ticket = float(total_revenue / len(completed_appointments)) if completed_appointments else 0
    ...
    def seed_training_data(self, users):
        ...
        self.db.commit()
        print(f"Created {len(created_modules)} training modules and {len(created_certs)} certifications")
        ...
        rules_data = [
            {
                ...
                "trigger_config": {"event": "appointment_no_show", "conditions": {"status": "no_show"}},
            ...
            },
            {
                ...
                "trigger_config": {"metric": "overall_score", "operator": "<", "threshold": 75},
    ...
    def print_summary(self):
        ...
        print(f"  - Super Admins: {self.db.query(User).filter(User.role == 'super_admin').count()}")
        print(f"  - Admins: {self.db.query(User).filter(User.role == 'admin').count()}")
        print(f"  - Mentors: {self.db.query(User).filter(User.role == 'mentor').count()}")
        print(f"  - Barbers: {self.db.query(User).filter(User.role == 'barber').count()}")
        ...
        print(f"  - Completed: {self.db.query(Appointment).filter(Appointment.status == 'completed').count()}")
        print(f"  - Scheduled: {self.db.query(Appointment).filter(Appointment.status == 'scheduled').count()}")
