...
try:
    ...
    from services.query_optimization_service import QueryPerformanceMonitor, DatabaseOptimizer
...
class PerformanceTestSuite:
    ...
    async def test_database_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for query in basic_queries:
                ...
                print(f"   Query: {query} | Time: {query_end - query_start:.4f}s | Result: {result}")
            ...
            for i, query in enumerate(complex_queries):
                ...
                try:
                    ...
                    print(f"   Complex Query {i+1}: {query_end - query_start:.4f}s | Rows: {len(result)}")
                except Exception as e:
            ...
            results["query_optimization"]["complex_queries"] = {
                ...
                "average_time": statistics.mean([t for t in complex_query_times if t != float('inf')]),
            ...
            current_avg = statistics.mean(query_times + [t for t in complex_query_times if t != float('inf')])
            
            if current_avg > 0:
                improvement_percentage = max(0, min(70, (baseline_time - current_avg) / baseline_time * 100))
            else:
    ...
    def test_booking_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ Service listing successful ({service_time:.3f}s) - {len(services)} services")
            else:
                results["service_listing"] = {
                    "success": False,
                    "status_code": response.status_code,
                ...
                if barbers:
                    ...
                    if response.status_code == 200:
                        ...
                        results["availability_check"] = {
                            ...
                            "slots_available": len(availability.get("available_slots", [])),
                            "date_checked": tomorrow
                        }
                        print(f"   ✅ Availability check successful ({availability_time:.3f}s)")
                    else:
                        results["availability_check"] = {
                            "success": False,
                            "status_code": response.status_code,
                ...
                if clients and barbers and services:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        if response.status_code in [200, 201]:
                            ...
                            print(f"   ✅ Payment processing successful ({payment_time:.3f}s)")
                            ...
                            print(f"   ⚠️  Payment processing failed: {response.status_code} (booking still created)")
                    else:
                        results["booking_creation"] = {
                            "success": False,
                            "status_code": response.status_code,
    ...
    def test_security_implementations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/api/v1/auth/register", json=weak_password_data)
            ...
            results["api_rate_limiting"] = {
                ...
                "avg_response_time": statistics.mean([r["response_time"] for r in rate_limit_results])
            }
            ...
            results["input_validation"]["xss_prevention"] = {
                "endpoint_protected": response.status_code == 401,  # Should require auth
                "status_code": response.status_code
            }
            ...
            security_checks = [
                ...
                results["password_hashing"]["weak_password_rejected"]["properly_rejected"],
    ...
    def test_authentication_flow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if results["forgot_password"]["implemented"]:
                ...
                print(f"   ⚠️  Forgot password not fully implemented (status: {response.status_code})")
            ...
            response = requests.post(f"{self.base_url}/api/v1/auth/refresh", json=refresh_data)
        ...
        try:
            ...
            for concurrent_users in concurrency_levels:
                ...
                results["concurrent_users"][f"{concurrent_users}_users"] = {
                    ...
                    "requests_per_second": len(concurrent_results) / (end_time - start_time)
                }
                
                print(f"     Success rate: {len(successful_requests)}/{len(concurrent_results)} ({len(successful_requests) / len(concurrent_results) * 100:.1f}%)")
    ...
    def validate_production_readiness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                required_tables = ["users", "barbers", "clients", "appointments", "services"]
            ...
            results["deployment_readiness"] = {
                ...
                "all_endpoints_responsive": all(ep.get("responsive", False) for ep in endpoint_results.values())
            }
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "performance_improvement" in self.results["database_performance"]:
                improvement = self.results["database_performance"]["performance_improvement"]
            ...
            if "overall_security_score" in self.results["security_validation"]:
                scores.append(self.results["security_validation"]["overall_security_score"])
            ...
            security_score = self.results["security_validation"].get("overall_security_score", 0)
            ...
            readiness_score = self.results["production_readiness"].get("readiness_score", 0)
            ...
            if security_score < 90:
                recommendations.append("Implement additional security measures (rate limiting, input validation)")
            ...
            if load_issues:
                recommendations.append(f"Optimize performance for concurrent users: {', '.join(load_issues)}")
            
            summary["recommendations"] = recommendations
            
            # Performance metrics summary
            summary["performance_metrics"] = {
                "avg_query_time": self.results["database_performance"].get("query_optimization", {}).get("basic_queries", {}).get("average_time", 0),
                "max_concurrent_users": max([int(k.split('_')[0]) for k in load_results.keys()] + [0]),
