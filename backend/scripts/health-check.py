...
class HealthChecker:
    ...
    def check_endpoint(
        self, endpoint, method="GET", expected_status=200, data=None, name=None
    ):
        ...
        try:
            ...
            if response.status_code == expected_status:
                print_status(
                    "success",
                    f"{check_name} - {response.status_code} ({response_time:.0f}ms)",
                )
                self.results["summary"]["passed"] += 1
            else:
                print_status(
                    "error",
                    f"{check_name} - {response.status_code} (expected {expected_status})",
                )
                self.results["summary"]["failed"] += 1

        except requests.exceptions.Timeout:
            check_result.update({"error": "Timeout", "success": False})
            print_status("error", f"{check_name} - Timeout")
            self.results["summary"]["failed"] += 1

        except requests.exceptions.ConnectionError:
            check_result.update({"error": "Connection Error", "success": False})
            print_status("error", f"{check_name} - Connection Error")
            self.results["summary"]["failed"] += 1

        except Exception as e:
            check_result.update({"error": str(e), "success": False})
    ...
    def run_health_checks(self):
        ...
        if self.auth_token:
            ...
            self.check_endpoint(
                "/api/v1/analytics/dashboard", name="Analytics Dashboard"
            )
        else:
            print_status(
                "warning",
        ...
        services_check = next(
            (c for c in self.results["checks"] if c["name"] == "List Services"), None
        )
        ...
        if slow_endpoints:
            for endpoint in slow_endpoints:
                print_status(
                    "warning",
                    f"{endpoint['name']} is slow ({endpoint['response_time_ms']}ms)",
                )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Health check for 6FB Booking Platform"
    )
    parser.add_argument(
    ...
    if args.email and args.password and not auth_token:
        ...
        if auth_token:
            ...
            print_status(
                "warning",
                "Authentication failed - continuing with public endpoints only",
            )
