...
def test_password_validation():
    ...
    for password, should_pass, description in test_cases:
        ...
        if is_valid == should_pass:
            log_test(f"Password validation - {description}", True, 
                    f"{'Accepted' if is_valid else 'Rejected'}: {error_msg or 'Valid'}")
        else:
            log_test(f"Password validation - {description}", False,
...
def test_jwt_token_generation():
    ...
    try:
        ...
        try:
            ...
            log_test("JWT expired token rejection", True, "Expired token correctly rejected")
        
        # Test invalid signature
        try:
            jwt.decode(token, "wrong_secret", algorithms=[settings.JWT_ALGORITHM])
            log_test("JWT invalid signature rejection", False, "Invalid signature was accepted")
            return False
        except jwt.InvalidTokenError:
            log_test("JWT invalid signature rejection", True, "Invalid signature correctly rejected")
...
def test_password_hashing():
    ...
    try:
        ...
        if pwd_context.verify(test_password, hashed):
            log_test("Password verification - correct", True, "Password verified successfully")
        else:
            log_test("Password verification - correct", False, "Failed to verify correct password")
            return False
        
        # Test wrong password
        if not pwd_context.verify("WrongPassword123!", hashed):
            log_test("Password verification - incorrect", True, "Wrong password correctly rejected")
        else:
            log_test("Password verification - incorrect", False, "Wrong password was accepted")
            return False
        
        # Test that same password generates different hashes
        hashed2 = pwd_context.hash(test_password)
        if hashed != hashed2:
            log_test("Password hash uniqueness", True, "Same password generates different hashes")
        else:
            log_test("Password hash uniqueness", False, "Same password generated identical hashes")
            return False
    ...
    try:
        ...
        for role, permission, resource_id, should_have in permission_tests:
            ...
            if has_perm == should_have:
                log_test(f"Permission check - {role}:{permission}", True,
                        f"{'Has' if has_perm else 'Does not have'} permission (expected)")
            else:
                log_test(f"Permission check - {role}:{permission}", False,
        ...
        for role in roles:
            ...
            if permissions and len(permissions) > 0:
                log_test(f"Get {role} permissions", True, f"Has {len(permissions)} permissions")
            else:
                # Barber and staff have limited permissions, so empty is ok for them
                if role in ["barber", "staff"] and len(permissions) < 5:
                    log_test(f"Get {role} permissions", True, f"Limited permissions (expected)")
                else:
...
def test_security_settings():
    ...
    if hasattr(settings, 'JWT_SECRET_KEY') and settings.JWT_SECRET_KEY:
        if len(settings.JWT_SECRET_KEY) >= 32:
            log_test("JWT secret key length", True, f"Key length: {len(settings.JWT_SECRET_KEY)} chars")
        else:
            log_test("JWT secret key length", False, f"Key too short: {len(settings.JWT_SECRET_KEY)} chars")
            all_passed = False
    else:
    ...
    if hasattr(settings, 'JWT_ALGORITHM') and settings.JWT_ALGORITHM:
        if settings.JWT_ALGORITHM in ["HS256", "HS384", "HS512", "RS256"]:
            ...
            log_test("JWT algorithm", False, f"Unusual algorithm: {settings.JWT_ALGORITHM}")
            all_passed = False
    else:
    ...
    if hasattr(settings, 'ACCESS_TOKEN_EXPIRE_MINUTES'):
        ...
        if 15 <= expire_mins <= 1440:  # Between 15 mins and 24 hours
            log_test("Token expiration", True, f"Tokens expire in {expire_mins} minutes")
        else:
            log_test("Token expiration", False, f"Unusual expiration: {expire_mins} minutes")
            all_passed = False
    else:
        log_test("Token expiration", False, "ACCESS_TOKEN_EXPIRE_MINUTES not configured")
        all_passed = False
