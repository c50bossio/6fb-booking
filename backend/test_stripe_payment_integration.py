...
class StripePaymentTester:
        ...
        try:
            ...
            has_keys = all(
                [
                    ...
                    and "pk_" in settings.STRIPE_PUBLISHABLE_KEY,
                    settings.STRIPE_CONNECT_CLIENT_ID
                    and "ca_" in settings.STRIPE_CONNECT_CLIENT_ID,
                ]
            )
    ...
    def test_barber_stripe_status(self):
        ...
        if status_response.status_code == 200:
            ...
            print_test_result(
                "Stripe status check", False, f"Status {status_response.status_code}"
            )
    ...
    def test_payment_split_calculation(self):
        ...
        for case in test_cases:
            response = self.session.get(
                ...
                params={"barber_id": barber_id, **case},
            )
    ...
    def test_webhook_endpoint(self):
        ...
        response = self.session.post(f"{API_V1}/webhooks/stripe", json={"test": "data"})
    ...
    def test_stripe_connect_flow(self):
        ...
        if oauth_response.status_code == 200:
            ...
            if "url" in data and "stripe.com" in data["url"]:
                ...
                print_test_result(
                    "OAuth URL generation", False, "Invalid response format"
                )
        elif oauth_response.status_code == 400:
            # Already connected
            print_test_result(
                "OAuth URL generation", True, "Barber already connected (expected)"
            )
        else:
            print_test_result(
                "OAuth URL generation", False, f"Status {oauth_response.status_code}"
            )
