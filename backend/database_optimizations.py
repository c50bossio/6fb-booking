...
from sqlalchemy import (
    Index, text, func, inspect,
    Column, Integer, String, DateTime, Boolean, Float, Date, Time
)
...
class DatabaseOptimizer:
    ...
    def create_performance_indexes(self):
        ...
        indexes = [
            ...
            Index('idx_appointments_date_barber', 'appointments.appointment_date', 'appointments.barber_id'),
            Index('idx_appointments_date_client', 'appointments.appointment_date', 'appointments.client_id'),
            Index('idx_appointments_date_status', 'appointments.appointment_date', 'appointments.status'),
            Index('idx_appointments_barber_status', 'appointments.barber_id', 'appointments.status'),
            
            # Revenue analysis queries
            Index('idx_appointments_revenue_date', 'appointments.service_revenue', 'appointments.appointment_date'),
            Index('idx_appointments_completed_revenue', 'appointments.is_completed', 'appointments.service_revenue', 'appointments.appointment_date'),
            
            # Customer analysis
            Index('idx_appointments_customer_type_date', 'appointments.customer_type', 'appointments.appointment_date'),
            Index('idx_appointments_booking_source', 'appointments.booking_source', 'appointments.appointment_date'),
            
            # Payment processing
            Index('idx_appointments_payment_status', 'appointments.payment_status', 'appointments.appointment_date'),
            ...
            # === BARBER INDEXES ===
            Index('idx_barbers_location_active', 'barbers.location_id', 'barbers.is_active'),
            ...
            Index('idx_clients_location_created', 'clients.preferred_location_id', 'clients.created_at'),
            
            # === BOOKING SYSTEM INDEXES ===
            # Service lookups
            Index('idx_services_category_active', 'services.category_id', 'services.is_active'),
            Index('idx_services_location_active', 'services.location_id', 'services.is_active'),
            Index('idx_services_barber_active', 'services.barber_id', 'services.is_active'),
            Index('idx_services_price_range', 'services.base_price', 'services.is_active'),
            
            # Booking slots optimization
            Index('idx_booking_slots_date_barber', 'booking_slots.slot_date', 'booking_slots.barber_id', 'booking_slots.is_available'),
            Index('idx_booking_slots_location_date', 'booking_slots.location_id', 'booking_slots.slot_date', 'booking_slots.is_available'),
            Index('idx_booking_slots_service_date', 'booking_slots.service_id', 'booking_slots.slot_date', 'booking_slots.is_available'),
            
            # Barber availability
            Index('idx_barber_availability_schedule', 'barber_availability.barber_id', 'barber_availability.day_of_week', 'barber_availability.is_available'),
            Index('idx_barber_availability_location', 'barber_availability.location_id', 'barber_availability.day_of_week'),
            
            # === PAYMENT INDEXES ===
            Index('idx_payments_appointment_status', 'payments.appointment_id', 'payments.status'),
            Index('idx_payments_date_status', 'payments.payment_date', 'payments.status'),
            Index('idx_payments_method_date', 'payments.payment_method', 'payments.payment_date'),
            Index('idx_payments_amount_date', 'payments.amount', 'payments.payment_date'),
            
            # === ANALYTICS INDEXES ===
            # Daily metrics
            Index('idx_daily_metrics_date_barber', 'daily_metrics.metric_date', 'daily_metrics.barber_id'),
            Index('idx_daily_metrics_location_date', 'daily_metrics.location_id', 'daily_metrics.metric_date'),
            
            # Weekly metrics
            Index('idx_weekly_metrics_week_barber', 'weekly_metrics.week_start_date', 'weekly_metrics.barber_id'),
            Index('idx_weekly_metrics_location_week', 'weekly_metrics.location_id', 'weekly_metrics.week_start_date'),
            
            # Monthly metrics
            Index('idx_monthly_metrics_month_barber', 'monthly_metrics.month_start_date', 'monthly_metrics.barber_id'),
            Index('idx_monthly_metrics_location_month', 'monthly_metrics.location_id', 'monthly_metrics.month_start_date'),
            
            # === COMMUNICATION INDEXES ===
            Index('idx_email_logs_status_date', 'email_logs.status', 'email_logs.sent_at'),
            Index('idx_sms_logs_status_date', 'sms_logs.status', 'sms_logs.sent_at'),
            Index('idx_notifications_user_read', 'notifications.user_id', 'notifications.is_read', 'notifications.created_at'),
            
            # === COMPENSATION/PAYMENT SPLITS ===
            Index('idx_compensation_plans_barber_active', 'compensation_plans.barber_id', 'compensation_plans.is_active'),
            Index('idx_payment_history_barber_date', 'payment_history.barber_id', 'payment_history.payment_date'),
            Index('idx_commission_calculations_date', 'commission_calculations.calculation_date'),
            
            # === REVIEWS INDEXES ===
            Index('idx_reviews_barber_rating', 'reviews.barber_id', 'reviews.overall_rating', 'reviews.created_at'),
            Index('idx_reviews_location_rating', 'reviews.location_id', 'reviews.overall_rating', 'reviews.created_at'),
            Index('idx_reviews_appointment_id', 'reviews.appointment_id'),
            Index('idx_reviews_verified_featured', 'reviews.is_verified', 'reviews.is_featured', 'reviews.is_hidden'),
            
            # === WAIT LIST INDEXES ===
            Index('idx_wait_lists_preferred_date', 'wait_lists.preferred_date', 'wait_lists.status'),
            Index('idx_wait_lists_client_status', 'wait_lists.client_id', 'wait_lists.status'),
            Index('idx_wait_lists_service_date', 'wait_lists.service_id', 'wait_lists.preferred_date', 'wait_lists.status'),
        ]
    ...
    def analyze_table_statistics(self):
        ...
        if 'sqlite' in str(self.engine.url).lower():
            ...
            tables = [
                'appointments', 'users', 'barbers', 'clients', 'payments',
                'services', 'booking_slots', 'daily_metrics', 'reviews'
            ]
            for table in tables:
                try:
    ...
    def optimize_slow_queries(self):
        ...
        if 'postgresql' in str(self.engine.url).lower():
            try:
                ...
                self.session.execute(text("CREATE EXTENSION IF NOT EXISTS pg_stat_statements"))
