...
class BookingAPITester:
    ...
    def test_public_endpoints(self):
        ...
        if resp.status_code == 200:
            ...
            if locations:
                ...
                resp = requests.get(
                    f"{self.base_url}/booking/public/shops/{location_id}/barbers"
                )
                if resp.status_code == 200:
                    barbers = resp.json()
                    ...
                    if barbers:
                        ...
                        barber_id = barber["id"]
                        print(
                            f"   First barber: {barber['first_name']} {barber['last_name']}"
                        )
                        ...
                        resp = requests.get(
                            f"{self.base_url}/booking/public/barbers/{barber_id}/services"
                        )
                        if resp.status_code == 200:
                            services = resp.json()
                            ...
                            for cat, svcs in by_category.items():
                                ...
                                for svc in svcs[:2]:  # Show first 2
                                    ...
                                    if svc["requires_deposit"]:
                                        deposit_info = (
                                            f" (Deposit: ${svc['deposit_amount']})"
                                        )
                                    print(
                                        f"     - {svc['name']}: ${svc['base_price']:.2f} ({svc['duration_minutes']}min){deposit_info}"
                                    )
                            ...
                            if services:
                                ...
                                if resp.status_code == 200:
                                    ...
                                    unavailable = [
                                        s for s in slots if not s["available"]
                                    ]
                                    ...
                                    if available:
                                        ...
                                        self.test_booking_creation(
                                            barber_id, service_id, available[0]
                                        )
                                    else:
                                        self.print_result(
                                            False, "No available slots found"
                                        )

                                else:
                                    self.print_result(
                                        False,
                                        f"Availability check failed: {resp.status_code}",
                                        resp.text,
                                    )
                        else:
                            self.print_result(
                                False,
                                f"Get services failed: {resp.status_code}",
                                resp.text,
                            )
                else:
                    self.print_result(
                        False, f"Get barbers failed: {resp.status_code}", resp.text
                    )
        else:
            self.print_result(
                False, f"Get locations failed: {resp.status_code}", resp.text
            )
    ...
    def test_booking_creation(self, barber_id, service_id, slot):
        ...
        resp = requests.post(
            f"{self.base_url}/booking/public/bookings/create", json=booking_data
        )
        ...
        if resp.status_code == 200:
            ...
            self.print_result(
                False, f"Booking creation failed: {resp.status_code}", resp.text
            )
    ...
    def test_booking_confirmation(self, booking_token):
        ...
        resp = requests.get(
            f"{self.base_url}/booking/public/bookings/confirm/{booking_token}"
        )
    ...
    def test_double_booking(self, original_booking):
        ...
        resp = requests.post(
            f"{self.base_url}/booking/public/bookings/create", json=original_booking
        )

        if resp.status_code == 400:
            self.print_result(
                True,
                "Double booking correctly prevented",
                "System rejected attempt to book already taken slot",
            )
        else:
            self.print_result(
                False, f"Double booking was not prevented! Status: {resp.status_code}"
            )
    ...
    def test_booking_rules(self):
        ...
        resp = requests.post(
            f"{self.base_url}/booking/public/bookings/create", json=past_booking
        )
        ...
        if resp.status_code in [400, 422]:
            ...
            self.print_result(
                False, f"Past date booking not prevented: {resp.status_code}"
            )
    ...
    def test_authenticated_endpoints(self):
        ...
        resp = requests.post(f"{self.base_url}/auth/login", data=login_data)
        ...
        if resp.status_code == 200:
            ...
            if resp.status_code == 200:
                ...
                self.print_result(
                    False, f"Calendar retrieval failed: {resp.status_code}"
                )
