...
try:
    ...
    sentry_sdk.set_context("parsing", {
...
try:
    ...
    sentry_sdk.add_breadcrumb(
        message="Starting large data processing",
        category="memory",
        level="warning"
    )
    ...
    memory_error = MemoryError("Unable to allocate 8GB for booking analytics processing")
...
try:
    ...
    raise UnauthorizedAccessError(f"User {user_id} does not have permission to access {resource}")
