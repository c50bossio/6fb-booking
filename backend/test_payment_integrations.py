...
class PaymentIntegrationTester:
    ...
    def log_result(
        self, test_name: str, provider: str, success: bool, details: str = ""
    ):
        """Log test result"""
        ...
        if success:
            ...
            self.results["overall"]["errors"].append(
                f"{provider}: {test_name} - {details}"
            )
    ...
    def test_stripe_connection(self):
        ...
        try:
            ...
            self.log_result(
                "API Connection", "stripe", False, f"Connection failed: {str(e)}"
            )
    ...
    def test_stripe_connect_oauth(self):
        ...
        try:
            ...
            success = oauth_url and oauth_url.startswith(
            ...
            self.log_result(
                ...
                (
                    f"URL: {oauth_url[:100]}..."
                    if success
                    else "Failed to generate OAuth URL"
                ),
            )

        except Exception as e:
            self.log_result("OAuth URL Generation", "stripe", False, f"Error: {str(e)}")
    ...
    def test_square_connection(self):
        ...
        try:
            ...
            self.log_result(
                "API Connection", "square", False, f"Connection failed: {str(e)}"
            )
    ...
    def generate_report(self):
        ...
        print(
            f"   📊 Success Rate: {(self.results['overall']['passed'] / (self.results['overall']['passed'] + self.results['overall']['failed']) * 100):.1f}%"
        )
        ...
        report_file = (
            f"payment_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
