...
class EndpointStats:
    ...
    def update(self, metrics: RequestMetrics):
        ...
        self.avg_response_time = (
            self.avg_response_time * (self.total_requests - 1)
...
class LoadMonitor:
    ...
    def get_current_load(self) -> Dict[str, Any]:
        ...
        current_rps = sum(self.requests_per_second) / max(
            1, len(self.requests_per_second)
        )
        ...
        return {
            ...
            "cpu_usage_percent": (
                self.cpu_usage_history[-1] if self.cpu_usage_history else 0
            ),
            "memory_usage_percent": (
                self.memory_usage_history[-1] if self.memory_usage_history else 0
            ),
...
class PerformanceMetricsCollector:
    ...
    def record_request(self, metrics: RequestMetrics):
        ...
        self.load_monitor.record_request_end(
            metrics.response_time_ms, metrics.status_code >= 400
        )

    def get_endpoint_performance(self, limit: int = 20) -> List[Dict[str, Any]]:
        """Get top performing/worst performing endpoints"""
        sorted_endpoints = sorted(
            self.endpoint_stats.values(), key=lambda x: x.total_requests, reverse=True
        )
    ...
    def get_error_prone_endpoints(
        self, min_requests: int = 10, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    def get_category_performance(self) -> Dict[str, Any]:
        ...
        for endpoint in self.endpoint_stats.values():
            ...
            stats["total_response_time"] += (
                endpoint.avg_response_time * endpoint.total_requests
            )
    ...
    def get_time_series_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_response_time": (
                    sum(req.response_time_ms for req in recent_requests)
                    / len(recent_requests)
                    if recent_requests
                    else 0
                ),
...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            metrics = RequestMetrics(
                ...
                endpoint_category=self.metrics_collector.categorize_endpoint(
                    request.url.path
                ),
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "endpoint_performance": self.metrics_collector.get_endpoint_performance(
                limit=10
            ),
            "slow_endpoints": self.metrics_collector.get_slow_endpoints(limit=5),
            "error_prone_endpoints": self.metrics_collector.get_error_prone_endpoints(
                limit=5
            ),
