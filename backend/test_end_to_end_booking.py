    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 401:
                print(
                    "   🔐 Calendar events require authentication (✅ security working)"
                )
            elif response.status_code == 200:
                events = response.json()
                print(
                    f"   ✅ Calendar events endpoint working: {len(events)} events found"
                )
            else:
        ...
        try:
            ...
            if response.status_code == 401:
                print(
                    "   🔐 Availability requires authentication (✅ security working)"
                )
            elif response.status_code == 200:
                ...
                if "available_slots" in availability:
                    print(
                        f"   📅 Available slots found: {len(availability['available_slots'])}"
                    )
        ...
        try:
            ...
            if response.status_code == 401:
                print(
                    "   🔐 Google Calendar OAuth requires authentication (✅ security working)"
                )
            elif response.status_code == 200:
        ...
        try:
            ...
            response = await client.post(
                f"{BASE_URL}/api/v1/calendar/appointments", json=appointment_data
            )

            if response.status_code == 401:
                print(
                    "   🔐 Appointment creation requires authentication (✅ security working)"
                )
