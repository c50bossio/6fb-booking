    ...
    def parse_database_url(self):
        ...
        try:
            ...
            self.connection_params = {
                ...
                "database": (
                    self.parsed_url.path.lstrip("/") if self.parsed_url.path else None
                ),
                "user": (
                    unquote(self.parsed_url.username)
                    if self.parsed_url.username
                    else None
                ),
                "password": (
                    unquote(self.parsed_url.password)
...
class RenderDatabaseConnection:
            ...
            try:
                ...
                if driver_name == "psycopg2":
                    pool_config["connect_args"] = {
                        "sslmode": self.config.connection_params.get(
                            "sslmode", "require"
                        ),
                        "connect_timeout": 10,
                    ...
                    pool_config["connect_args"] = {
                        "ssl_context": (
                            True
                            if self.config.connection_params.get("sslmode") == "require"
                            else None
                        ),
                        "timeout": 10,
                    }
                ...
                logger.info(
                    f"Successfully initialized database with {driver_name} driver"
                )
                return True
    ...
    async def initialize_async_engine(self, is_production: bool = True) -> bool:
        ...
        if not ASYNCPG_AVAILABLE:
            logger.warning(
                "asyncpg not available, skipping async engine initialization"
            )
            return False
        ...
        try:
            ...
            # Create async engine
            self.async_engine = create_async_engine(url, **pool_config)
    ...
    def get_db(self):
        ...
        if not self.SessionLocal:
            raise RuntimeError(
                "Database not initialized. Call initialize_sync_engine first."
            )
    ...
    async def get_async_db(self):
        ...
        if not self.AsyncSessionLocal:
            raise RuntimeError(
                "Async database not initialized. Call initialize_async_engine first."
            )
