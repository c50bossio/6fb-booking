...
def receive_checkin(dbapi_connection, connection_record):
    ...
    if hasattr(connection_record, 'checkout_time'):
        ...
        logger.debug(f"Connection checked in: {id(dbapi_connection)} (used for {usage_time:.2f}s)")
    ...
    def bulk_insert(db: Session, model_class, records: list, batch_size: int = 1000):
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                logger.error(f"Bulk insert failed for batch {i//batch_size + 1}: {str(e)}")
                db.rollback()
                raise
    ...
    def bulk_update(db: Session, model_class, updates: list, batch_size: int = 1000):
        ...
        for i in range(0, len(updates), batch_size):
            ...
            try:
                ...
                logger.debug(f"Bulk updated {len(batch)} {model_class.__name__} records")
            except Exception as e:
                logger.error(f"Bulk update failed for batch {i//batch_size + 1}: {str(e)}")
                db.rollback()
                raise
