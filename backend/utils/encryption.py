...
def migrate_plaintext_to_encrypted(db_session, model_class, field_name: str):
    ...
    """
    logger.warning(
        f"Starting migration of {model_class.__name__}.{field_name} to encrypted format"
    )

    # Get all records with non-null plaintext data
    records = (
        db_session.query(model_class)
        .filter(getattr(model_class, field_name).isnot(None))
        .all()
    )
    ...
    for record in records:
        ...
        if len(plaintext_value) > 50 and any(
            c in plaintext_value for c in ["+", "/", "="]
        ):
            continue
    ...
    db_session.commit()
    logger.info(
        f"Migrated {migrated_count} records for {model_class.__name__}.{field_name}"
    )
