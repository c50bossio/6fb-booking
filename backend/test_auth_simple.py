...
def test_auth_direct():
    ...
    test_emails = ["test@example.com", "admin@6fb.com"]
    test_passwords = ["testpassword123", "admin123"]
    ...
    # Get all users first
    cursor.execute(
        "SELECT id, email, first_name, last_name, hashed_password, role, is_active FROM users"
    )
        ...
        for user in all_users:
            ...
            try:
                ...
                if pwd_context.verify(password, hashed_password):
                    print(
                        f"  ✅ Password matches user ID {user_id}: {first_name} {last_name} ({role})"
                    )
    ...
    simple_password = "password123"  # pragma: allowlist secret
    ...
    try:
        ...
        if "UNIQUE constraint failed" in str(e) or "already exists" in str(e):
            print(
                f"  ℹ️  Simple test user already exists: {simple_email} / {simple_password}"
            )
        else:
