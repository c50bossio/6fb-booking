...
class PaymentSystemTester:
    ...
    def setup_auth(self):
        ...
        if login_response.status_code == 200:
            ...
            self.log_test(
                "Authentication", False, error=f"Login failed: {login_response.text}"
            )
            return False
    ...
    def test_payment_intent_creation(self):
        ...
        if locations_response.status_code != 200:
            self.log_test(
                "Payment Intent Creation", False, error="Could not fetch locations"
            )
            return
        ...
        response = self.session.post(
            f"{API_V1}/payments/payment-intents", json=payment_data
        )
    ...
    def test_stripe_connect_oauth(self):
        ...
        callback_response = self.session.post(
            ...
            json={"code": "ac_test_code", "state": f"barber_{barber_id}"},
        )
    ...
    def test_webhook_security(self):
        ...
        if response.status_code == 400:
            self.log_test(
                "Webhook Security - Invalid Signature", True, {"rejected_invalid": True}
            )
        else:
            self.log_test(
    ...
    def test_payment_security_features(self):
        ...
        response = self.session.post(
            f"{API_V1}/payments/payment-intents", json=large_payment
        )

        if response.status_code == 400:
            self.log_test(
                "Payment Security - Amount Limit", True, {"rejected_over_limit": True}
            )
        else:
            self.log_test(
        ...
        response = self.session.post(
            f"{API_V1}/payments/payment-intents", json=negative_payment
        )

        if response.status_code == 400:
            self.log_test(
                "Payment Security - Negative Amount", True, {"rejected_negative": True}
            )
        else:
            self.log_test(
    ...
    def test_stripe_connection_status(self):
        ...
        if locations_response.status_code == 200:
            ...
            self.log_test(
                "Stripe Connection Status", False, error="Could not fetch locations"
            )
    ...
    def run_all_tests(self):
        ...
        print(
            f"Success Rate: {(self.test_results['summary']['passed'] / self.test_results['summary']['total'] * 100):.1f}%"
        )
