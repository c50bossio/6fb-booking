...
class SentryAutoFixer:
    ...
    def classify_error(self, error_message: str, stack_trace: str) -> Optional[ErrorPattern]:
    ...
    async def apply_fix(self, fix_data: Dict, error_pattern: ErrorPattern) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not fix_analysis:
                return {
                    ...
                    "reason": "Claude analysis failed"
                }
            
            # Apply fix if confidence and safety checks pass
            if error_pattern.confidence == FixConfidence.SAFE:
                fix_success, fix_message = await self.apply_fix(fix_analysis, error_pattern)
...
async def handle_sentry_webhook(request: Request):
    ...
    try:
        ...
        if payload.get("action") != "created" or payload.get("data", {}).get("issue", {}).get("type") != "error":
...
async def get_status():
    ...
    return {
        ...
        "supported_error_types": [p.error_type for p in auto_fixer.error_patterns if p.auto_fixable]
    }
