...
try:
    ...
    if not payment_model:
        ...
        print(
            f"   Payment Type: {payment_model.payment_type.value if payment_model.payment_type else 'None'}"
        )
        print(
            f"   Service Commission Rate: {payment_model.service_commission_rate * 100 if payment_model.service_commission_rate else 0}%"
        )
        print(
            f"   Product Commission Rate: {payment_model.product_commission_rate * 100 if payment_model.product_commission_rate else 0}%"
        )
        ...
        if payment_model.stripe_connect_account_id:
            ...
            print(
                f"   Onboarding Completed: {'✅ Yes' if payment_model.stripe_onboarding_completed else '❌ No'}"
            )
            print(
                f"   Payouts Enabled: {'✅ Yes' if payment_model.stripe_payouts_enabled else '❌ No'}"
            )
        else:
    ...
    if len(all_payment_models) > 1:
        ...
        for pm in all_payment_models:
            print(
                f"   - ID: {pm.id}, Active: {pm.active}, Stripe: {pm.stripe_connect_account_id or 'Not connected'}"
            )
