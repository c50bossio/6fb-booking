...
class SquareService:
    ...
    def get_oauth_url(self, barber_id: int, state: str = None) -> str:
        ...
        if not state:
            state = hashlib.sha256(f"{barber_id}-{datetime.utcnow().isoformat()}".encode()).hexdigest()
    ...
    async def get_merchant_info(self, access_token: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.api_base_url}/v2/merchants",
                headers=headers
            )
    ...
    async def create_square_account(
        ...
        existing_account = db.query(SquareAccount).filter(
        ...
        if existing_account:
            ...
            if 'expires_at' in tokens:
                existing_account.token_expires_at = datetime.fromisoformat(tokens['expires_at'])
        ...
        square_account = SquareAccount(
            ...
            token_expires_at=datetime.fromisoformat(tokens['expires_at']) if 'expires_at' in tokens else None,
    ...
    async def create_payment(
        ...
        square_payment = SquarePayment(
            ...
            approved_money=Decimal(str(payment_data.get('approved_money', {}).get('amount', 0))) / 100,
            processing_fee_money=Decimal(str(payment_data.get('processing_fee', [{}])[0].get('amount_money', {}).get('amount', 0))) / 100 if payment_data.get('processing_fee') else Decimal('0'),
        ...
        if card_details:
            ...
            square_payment.card_exp_month = card_details.get('card', {}).get('exp_month')
    ...
    async def handle_webhook(self, db: Session, webhook_data: Dict[str, Any]) -> bool:
        ...
        existing_event = db.query(SquareWebhookEvent).filter(
            SquareWebhookEvent.square_event_id == event_id
        ).first()
        ...
        webhook_event = SquareWebhookEvent(
            ...
            event_created_at=datetime.fromisoformat(webhook_data.get('created_at', datetime.utcnow().isoformat()))
        )
    ...
    async def _handle_payment_updated(self, db: Session, webhook_data: Dict[str, Any]):
        ...
        payment = db.query(SquarePayment).filter(
            SquarePayment.square_payment_id == square_payment_id
        ).first()
        
        if payment:
            payment.status = payment_data.get('status', '').lower()
            payment.approved_money = Decimal(str(payment_data.get('approved_money', {}).get('amount', 0))) / 100
            db.commit()
    ...
    def verify_webhook_signature(self, payload: str, signature: str, webhook_key: str) -> bool:
    ...
    async def refresh_access_token(self, db: Session, square_account: SquareAccount) -> str:
        ...
        if 'expires_at' in tokens:
            square_account.token_expires_at = datetime.fromisoformat(tokens['expires_at'])
