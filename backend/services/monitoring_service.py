...
class SystemMetricsCollector:
    ...
    def collect_system_metrics(self) -> List[Metric]:
        ...
        try:
            ...
            metrics.append(Metric("system.network.bytes_sent", network.bytes_sent, "bytes"))
            metrics.append(Metric("system.network.bytes_recv", network.bytes_recv, "bytes"))
            metrics.append(Metric("system.network.packets_sent", network.packets_sent, "packets"))
            metrics.append(Metric("system.network.packets_recv", network.packets_recv, "packets"))
    ...
    def collect_process_metrics(self) -> List[Metric]:
        ...
        try:
            ...
            metrics.append(Metric("process.file_descriptors.count", num_fds, "descriptors"))
        ...
        try:
            with self.db_session_factory() as db:
                ...
                try:
                    ...
                    metrics.append(Metric("database.connections.active", connection_count, "connections"))
                except Exception:
                ...
                try:
                    ...
                    metrics.append(Metric("database.appointments.count", appointment_count, "records"))
                except Exception:
                    pass
                ...
                try:
                    ...
                    metrics.append(Metric("database.users.count", user_count, "records"))
                except Exception:
                    pass
    ...
    async def collect_redis_metrics(self) -> List[Metric]:
        ...
        try:
            ...
            if 'used_memory' in info:
                metrics.append(Metric("redis.memory.used", info['used_memory'], "bytes"))
            if 'used_memory_peak' in info:
                metrics.append(Metric("redis.memory.peak", info['used_memory_peak'], "bytes"))
            
            # Connection metrics
            if 'connected_clients' in info:
                metrics.append(Metric("redis.clients.connected", info['connected_clients'], "clients"))
...
class AlertManager:
    ...
    def add_alert_rule(self, name: str, condition: Callable[[List[Metric]], bool], level: AlertLevel, message: str):
        """Add alert rule"""
    ...
    async def check_alerts(self, metrics: List[Metric], health_checks: List[HealthCheck]):
    ...
    async def create_alert(self, alert_id: str, level: AlertLevel, title: str, message: str, component: str):
...
class MonitoringService:
    ...
    def setup_default_alerts(self):
        ...
        self.alert_manager.add_alert_rule(
            "high_cpu_usage",
            lambda metrics: any(m.name == "system.cpu.usage" and m.value > 80 for m in metrics),
            AlertLevel.WARNING,
        ...
        self.alert_manager.add_alert_rule(
            "high_memory_usage",
            lambda metrics: any(m.name == "system.memory.usage" and m.value > 85 for m in metrics),
            AlertLevel.WARNING,
        ...
        self.alert_manager.add_alert_rule(
            "high_disk_usage",
            lambda metrics: any(m.name == "system.disk.usage" and m.value > 90 for m in metrics),
            AlertLevel.CRITICAL,
        ...
        self.alert_manager.add_alert_rule(
            "high_error_rate",
            lambda metrics: any(m.name == "app.error_rate" and m.value > 5 for m in metrics),
            AlertLevel.ERROR,
    ...
    async def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_checks": {name: asdict(check) for name, check in latest_health.items()},
