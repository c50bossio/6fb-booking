...
class AdvancedCacheService:
    ...
    def _init_redis(self):
        ...
        try:
            ...
            logger.warning(f"Redis connection failed: {str(e)}. Falling back to memory-only cache.")
    ...
    async def set(self, key: str, value: Any, ttl: int = None, prefix: str = None) -> bool:
    ...
    async def get_or_set(self, key: str, factory_func: Callable, ttl: int = None, prefix: str = None) -> Any:
    ...
    async def warm_up_cache(self, db: Session):
        ...
        try:
            ...
            for user in active_users:
                await self.set(f"user:{user.id}", user, self.config.LONG_TTL, self.config.USER_PREFIX)
            
            # Warm up active barbers
            active_barbers = db.query(Barber).filter(Barber.is_active == True).all()
            for barber in active_barbers:
                await self.set(f"barber:{barber.id}", barber, self.config.LONG_TTL, self.config.BARBER_PREFIX)
            
            # Warm up active services
            active_services = db.query(Service).filter(Service.is_active == True).all()
            for service in active_services:
                await self.set(f"service:{service.id}", service, self.config.LONG_TTL, self.config.BOOKING_PREFIX)
            
            logger.info(f"Cache warm-up completed. Cached {len(active_users)} users, {len(active_barbers)} barbers, {len(active_services)} services")
