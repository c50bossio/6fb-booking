...
class ClientFollowUpService:
    ...
    def _load_default_campaigns(self) -> Dict[str, FollowUpCampaign]:
        ...
        return {
            ...
            "vip_client_care": FollowUpCampaign(
                ...
                trigger_conditions={"total_spent": 1000, "visit_frequency": "high"},
                message_templates={
            ...
            "no_show_recovery": FollowUpCampaign(
                ...
                trigger_conditions={"appointment_status": "no_show"},
                message_templates={
    ...
    async def _process_campaign(self, campaign: FollowUpCampaign):
        ...
        try:
            ...
            logger.info(
                f"Processed {len(eligible_clients)} clients for campaign: {campaign.name}"
            )
    ...
    async def _find_eligible_clients(self, campaign: FollowUpCampaign) -> List[Client]:
        ...
        if campaign.target_segment == ClientSegment.NEW_CLIENT:
            ...
            query = query.filter(
                and_(Client.total_visits >= 3, Client.total_visits <= 20)
            )
            ...
            cutoff_date = date.today() - timedelta(
                days=conditions.get("days_since_last_visit", 60)
            )
            query = query.filter(
                and_(
        ...
        if "days_since_last_visit" in conditions:
            cutoff_date = date.today() - timedelta(
                days=conditions["days_since_last_visit"]
            )
            query = query.filter(Client.last_visit_date <= cutoff_date)

        return query.all()

    async def _execute_campaign_for_client(
        self, campaign: FollowUpCampaign, client: Client
    ):
        ...
        try:
            ...
            if schedule_key:
                ...
                await self._send_followup_message(
                    campaign, client, schedule_key, schedule_config
                )

        except Exception as e:
            logger.error(f"Error executing campaign for client {client.id}: {e}")

    def _determine_campaign_step(
        self, campaign: FollowUpCampaign, client: Client
    ) -> Optional[str]:
        ...
        if campaign.followup_type == FollowUpType.POST_APPOINTMENT:
            ...
            if client.last_visit_date:
                days_since = (date.today() - client.last_visit_date).days
                avg_frequency = campaign.trigger_conditions.get(
                    "average_visit_frequency", 30
                )

                if (
                    days_since >= avg_frequency - 5
                ):  # Remind 5 days before typical booking
                    return "initial"
    ...
    async def _send_followup_message(
        ...
        template_key = (
            f"{message_type}_{step_key}"
            if f"{message_type}_{step_key}" in campaign.message_templates
            else message_type
        )
    ...
    def _personalize_message(self, template: str, client: Client) -> str:
        ...
        personalizations = {
            ...
            "{last_visit}": (
                client.last_visit_date.strftime("%B %d")
                if client.last_visit_date
                else "recently"
            ),
        }
    ...
    async def _send_email_followup(
        self, client: Client, message: str, campaign: FollowUpCampaign
    ):
        """Send email follow-up"""
        logger.info(
            f"Sending email follow-up to {client.email} for campaign {campaign.name}"
        )
    ...
    async def _send_sms_followup(
        self, client: Client, message: str, campaign: FollowUpCampaign
    ):
        """Send SMS follow-up"""
        logger.info(
            f"Sending SMS follow-up to {client.phone} for campaign {campaign.name}"
        )
    ...
    async def _log_followup_sent(
        self, client: Client, campaign: FollowUpCampaign, step: str, message_type: str
    ):
    ...
    async def _process_birthday_followups(self):
        ...
        birthday_clients = [
            client
            for client in self.db.query(Client)
            .filter(Client.date_of_birth.isnot(None))
            .all()
            if client.date_of_birth
            and client.date_of_birth.month == today.month
    ...
    async def _send_birthday_message(self, client: Client):
        ...
        await self._send_email_followup(
            client,
            message,
            FollowUpCampaign(
                id="birthday",
    ...
    def toggle_campaign(self, campaign_id: str, is_active: bool):
        ...
        if campaign_id in self.campaigns:
            ...
            logger.info(
                f"Toggled campaign {campaign_id}: {'active' if is_active else 'inactive'}"
            )
