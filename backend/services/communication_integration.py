...
class CommunicationIntegrationService:
    ...
    async def send_appointment_notifications(
        ...
        try:
            ...
            barber_user = db.query(User).filter(User.id == appointment.barber_id).first()
            
            if not client or not barber_user:
                logger.error(f"Missing client or barber for appointment {appointment.id}")
                return
            ...
            appointment_data = {
                ...
                "location_name": appointment.location.name if appointment.location else "N/A",
                "location_address": appointment.location.address if appointment.location else "N/A",
    ...
    async def _send_appointment_confirmation(
        ...
        if client_user:
            prefs = db.query(NotificationPreference).filter(
                NotificationPreference.user_id == client_user.id
            ).first()
    ...
    async def send_payment_notifications(
        ...
        try:
            ...
            payment_data = {
                ...
                "payment_method": payment.payment_method.type.value if payment.payment_method else "N/A",
                "card_last_four": payment.payment_method.last_four if payment.payment_method else None,
    ...
    async def send_welcome_notification(db: Session, user: User):
        ...
        try:
            user_data = {
                ...
                "email": user.email,
                "role": user.role,
                "location": user.primary_location.name if user.primary_location else None
            }
