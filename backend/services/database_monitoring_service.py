...
class DatabaseMonitoringService:
        ...
        if metric.execution_time > self.thresholds['very_slow_query_time']:
            with sentry_sdk.push_scope() as scope:
                ...
                sentry_sdk.capture_message(
                    f"Very slow query: {metric.execution_time:.2f}s",
                    level="warning"
                )
    ...
    def _create_alert(self, alert_id: str, severity: str, title: str, 
        ...
        with sentry_sdk.push_scope() as scope:
            ...
            if severity == "critical":
                ...
                sentry_sdk.capture_message(f"{severity.upper()}: {title} - {message}", level="warning")
    ...
    def get_query_performance_summary(self, time_range_minutes: int = 60) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "performance": {
                ...
                "slow_queries": len([t for t in execution_times if t > self.thresholds['slow_query_time']]),
                "very_slow_queries": len([t for t in execution_times if t > self.thresholds['very_slow_query_time']])
            },
            "by_query_type": {
                query_type: {
                    "count": len(times),
