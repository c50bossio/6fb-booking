...
class TrainingService:
    ...
    async def get_available_modules(self, user: User) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for module in modules:
                ...
                can_access = self._can_user_access_module(
                    user.id, module, current_level
                )
                ...
                module_data = {
                    ...
                    "enrollment_status": (
                        enrollment.status if enrollment else "not_enrolled"
                    ),
    ...
    def _can_user_access_module(
        self, user_id: int, module: TrainingModule, current_cert_level: Optional[str]
    ) -> bool:
    ...
    async def enroll_user_in_module(
        self, user_id: int, module_id: int, enrolled_by: Optional[int] = None
    ) -> TrainingEnrollment:
    ...
    async def start_module_attempt(
        ...
        try:
            ...
            attempt = TrainingAttempt(
                enrollment_id=enrollment.id, attempt_number=enrollment.attempts + 1
            )
            ...
            logger.info(
                f"Started attempt {attempt.attempt_number} for user {user_id} on module {module_id}"
            )
            return attempt
    ...
    async def complete_module_attempt(
        self, attempt_id: int, score: float, answers: Dict[str, Any]
    ) -> TrainingAttempt:
        """Complete a training module attempt"""
        try:
            attempt = (
                self.db.query(TrainingAttempt)
                .filter(TrainingAttempt.id == attempt_id)
                .first()
            )
            if not attempt:
            ...
            if passed:
                ...
                enrollment.certification_points = self._calculate_certification_points(
                    module, score
                )
            else:
            ...
            logger.info(
                f"Completed attempt {attempt_id} with score {score} (passed: {passed})"
            )
            return attempt
    ...
    def _calculate_certification_points(
        self, module: TrainingModule, score: float
    ) -> float:
        """Calculate certification points based on module and score"""
        base_points = {"basic": 10, "intermediate": 15, "advanced": 20, "specialty": 12}
    ...
    async def _check_certification_eligibility(self, user_id: int):
        ...
        try:
            ...
            logger.error(
                f"Error checking certification eligibility for user {user_id}: {e}"
            )

    async def _is_user_eligible_for_certification(
        self, user_id: int, certification: Certification
    ) -> bool:
        ...
        if certification.required_modules:
            for module_id in certification.required_modules:
                ...
                if (
                    not enrollment
                    or enrollment.best_score < certification.required_score_average
                ):
                    return False
    ...
    async def _award_certification(
        self, user_id: int, certification_id: int, mentor_id: Optional[int] = None
    ):
        ...
        try:
            ...
            expiry_date = datetime.utcnow() + timedelta(
                days=certification.validity_period * 30
            )
    ...
    def _get_user_highest_certification(
        self, user_id: int
        ...
        highest_cert = max(
            user_certs, key=lambda x: cert_hierarchy.get(x.certification.level, 0)
        )
        return highest_cert
    ...
    async def get_user_training_progress(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            enrollments = (
                self.db.query(TrainingEnrollment)
                .filter(TrainingEnrollment.user_id == user_id)
                .all()
            )
            ...
            assessments = (
                self.db.query(SkillAssessment)
                .filter(SkillAssessment.user_id == user_id)
                .order_by(desc(SkillAssessment.assessment_date))
                .all()
            )
            ...
            in_progress_modules = len(
                [e for e in enrollments if e.status == "in_progress"]
            )
            ...
            progress_data = {
                "user_info": {
                    ...
                    "current_certification": (
                        current_cert.certification.level if current_cert else None
                    ),
                    "certification_date": (
                        current_cert.earned_date.isoformat() if current_cert else None
                    ),
                },
                "overall_progress": {
                    ...
                    "certification_points": sum(
                        e.certification_points or 0 for e in enrollments
                    ),
                },
                "module_progress": [
                    {
                ...
                "certifications": [
                    {
                        ...
                        "expiry_date": (
                            c.expiry_date.isoformat() if c.expiry_date else None
                        ),
                ...
                "skill_assessments": [
                    {
                        ...
                        "assessor_name": (
                            a.assessor.full_name if a.assessor else "Unknown"
                        ),
                    }
    ...
    async def get_recommended_training_path(
        ...
        try:
                    ...
                    path_modules.append(
                        {
                            ...
                            "progress": (
                                enrollment.progress_percentage if enrollment else 0
                            ),
                        }
                    )
            ...
            return {
                ...
                "overall_progress": self._calculate_path_progress(
                    user_id, training_path.ordered_modules
                ),
            }

        except Exception as e:
            logger.error(
                f"Error getting recommended training path for user {user_id}: {e}"
            )
            return None
