...
class RevenueManagementService:
    ...
    async def calculate_barber_commission(
        ...
        try:
            ...
            commission_structure = self.commission_structures.get(
                cert_level, self.commission_structures["no_certification"]
            )

            # Calculate base commission
            base_percentage = Decimal(
                str(commission_structure.base_percentage)
            ) / Decimal("100")
            base_commission = gross_revenue * base_percentage

            # Calculate certification bonus
            cert_bonus_percentage = Decimal(
                str(commission_structure.certification_bonus)
            ) / Decimal("100")
            ...
            franchise_fee_percentage = Decimal(
                str(commission_structure.franchise_fee)
            ) / Decimal("100")
    ...
    def _get_barber_certification_level(self, barber_id: int) -> str:
        ...
        try:
            ...
            user_cert = (
                ...
                .order_by(desc(UserCertification.earned_date))
                .first()
            )
            ...
            logger.error(
                f"Error getting certification level for barber {barber_id}: {e}"
            )
            return "no_certification"
    ...
    async def _calculate_performance_bonus(
        ...
        try:
            ...
            score_data = calculator.calculate_sixfb_score(
                barber_id, "monthly", start_date, end_date
            )
            ...
            base_performance_percentage = Decimal(
                str(commission_structure.performance_bonus)
            ) / Decimal("100")
            performance_bonus = (
                gross_revenue * base_performance_percentage * bonus_multiplier
            )
    ...
    async def _calculate_mentor_override(
        self,
        barber_id: int,
        gross_revenue: Decimal,
        commission_structure: CommissionStructure,
        ...
        try:
            ...
            location = (
                ...
                .filter(Location.id == barber.location_id)
                .first()
            )
            ...
            override_percentage = Decimal(
                str(commission_structure.mentor_override)
            ) / Decimal("100")
    ...
    async def calculate_location_revenue_breakdown(
        ...
        try:
            ...
            for barber in barbers:
                ...
                payout = await self.calculate_barber_commission(
                ...
                barber_breakdown = {
                    ...
                    "certification_level": self._get_barber_certification_level(
                        barber.id
                    ),
                ...
                for appointment in appointments:
                    breakdown["revenue_streams"]["service_revenue"] += Decimal(
                        str(appointment.service_revenue or 0)
                    )
                    breakdown["revenue_streams"]["product_revenue"] += Decimal(
                        str(appointment.product_revenue or 0)
                    )
                    breakdown["revenue_streams"]["tip_revenue"] += Decimal(
                        str(appointment.tip_amount or 0)
                    )
            ...
            for key in breakdown["revenue_streams"]:
                breakdown["revenue_streams"][key] = float(
                    breakdown["revenue_streams"][key]
                )
    ...
    async def get_network_revenue_analytics(
        ...
        try:
            ...
            for location in locations:
                ...
                network_analytics["network_totals"][
                    "total_gross_revenue"
                ] += location_breakdown["totals"]["gross_revenue"]
                network_analytics["network_totals"][
                    "total_commissions_paid"
                ] += location_breakdown["totals"]["barber_commissions"]
                network_analytics["network_totals"][
                    "total_franchise_fees"
                ] += location_breakdown["totals"]["franchise_fees"]
                network_analytics["network_totals"][
                    "total_mentor_overrides"
                ] += location_breakdown["totals"]["mentor_overrides"]
                network_analytics["network_totals"][
                    "network_profit"
                ] += location_breakdown["totals"]["location_profit"]
                ...
                location_perf = {
                    ...
                    "profit_margin": (
                        ...
                        if location_breakdown["totals"]["gross_revenue"] > 0
                        else 0
                    ),
                }
                ...
                for barber_data in location_breakdown["barber_breakdowns"]:
                    ...
                    if cert_level in network_analytics["commission_distribution"]:
                        network_analytics["commission_distribution"][cert_level][
                            "count"
                        ] += 1
                        network_analytics["commission_distribution"][cert_level][
                            "total_commission"
                        ] += barber_data["total_commission"]
            ...
            network_analytics["location_performance"].sort(
                key=lambda x: x["gross_revenue"], reverse=True
            )
        ...
        try:
            ...
            report = {
                ...
                "commission_structure": {
                    ...
                    "base_percentage": self.commission_structures[
                        self._get_barber_certification_level(barber_id)
                    ].base_percentage,
                    "certification_bonus_percentage": self.commission_structures[
                        self._get_barber_certification_level(barber_id)
                    ].certification_bonus,
                    "franchise_fee_percentage": self.commission_structures[
                        self._get_barber_certification_level(barber_id)
                    ].franchise_fee,
                },
                ...
                "statistics": {
                    ...
                    "average_ticket": (
                        float(payout.gross_revenue / len(appointments))
                        if appointments
                        else 0
                    ),
    ...
    async def process_bulk_payouts(
        ...
        try:
            ...
            if not end_date:
                end_date = date.today().replace(day=1) - timedelta(
                    days=1
