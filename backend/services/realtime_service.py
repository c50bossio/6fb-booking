...
class ConnectionManager:
    ...
    async def connect(
        ...
        logger.info(
            f"WebSocket connected: {connection_id} (user: {user_id}, barber: {barber_id})"
        )
    ...
    async def broadcast(
        self, event: RealtimeEvent, exclude_connections: Set[str] = None
    ):
        ...
        if self.redis_client:
            try:
                await self.redis_client.publish(
                    self.redis_channel, json.dumps(event.to_dict())
                )
            except Exception as e:
...
class RealtimeEventService:
    ...
    def _register_default_handlers(self):
        ...
        self.connection_manager.add_event_handler(
            EventType.USER_ONLINE, self._handle_user_status_change
        )
        self.connection_manager.add_event_handler(
            EventType.USER_OFFLINE, self._handle_user_status_change
        )
    ...
    async def barber_status_changed(
        self, barber_id: str, status: str, location_id: str = None
    ):
