...
class OptimizedAnalyticsService:
    ...
    def get_dashboard_metrics(
        ...
        base_query = self.db.query(
            ...
            func.sum(func.coalesce(Appointment.service_revenue, 0)).label('service_revenue'),
            func.sum(func.coalesce(Appointment.tip_amount, 0)).label('tip_revenue'),
            func.sum(func.coalesce(Appointment.product_revenue, 0)).label('product_revenue'),
            
            # Appointment counts
            func.count(Appointment.id).label('total_appointments'),
            func.sum(case((Appointment.status == 'completed', 1), else_=0)).label('completed_appointments'),
            func.sum(case((Appointment.status == 'cancelled', 1), else_=0)).label('cancelled_appointments'),
            func.sum(case((Appointment.status == 'no_show', 1), else_=0)).label('no_show_appointments'),
            
            # Client metrics
            func.count(func.distinct(Appointment.client_id)).label('unique_clients'),
            func.sum(case((Appointment.customer_type == 'new', 1), else_=0)).label('new_clients'),
            func.sum(case((Appointment.customer_type == 'returning', 1), else_=0)).label('returning_clients'),
            ...
            func.avg(func.coalesce(Appointment.duration_minutes, 60)).label('avg_duration'),
        ).filter(
            and_(
        ...
        # Apply filters
        if location_id:
            base_query = base_query.join(Barber).filter(Barber.location_id == location_id)
        if barber_id:
        ...
        if result.total_appointments > 0:
            completion_rate = (result.completed_appointments / result.total_appointments) * 100
            
        no_show_rate = 0
        if result.total_appointments > 0:
            no_show_rate = (result.no_show_appointments / result.total_appointments) * 100
    ...
    def get_revenue_trends(
        ...
        return [
            {
                'period': result.period.strftime(date_format) if hasattr(result.period, 'strftime') else str(result.period),
    ...
    def get_barber_performance_summary(
        ...
        query = self.db.query(
            ...
            func.count(func.distinct(Appointment.client_id)).label('unique_clients'),
            func.avg(func.coalesce(Appointment.client_satisfaction, 4.5)).label('avg_rating')
        ).select_from(Barber).join(
        ...
        for result in results:
            ...
            performance_data.append({
                ...
                'clients': {
                    'unique': result.unique_clients or 0
                },
    ...
    def get_service_analytics(
        ...
        results = query.group_by(Appointment.service_name).order_by(
    ...
    def get_client_retention_metrics(
        ...
        if location_id:
            client_query = client_query.join(Barber).filter(Barber.location_id == location_id)
