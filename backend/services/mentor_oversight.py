...
class MentorOversightService:
    ...
    async def get_mentor_dashboard(self, mentor_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            assigned_locations = (
                self.db.query(Location).filter(Location.mentor_id == mentor_id).all()
            )
            ...
            dashboard_data = {
                "mentor_info": {
                    ...
                    "mentor_since": (
                        mentor.mentor_since.isoformat() if mentor.mentor_since else None
                    ),
                },
                "overview": {
            ...
            for location in assigned_locations:
                location_analytics = (
                    await self.location_service.calculate_location_analytics(
                        location.id, "weekly"
                    )
                )
                ...
                dashboard_data["location_performance"].append(
                    {
                        ...
                        "target_achievement": location_analytics["revenue"][
                            "target_achievement"
                        ],
                        "status": (
                            "above_target"
            ...
            dashboard_data["overview"]["average_network_score"] = (
                total_score / len(assigned_locations) if assigned_locations else 0
            )
    ...
    async def _get_mentee_progress(self, barber: Barber) -> MenteeProgress:
        ...
        try:
            ...
            previous_score = calculator.calculate_sixfb_score(
                barber.id, "weekly", prev_start, prev_end
            )
            ...
            strengths, areas_for_improvement = self._analyze_barber_performance(
                current_score
            )
    ...
    def _analyze_barber_performance(
        ...
        for component, value in components.items():
            ...
            if value >= 90:
                ...
                areas_for_improvement.append(
                    f"{component_name} needs improvement ({value:.1f})"
                )
            elif value < 80:
                areas_for_improvement.append(
                    f"{component_name} has room for growth ({value:.1f})"
                )

        return strengths, areas_for_improvement

    def _generate_next_actions(
        self, score_data: Dict[str, Any], trend: str
    ) -> List[str]:
        ...
        if components:
            ...
            if lowest_component[1] < 70:
                actions.append(
                    f"Priority: Focus on {component_name} - schedule training session"
                )

            # Specific actions based on component
            if (
                "booking_utilization" in lowest_component[0]
                and lowest_component[1] < 75
            ):
    ...
    # Mentee Management
    async def get_mentee_detailed_report(
        self, mentor_id: int, barber_id: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            location = (
                ...
                .filter(
                    and_(
                        Location.id == barber.location_id,
            ...
            if not location:
                raise ValueError(
                    f"Mentor {mentor_id} does not have access to barber {barber_id}"
                )
            ...
            report = {
                ...
                "current_performance": calculator.calculate_sixfb_score(
                    barber.id, "weekly"
                ),
                "monthly_performance": calculator.calculate_sixfb_score(
                    barber.id, "monthly"
                ),
    ...
    async def _get_performance_trend(self, barber_id: int) -> Dict[str, Any]:
        ...
        for week in range(8):
            ...
            score = calculator.calculate_sixfb_score(
            ...
            appointment_data.append(
                {
                    ...
                    "revenue": (apt.service_revenue or 0)
                    + (apt.tip_amount or 0)
                    + (apt.product_revenue or 0),
                    "status": apt.status,
    ...
    # Goal Management
    async def create_mentee_goal(
        self, mentor_id: int, barber_id: int, goal_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not barber
                or not barber.location
                or barber.location.mentor_id != mentor_id
            ):
    ...
    async def add_mentor_note(
        self, mentor_id: int, barber_id: int, note_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_mentor_network_analytics(self, mentor_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for location in locations:
                analytics = await self.location_service.calculate_location_analytics(
                    location.id, "weekly"
                )
            ...
            network_data["summary"]["average_score"] = (
                total_score / len(locations) if locations else 0
            )
