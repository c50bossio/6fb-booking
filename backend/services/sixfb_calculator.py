...
class SixFBCalculator:
    ...
    def calculate_daily_metrics(self, barber_id: int, target_date: date) -> Dict:
        ...
        total_service_revenue = sum(apt.service_revenue or 0 for apt in completed_appointments)
        total_tips = sum(apt.tip_amount or 0 for apt in completed_appointments)
        total_product_revenue = sum(apt.product_revenue or 0 for apt in completed_appointments)
        total_revenue = total_service_revenue + total_tips + total_product_revenue
        
        # Customer type breakdown
        new_customers = [apt for apt in completed_appointments if apt.customer_type == "new"]
        returning_customers = [apt for apt in completed_appointments if apt.customer_type == "returning"]
        ...
        returning_customer_revenue = sum(apt.total_revenue for apt in returning_customers)
        
        # Average calculations
        average_ticket = total_revenue / completed_count if completed_count > 0 else 0
        average_service_revenue = total_service_revenue / completed_count if completed_count > 0 else 0
        average_tip_percentage = (total_tips / total_service_revenue * 100) if total_service_revenue > 0 else 0
        
        # Get barber's daily capacity target
        barber = self.db.query(Barber).filter(Barber.id == barber_id).first()
        daily_capacity = (barber.target_booking_capacity or 40) // 7  # Weekly target / 7 days
        booking_rate = (completed_count / daily_capacity * 100) if daily_capacity > 0 else 0
    ...
    def calculate_weekly_metrics(self, barber_id: int, week_start: date) -> Dict:
        ...
        booking_rate = (weekly_totals["completed_appointments"] / weekly_capacity * 100) if weekly_capacity > 0 else 0
        
        average_ticket = weekly_totals["total_revenue"] / weekly_totals["completed_appointments"] if weekly_totals["completed_appointments"] > 0 else 0
        
        # Calculate week-over-week growth
        previous_week_start = week_start - timedelta(days=7)
        previous_week_metrics = self.calculate_weekly_metrics(barber_id, previous_week_start) if week_start > date.today() - timedelta(days=365) else None
        
        revenue_growth_rate = 0
        appointment_growth_rate = 0
        if previous_week_metrics:
            if previous_week_metrics["total_revenue"] > 0:
                revenue_growth_rate = ((weekly_totals["total_revenue"] - previous_week_metrics["total_revenue"]) / previous_week_metrics["total_revenue"]) * 100
            if previous_week_metrics["completed_appointments"] > 0:
                appointment_growth_rate = ((weekly_totals["completed_appointments"] - previous_week_metrics["completed_appointments"]) / previous_week_metrics["completed_appointments"]) * 100
    ...
    def calculate_customer_analytics(self, barber_id: int, period_days: int = 30) -> Dict:
        ...
        for apt in appointments:
            customer_types[apt.customer_type] = customer_types.get(apt.customer_type, 0) + 1
        ...
        at_risk_customers = [c for c in clients if c.last_visit_date and c.last_visit_date < at_risk_date]
    ...
    def calculate_sixfb_score(self, barber_id: int, period_type: str = "weekly") -> Dict:
        ...
        revenue_growth_score = max(0, min(100, 50 + revenue_growth))  # 50 is baseline, growth adds points
        
        # Customer retention score (based on returning customer percentage)
        total_customers = metrics.get("new_customers", 0) + metrics.get("returning_customers", 0)
        retention_percentage = (metrics.get("returning_customers", 0) / total_customers * 100) if total_customers > 0 else 0
        ...
        ticket_achievement = (actual_ticket / target_ticket * 100) if target_ticket > 0 else 0
    ...
    def _calculate_retention_rate(self, barber_id: int, period_days: int) -> float:
        ...
        previous_customers = set(
            apt.client_id for apt in self.db.query(Appointment).filter(
                and_(
                    Appointment.barber_id == barber_id,
