...
class AutomatedReportingService:
    ...
    def _should_generate_report(self, config: ReportConfig, current_date: date, current_time) -> bool:
    ...
    async def _generate_report_data(self, config: ReportConfig, barber_id: Optional[int]) -> ReportData:
        ...
        if config.report_type == ReportType.DAILY_SUMMARY:
            ...
            data = await self._generate_weekly_performance(barber_id, period_start, period_end)
        
        elif config.report_type == ReportType.MONTHLY_OVERVIEW:
            data = await self._generate_monthly_overview(barber_id, period_start, period_end)
        
        elif config.report_type == ReportType.SIXFB_SCORECARD:
            data = await self._generate_sixfb_scorecard(barber_id, period_start, period_end)
        
        elif config.report_type == ReportType.CLIENT_RETENTION:
            data = await self._generate_client_retention_report(barber_id, period_start, period_end)
        
        elif config.report_type == ReportType.REVENUE_ANALYSIS:
            data = await self._generate_revenue_analysis(barber_id, period_start, period_end)
        ...
        recommendations = self._generate_recommendations(data, config.report_type) if config.include_recommendations else []
    ...
    async def _generate_daily_summary(self, barber_id: Optional[int], report_date: date) -> Dict[str, Any]:
        """Generate daily summary report"""
        query = self.db.query(Appointment).filter(Appointment.appointment_date == report_date)
        ...
        total_revenue = sum(a.service_revenue + a.tip_amount + a.product_revenue for a in appointments if a.status == 'completed')
        ...
        return {
            ...
            'revenue': {
                ...
                'target_achievement': (total_revenue / 300) * 100  # Assuming $300 daily target
            },
            'clients': {
                'new': new_clients,
    ...
    async def _generate_weekly_performance(self, barber_id: Optional[int], start_date: date, end_date: date) -> Dict[str, Any]:
        """Generate weekly performance report"""
        # Use existing calculator
        if barber_id:
            weekly_metrics = await self.calculator.get_weekly_metrics(barber_id, start_date, end_date)
            sixfb_score = self.calculator.calculate_sixfb_score(barber_id, "weekly")
        else:
            # Aggregate all barbers
            weekly_metrics = await self._get_business_weekly_metrics(start_date, end_date)
        ...
        return {
            ...
            'performance_grade': self._calculate_performance_grade(sixfb_score.get('overall_score', 0))
        }
    
    async def _generate_monthly_overview(self, barber_id: Optional[int], start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        # Monthly metrics
        total_revenue = sum(a.service_revenue + a.tip_amount + a.product_revenue for a in completed)
        ...
        while current_week_start <= end_date:
            ...
            week_appointments = [a for a in completed if current_week_start <= a.appointment_date <= week_end]
            week_revenue = sum(a.service_revenue + a.tip_amount + a.product_revenue for a in week_appointments)
        ...
        return {
            ...
            'summary': {
                ...
                'client_retention_rate': ((unique_clients - new_clients) / unique_clients * 100) if unique_clients > 0 else 0
            },
            'weekly_breakdown': weekly_breakdown
        }
    
    async def _generate_sixfb_scorecard(self, barber_id: Optional[int], start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        if barber_id:
            ...
            previous_score = self.calculator.calculate_sixfb_score(barber_id, "weekly", prev_start, prev_end)
        else:
            current_score = await self._get_business_sixfb_score()
            previous_score = await self._get_business_sixfb_score(start_date - timedelta(days=7))
        
        # Calculate trends
        score_change = current_score.get('overall_score', 0) - previous_score.get('overall_score', 0)
        ...
        return {
            ...
            'performance_grade': self._calculate_performance_grade(current_score.get('overall_score', 0))
        }
    
    async def _generate_client_retention_report(self, barber_id: Optional[int], start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        # Categorize clients
        active_clients = [c for c in clients if c.last_visit_date and c.last_visit_date >= start_date]
        at_risk_clients = [c for c in clients if c.last_visit_date and 
                          (date.today() - c.last_visit_date).days > 45]
        lost_clients = [c for c in clients if c.last_visit_date and 
                       (date.today() - c.last_visit_date).days > 90]
        ...
        return {
            ...
            'retention_metrics': {
                'retention_rate': (len(active_clients) / len(clients) * 100) if clients else 0,
                'churn_rate': (len(lost_clients) / len(clients) * 100) if clients else 0
            }
        }
    
    async def _generate_revenue_analysis(self, barber_id: Optional[int], start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        for appointment in appointments:
            ...
            daily_revenue[date_str] += appointment.service_revenue + appointment.tip_amount + appointment.product_revenue
        ...
        return {
            ...
            'revenue_per_appointment': total_revenue / len(appointments) if appointments else 0
        }
    
    def _generate_insights(self, data: Dict[str, Any], report_type: ReportType) -> List[str]:
        ...
        if report_type == ReportType.DAILY_SUMMARY:
            ...
            if completion_rate < 85:
                insights.append(f"Appointment completion rate is {completion_rate:.1f}% - consider reviewing no-show policies")
            
            revenue = data.get('revenue', {}).get('total', 0)
            if revenue < 250:
                insights.append("Daily revenue below target - focus on upselling and premium services")
            ...
            if score >= 90:
                insights.append("Excellent 6FB performance! You're in the top tier of barbers.")
            elif score >= 80:
                insights.append("Strong 6FB performance with room for optimization.")
            elif score < 70:
                insights.append("6FB score needs attention - focus on the lowest-performing components.")
            
            if trend == 'improving':
                insights.append("Your 6FB score is trending upward - keep up the great work!")
            elif trend == 'declining':
                insights.append("6FB score is declining - review recent changes and client feedback.")
    ...
    def _generate_recommendations(self, data: Dict[str, Any], report_type: ReportType) -> List[str]:
        ...
        if report_type == ReportType.DAILY_SUMMARY:
            ...
            if revenue < 250:
                recommendations.append("Increase average ticket by offering premium services and products")
            ...
            if retention_rate < 70:
                recommendations.append("Implement automated follow-up campaigns for inactive clients")
                recommendations.append("Gather feedback from departing clients")
            
            if at_risk > 10:
                recommendations.append("Create reactivation campaign for at-risk clients")
                recommendations.append("Offer special promotions to re-engage inactive clients")
        
        return recommendations
    
    async def _send_report(self, config: ReportConfig, report_data: ReportData, barber: Optional[Barber]):
    ...
    async def _send_email_report(self, config: ReportConfig, report_data: ReportData, barber: Optional[Barber]):
    ...
    async def _send_pdf_report(self, config: ReportConfig, report_data: ReportData, barber: Optional[Barber]):
        """Send report as PDF"""
        # In production, generate PDF and send via email
        logger.info(f"PDF report generated and sent: {config.name}")
    
    def _generate_html_report(self, report_data: ReportData, config: ReportConfig) -> str:
    ...
    # Helper methods
    async def _get_business_weekly_metrics(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        for barber in barbers:
            barber_metrics = await self.calculator.get_weekly_metrics(barber.id, start_date, end_date)
    ...
    async def _get_business_sixfb_score(self, reference_date: Optional[date] = None) -> Dict[str, Any]:
    ...
    async def generate_on_demand_report(self, report_type: ReportType, barber_id: Optional[int] = None) -> ReportData:
