...
class SmartSchedulingService:
    ...
    async def _analyze_historical_patterns(self):
        ...
        for appointment in appointments:
            ...
            hour = appointment.appointment_time.hour if appointment.appointment_time else 12
        ...
        for pattern in patterns.values():
            if pattern['count'] > 0:
                ...
                pattern['completion_rate'] = pattern['completion_rate'] / pattern['count'] * 100
        ...
        for client in clients:
            ...
            if len(appointments) >= 2:
                ...
                client_patterns[client.id] = {
                    ...
                    'avg_spend': sum(a.service_revenue + a.tip_amount + a.product_revenue for a in appointments) / len(appointments)
                }
    ...
    async def _analyze_demand_patterns(self):
        ...
        normalized_hourly = {h: (count / max_hourly_demand * 100) for h, count in demand_by_hour.items()}
        normalized_daily = {d: (count / max_daily_demand * 100) for d, count in demand_by_day.items()}
    ...
    async def generate_recommendations(self, barber_id: int, target_date: date = None) -> List[SchedulingRecommendation]:
        """Generate scheduling recommendations for barber"""
        if not target_date:
            target_date = date.today() + timedelta(days=1)
        
        logger.info(f"Generating scheduling recommendations for barber {barber_id} on {target_date}")
        ...
        try:
            ...
            recommendations.extend(await self._recommend_optimal_times(barber_id, target_date))
            recommendations.extend(await self._recommend_gap_filling(barber_id, target_date))
            recommendations.extend(await self._recommend_client_outreach(barber_id, target_date))
            recommendations.extend(await self._recommend_revenue_optimization(barber_id, target_date))
            
            # Sort by confidence score and expected revenue
            recommendations.sort(key=lambda x: (x.confidence_score, x.expected_revenue), reverse=True)
    ...
    async def _recommend_optimal_times(self, barber_id: int, target_date: date) -> List[SchedulingRecommendation]:
        ...
        for hour in range(9, 18):
            if hour not in booked_hours:
                ...
                if demand_score > 60 and avg_revenue > 40:  # Only recommend good time slots
    ...
    async def _recommend_gap_filling(self, barber_id: int, target_date: date) -> List[SchedulingRecommendation]:
    ...
    async def _recommend_client_outreach(self, barber_id: int, target_date: date) -> List[SchedulingRecommendation]:
        ...
        for client_id, pattern in self.client_patterns.items():
            ...
            if days_since_last >= (avg_interval * 0.8):
                ...
                if target_day_score > 0:  # Client has booked on this day before
                    ...
                    optimal_hour = statistics.mode(preferred_hours) if preferred_hours else 12
                    
                    suggested_time = datetime.combine(target_date, time(optimal_hour, 0))
    ...
    async def _recommend_revenue_optimization(self, barber_id: int, target_date: date) -> List[SchedulingRecommendation]:
    ...
    async def get_schedule_optimization_analysis(self, barber_id: int, 
        ...
        for day_offset in range(analysis_period_days):
            ...
            day_appointments = [a for a in appointments if a.appointment_date == current_date]
            ...
            if day_appointments:
                ...
                day_revenue = sum(
                    (a.service_revenue or 50) + (a.tip_amount or 0) + (a.product_revenue or 0)
                ...
                daily_analysis[current_date.isoformat()] = {
                    ...
                    'gaps': gaps,
                    'utilization': min(100, (day_hours / 8) * 100)  # Assuming 8-hour workday
                }
            else:
                daily_analysis[current_date.isoformat()] = {
                    'appointments': 0,
        ...
        avg_utilization = statistics.mean([daily_analysis[day]['utilization'] for day in daily_analysis])
        
        # Generate optimization suggestions
        optimization_suggestions = []
        
        if avg_utilization < 70:
            optimization_suggestions.append("Increase booking volume - schedule utilization is below 70%")
        
        if total_revenue / analysis_period_days < 400:
            optimization_suggestions.append("Focus on revenue optimization - daily average below target")
        ...
        return {
            ...
            'current_metrics': {
                ...
                'avg_revenue_per_appointment': total_revenue / total_appointments if total_appointments > 0 else 0
            },
            'daily_breakdown': daily_analysis,
            'optimization_opportunities': optimization_suggestions,
            'potential_revenue_increase': self._calculate_potential_revenue_increase(daily_analysis)
        }
    
    def _calculate_potential_revenue_increase(self, daily_analysis: Dict[str, Any]) -> float:
        ...
        for day_data in daily_analysis.values():
            ...
            if utilization < 80:
                potential_additional_revenue = (80 - utilization) / 100 * 400  # $400 target daily
                total_potential += potential_additional_revenue
        
        return total_potential
    
    async def get_optimal_time_slots(self, barber_id: int, target_date: date) -> List[TimeSlot]:
...
async def generate_scheduling_recommendations(barber_id: int, target_date: date = None) -> List[SchedulingRecommendation]:
