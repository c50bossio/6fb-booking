...
class ThreatDetector:
    ...
    def _check_rapid_requests(self, ip_address: str, window_seconds: int = 60, max_requests: int = 50) -> bool:
        ...
        if ip_address in self.suspicious_patterns:
            self.suspicious_patterns[ip_address] = [
                timestamp for timestamp in self.suspicious_patterns[ip_address]
    ...
    def _check_suspicious_headers(self, ip_address: str, headers: Dict[str, str]) -> List[SecurityEvent]:
    ...
    def _update_tracking_data(self, ip_address: str, endpoint: str, user_id: Optional[int]):
        ...
        for event in last_day_events:
            if event.ip_address:
                if event.ip_address not in ip_risk_scores:
                    ip_risk_scores[event.ip_address] = self.threat_detector.get_ip_risk_score(event.ip_address)
