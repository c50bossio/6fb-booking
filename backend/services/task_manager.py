...
class TaskWorker:
    ...
    def __init__(self, worker_id: str, task_queue: TaskQueue, task_registry: Dict[str, Callable]):
    ...
    async def execute_function(self, func: Callable, args: List[Any], kwargs: Dict[str, Any]) -> Any:
    ...
    async def handle_task_failure(self, task: Task):
        ...
        if task.retry_count <= task.max_retries:
            ...
            logger.warning(f"Task {task.id} failed, retry {task.retry_count}/{task.max_retries}")
        else:
...
class TaskScheduler:
    ...
    async def schedule_task(self, task: Task, delay_seconds: int = None, at_time: datetime = None):
    ...
    async def schedule_recurring(
        ...
        self.recurring_tasks[name] = {
            ...
            'kwargs': kwargs or {},
            'last_run': None,
            'next_run': datetime.utcnow() if start_immediately else datetime.utcnow() + timedelta(seconds=interval_seconds)
        }
        logger.info(f"Recurring task '{name}' scheduled with interval {interval_seconds}s")
    ...
    async def check_recurring_tasks(self):
        ...
        for name, config in self.recurring_tasks.items():
            if config['next_run'] <= now:
                ...
                logger.info(f"Recurring task '{name}' enqueued, next run: {config['next_run']}")
