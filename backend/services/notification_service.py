...
class NotificationService:
    ...
    async def send_appointment_notification(
        db: Session, user_id: int, appointment_data: dict, notification_type: str
    ):
        ...
        logger.info(
            f"Sent appointment notification to user {user_id}: {notification_type}"
        )
    ...
    async def send_team_update(
        db: Session, location_id: int, update_type: str, data: dict
    ):
        ...
        users = (
            db.query(User)
            .filter(User.primary_location_id == location_id, User.is_active == True)
            .all()
        )
    ...
    async def send_training_notification(
        db: Session, user_id: int, module_name: str, notification_type: str, data: dict
    ):
    ...
    async def send_revenue_milestone(
        db: Session, user_id: int, milestone_type: str, amount: float, period: str
    ):
        ...
        notification = Notification(
            ...
            message=message,
            data={"milestone_type": milestone_type, "amount": amount, "period": period},
            is_read=False,
            priority="high",
        )
        db.add(notification)
