...
class TeamAnalyticsService:
        ...
        try:
            ...
            analytics = {
                "barber_info": {
                    ...
                    "location_name": (
                        barber.location.name if barber.location else "No Location"
                    ),
                },
                "period": {
                ...
                "revenue_analysis": await self._analyze_barber_revenue(
                    barber_id, completed_appointments
                ),
                "client_analysis": await self._analyze_barber_clients(
                    barber_id, completed_appointments
                ),
    ...
    async def _calculate_score_trend(
        self, barber_id: int, weeks: int = 8
    ) -> Dict[str, Any]:
        ...
        for week in range(weeks):
            ...
            score = self.calculator.calculate_sixfb_score(
                barber_id, "weekly", week_start, week_end
            )
            trend_data.append(
                {
        ...
        if len(trend_data) >= 4:
            ...
            trend_direction = (
                ...
                else "declining" if recent_avg < older_avg - 2 else "stable"
            )
        else:
        ...
        return {
            ...
            "best_week_score": (
                max(trend_data, key=lambda x: x["score"]) if trend_data else None
            ),
            "average_score": (
                statistics.mean([w["score"] for w in trend_data]) if trend_data else 0
            ),
        }

    async def _analyze_barber_revenue(
        self, barber_id: int, appointments: List[Appointment]
    ) -> Dict[str, Any]:
        ...
        weekly_revenue = {}
        for appointment in appointments:
            week_start = appointment.appointment_date - timedelta(
                days=appointment.appointment_date.weekday()
            )
        ...
        return {
            ...
            "best_revenue_day": (
                max(revenue_by_day.items(), key=lambda x: x[1])
                if revenue_by_day
                else None
            ),
        }
    ...
    async def _analyze_barber_clients(
        self, barber_id: int, appointments: List[Appointment]
    ) -> Dict[str, Any]:
        ...
        for appointment in appointments:
            if appointment.client_id:
                client_frequency[appointment.client_id] = (
                    client_frequency.get(appointment.client_id, 0) + 1
                )
        ...
        for appointment in appointments:
            if appointment.client_id:
                ...
                client_revenue[appointment.client_id] = (
                    client_revenue.get(appointment.client_id, 0) + revenue
                )
        ...
        return {
            ...
            "client_frequency": dict(
                sorted(client_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "top_clients": [
                {"client_id": cid, "revenue": rev} for cid, rev in top_clients
            ],
        }
    ...
    async def _calculate_efficiency_metrics(
        ...
        cancellation_rate = (
            (cancellations / total_appointments * 100) if total_appointments > 0 else 0
        )
    ...
    async def _get_barber_rankings(self, barber_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            location_barbers = (
                ...
                .filter(Barber.location_id == barber.location_id)
                .all()
            )
            ...
            location_rank = next(
                (
                    i + 1
                    for i, (bid, _) in enumerate(location_scores)
                    if bid == barber_id
                ),
                0,
            )
            ...
            # Network ranking
            network_scores = sorted(
                barber_scores.items(), key=lambda x: x[1], reverse=True
            )
            network_rank = next(
                (
                    i + 1
                    for i, (bid, _) in enumerate(network_scores)
                    if bid == barber_id
                ),
                0,
            )
            ...
            lower_scores = sum(
                1 for score in barber_scores.values() if score < current_score
            )
            percentile = (
                (lower_scores / len(barber_scores) * 100) if barber_scores else 0
            )
    ...
    async def _generate_improvement_recommendations(
        self, barber_id: int, current_score: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    # Team Comparison Analytics
    async def get_team_comparison_analytics(
        ...
        try:
            ...
            if location_ids:
                locations = (
                    self.db.query(Location).filter(Location.id.in_(location_ids)).all()
                )
            else:
                locations = (
                    self.db.query(Location).filter(Location.is_active == True).all()
                )
            ...
            comparison_data["summary"]["avg_team_size"] = (
                total_barbers / len(locations) if locations else 0
            )
            comparison_data["summary"]["avg_network_score"] = (
            ...
            comparison_data["network_benchmarks"] = {
                "top_quartile_score": (
                    statistics.quantiles(all_scores, n=4)[2]
                    if len(all_scores) >= 4
                    else 0
                ),
                "median_score": statistics.median(all_scores) if all_scores else 0,
                "bottom_quartile_score": (
                    statistics.quantiles(all_scores, n=4)[0]
                    if len(all_scores) >= 4
                    else 0
                ),
                "revenue_per_barber_benchmark": (
                    total_revenue / total_barbers if total_barbers > 0 else 0
                ),
            }
    ...
    async def _get_location_team_analytics(self, location_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            barbers = (
                self.db.query(Barber).filter(Barber.location_id == location_id).all()
            )
            ...
            team_cohesion = max(
                0, 100 - (score_std_dev * 2)
            )  # Higher cohesion = lower standard deviation

            # Find top performer
            top_performer = (
                max(barber_performances, key=lambda x: x["score"])
                if barber_performances
                else None
            )
            ...
            logger.error(
                f"Error getting team analytics for location {location_id}: {e}"
            )
            return self._empty_team_analytics(location_id)

    def _empty_team_analytics(
        self, location_id: int, location_name: str = "Unknown"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            insights = {
                "network_overview": {
                    ...
                    "avg_network_score": (
                        statistics.mean(network_scores) if network_scores else 0
                    ),
                    "total_network_revenue": network_revenue,
                    "revenue_per_barber": (
                        network_revenue / len(all_barbers) if all_barbers else 0
                    ),
                },
                "performance_distribution": {
                    "excellent_performers": len(
                        [s for s in network_scores if s >= 90]
                    ),  # A grade
                    "good_performers": len(
                        [s for s in network_scores if 80 <= s < 90]
                    ),  # B grade
                    "average_performers": len(
                        [s for s in network_scores if 70 <= s < 80]
                    ),  # C grade
                    "needs_improvement": len(
                        [s for s in network_scores if s < 70]
                    ),  # Below C
                },
                "benchmarks": {
                    "top_10_percent_score": (
                        statistics.quantiles(network_scores, n=10)[8]
                        if len(network_scores) >= 10
                        else 0
                    ),
                    "median_score": (
                        statistics.median(network_scores) if network_scores else 0
                    ),
                    "bottom_10_percent_score": (
                        statistics.quantiles(network_scores, n=10)[0]
                        if len(network_scores) >= 10
                        else 0
                    ),
                },
