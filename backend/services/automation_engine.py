...
class AutomationEngine:
    ...
    def _load_default_rules(self):
        ...
        self.rules = {
            ...
            "appointment_reminder": AutomationRule(
                ...
                trigger_conditions={"schedule": "daily", "time": "10:00"},
                actions=[
                    {
            ...
            "revenue_target_missed": AutomationRule(
                ...
                trigger_conditions={"period": "weekly", "threshold_percentage": 80},
                actions=[
                    {
                        "type": ActionType.SEND_ALERT.value,
                        "severity": "high",
            ...
            "inactive_client_reactivation": AutomationRule(
                ...
                trigger_conditions={"days_inactive": 60},
                actions=[
                    {
                        "type": ActionType.SEND_EMAIL.value,
    ...
    async def _build_context(self, trigger_data: Dict[str, Any]) -> AutomationContext:
        ...
        if "appointment_id" in trigger_data:
            context.appointment = self.db.query(Appointment).get(
                trigger_data["appointment_id"]
            )
            if context.appointment:
                context.client = self.db.query(Client).get(
                    context.appointment.client_id
                )
                context.barber = self.db.query(Barber).get(
                    context.appointment.barber_id
                )
        ...
        if context.barber:
            context.sixfb_score = self.calculator.calculate_sixfb_score(
                context.barber.id
            )
    ...
    def _find_matching_rules(
        self, trigger_type: TriggerType, context: AutomationContext
    ) -> List[AutomationRule]:
        ...
        for rule in self.rules.values():
            ...
            if (
                rule.barber_id
                and context.barber
                and rule.barber_id != context.barber.id
            ):
                continue
    ...
    def _check_trigger_conditions(
        self, rule: AutomationRule, context: AutomationContext
    ) -> bool:
        ...
        try:
            ...
            # 6FB score condition
            if (
                "metric" in conditions
                and conditions["metric"] == "sixfb_score"
                and context.sixfb_score
            ):
            ...
            if "days_inactive" in conditions and context.client:
                ...
                if context.client.last_visit_date:
                    days_since_visit = (
                        datetime.now().date() - context.client.last_visit_date
                    ).days
    ...
    def _generate_email_content(
        self, template: str, context: AutomationContext
    ) -> Dict[str, str]:
    ...
    def _generate_sms_content(self, template: str, context: AutomationContext) -> str:
        ...
        return templates.get(
            template, "This is an automated message from your barbershop."
        )

    async def _create_report(
        self, report_type: str, context: AutomationContext
    ) -> Dict[str, Any]:
    ...
    def toggle_rule(self, rule_id: str, is_active: bool):
        ...
        if rule_id in self.rules:
            ...
            logger.info(
                f"Toggled rule {rule_id}: {'active' if is_active else 'inactive'}"
            )
