...
class MetricsService:
    ...
    def update_daily_metrics(self, barber_id: int, target_date: date) -> DailyMetrics:
        ...
        calculated_metrics = self.calculator.calculate_daily_metrics(
            barber_id, target_date
        )
        ...
        existing_metrics = (
            ...
            .filter(
                DailyMetrics.barber_id == barber_id, DailyMetrics.date == target_date
            )
            .first()
        )
        ...
        if existing_metrics:
            ...
            metrics_record = DailyMetrics(
                ...
                returning_customer_revenue=calculated_metrics[
    ...
    def update_weekly_metrics(self, barber_id: int, week_start: date) -> WeeklyMetrics:
        ...
        """
        calculated_metrics = self.calculator.calculate_weekly_metrics(
            barber_id, week_start
        )
        ...
        if existing_metrics:
            # Update existing record
            existing_metrics.total_appointments = calculated_metrics[
                "total_appointments"
            ]
            existing_metrics.completed_appointments = calculated_metrics[
                "completed_appointments"
            ]
            existing_metrics.total_service_revenue = calculated_metrics[
                "total_service_revenue"
            ]
            existing_metrics.total_tip_amount = calculated_metrics["total_tips"]
            existing_metrics.total_product_revenue = calculated_metrics[
            ...
            existing_metrics.returning_customers = calculated_metrics[
                "returning_customers"
            ]
            ...
            existing_metrics.revenue_growth_rate = calculated_metrics[
                "revenue_growth_rate"
            ]
            existing_metrics.appointment_growth_rate = calculated_metrics[
    ...
    def update_sixfb_score(
        self, barber_id: int, calculation_date: date, period_type: str = "weekly"
    ) -> SixFBScore:
        """
        ...
        if existing_score:
            # Update existing record
            existing_score.booking_utilization_score = calculated_score["components"][
                "booking_utilization_score"
            ]
            existing_score.revenue_growth_score = calculated_score["components"][
                "revenue_growth_score"
            ]
            existing_score.customer_retention_score = calculated_score["components"][
                "customer_retention_score"
            ]
            existing_score.average_ticket_score = calculated_score["components"][
                "average_ticket_score"
            ]
            existing_score.service_quality_score = calculated_score["components"][
            ...
            score_record = SixFBScore(
                ...
                booking_utilization_score=calculated_score["components"][
                    "booking_utilization_score"
                ],
                revenue_growth_score=calculated_score["components"][
                    "revenue_growth_score"
                ],
                customer_retention_score=calculated_score["components"][
                    "customer_retention_score"
                ],
                average_ticket_score=calculated_score["components"][
                    "average_ticket_score"
                ],
                service_quality_score=calculated_score["components"][
                    "service_quality_score"
                ],
    ...
    def get_dashboard_metrics(self, barber_id: int) -> Dict:
        ...
        if previous_weekly_metrics:
            if previous_weekly_metrics.total_revenue > 0:
                revenue_change = (
                    (
                        ...
                        - previous_weekly_metrics.total_revenue
                    )
                    / previous_weekly_metrics.total_revenue
                ) * 100
            if previous_weekly_metrics.completed_appointments > 0:
                appointments_change = (
                    (
                        weekly_metrics.completed_appointments
                        - previous_weekly_metrics.completed_appointments
                    )
                    / previous_weekly_metrics.completed_appointments
                ) * 100
