...
class GoogleCalendarService:
    ...
    def get_authorization_url(self, barber_id: int, state: Optional[str] = None) -> str:
        ...
        try:
            ...
            flow = Flow.from_client_config(
                oauth_config, scopes=self.SCOPES, state=state or str(barber_id)
            )
            ...
            logger.error(
                f"Error generating authorization URL for barber {barber_id}: {str(e)}"
            )
            raise
    ...
    def handle_oauth_callback(self, barber_id: int, authorization_code: str) -> bool:
        ...
        try:
            oauth_config = self._get_oauth_config()
            flow = Flow.from_client_config(
                oauth_config, scopes=self.SCOPES, state=str(barber_id)
            )
            ...
            logger.info(
                f"Successfully connected Google Calendar for barber {barber_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error handling OAuth callback for barber {barber_id}: {str(e)}"
            )
            return False
    ...
    def _load_credentials(self, barber_id: int) -> Optional[Credentials]:
        ...
        try:
            ...
            logger.warning(
                f"Could not load credentials for barber {barber_id}: {str(e)}"
            )
            return None
    ...
    def disconnect(self, barber_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error disconnecting Google Calendar for barber {barber_id}: {str(e)}"
            )
            return False
    ...
    def get_calendar_service(self, barber_id: int):
        ...
        if not credentials or not credentials.valid:
            raise ValueError(
                f"No valid Google Calendar credentials for barber {barber_id}"
            )

        return build("calendar", "v3", credentials=credentials)

    def create_calendar_event(
        self, barber_id: int, appointment: Appointment
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            # Create event
            client_name = (
                appointment.client.full_name if appointment.client else "Unknown Client"
            )
            event = {
            ...
            logger.info(
                f"Created Google Calendar event {event_id} for appointment {appointment.id}"
            )
            ...
            logger.error(
                f"Error creating Google Calendar event for appointment {appointment.id}: {str(e)}"
            )
            return None

    def update_calendar_event(
        self, barber_id: int, event_id: str, appointment: Appointment
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Updated Google Calendar event {event_id} for appointment {appointment.id}"
            )
            return True
    ...
    def sync_appointment(
        self, appointment: Appointment, action: str = "create"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if not self.is_connected(barber_id):
                logger.debug(
                    f"Google Calendar not connected for barber {barber_id}, skipping sync"
                )
                return None
            ...
            if action == "create":
                ...
                action == "update"
                and hasattr(appointment, "google_calendar_event_id")
                and appointment.google_calendar_event_id
            ):
                success = self.update_calendar_event(
                    barber_id, appointment.google_calendar_event_id, appointment
                )
                ...
                success = self.delete_calendar_event(
                    barber_id, appointment.google_calendar_event_id
                )
            ...
            logger.error(
                f"Error syncing appointment {appointment.id} with Google Calendar: {str(e)}"
            )
            return None
    ...
    def get_calendar_events(
        self, barber_id: int, start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting Google Calendar events for barber {barber_id}: {str(e)}"
            )
            return []
