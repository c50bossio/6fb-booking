...
class RBACService:
    ...
    # Permission Checking
    def has_permission(self, user: User, permission: str, resource_id: Optional[int] = None) -> bool:
    ...
    def _check_contextual_permission(self, user: User, permission: str, resource_id: Optional[int]) -> bool:
        """Check context-specific permissions (location access, ownership, etc.)"""
        
        # Location-based permissions
        if permission in [Permission.VIEW_ASSIGNED_LOCATIONS, Permission.MANAGE_ASSIGNED_LOCATIONS]:
            if user.role == "mentor":
                # Check if user is mentor for the location
                if resource_id:
                    location = self.db.query(Location).filter(Location.id == resource_id).first()
                    return location and location.mentor_id == user.id
            return True
        
        elif permission in [Permission.VIEW_LOCATION_ANALYTICS, Permission.MANAGE_LOCATION_APPOINTMENTS, Permission.VIEW_LOCATION_CLIENTS]:
            # Check if user has access to the location
            if resource_id:
                return self._user_has_location_access(user, resource_id)
            return True
        
        elif permission in [Permission.VIEW_OWN_ANALYTICS, Permission.MANAGE_OWN_APPOINTMENTS, Permission.VIEW_OWN_CLIENTS]:
    ...
    def _user_owns_resource(self, user: User, permission: str, resource_id: int) -> bool:
        """Check if user owns the specific resource"""
        
        if permission == Permission.VIEW_OWN_ANALYTICS or permission == Permission.MANAGE_OWN_APPOINTMENTS:
    ...
    async def create_user(self, user_data: Dict[str, Any], created_by: User) -> User:
        ...
        try:
            ...
            if not self._can_assign_role(created_by, target_role):
                raise HTTPException(status_code=403, detail=f"Cannot assign role: {target_role}")
    ...
    async def update_user_role(self, user_id: int, new_role: str, updated_by: User) -> User:
        """Update user role"""
        try:
            # Check permission
            ...
            # Log activity
            await self._log_user_activity(
                updated_by.id, "update_user_role",
                f"Changed user {user.email} role from {old_role} to {new_role}",
                "user", str(user.id)
            )
            
            logger.info(f"User {user.email} role changed from {old_role} to {new_role} by {updated_by.email}")
            return user
    ...
    async def grant_custom_permission(self, user_id: int, permission: str, granted_by: User) -> User:
        ...
        try:
            ...
            if permission not in custom_permissions:
                ...
                # Log activity
                await self._log_user_activity(
                    granted_by.id, "grant_permission",
    ...
    # Session Management
    async def create_user_session(self, user: User, device_info: str, ip_address: str, 
                                 location_id: Optional[int] = None) -> UserSession:
    ...
    # Activity Logging
    async def _log_user_activity(self, user_id: int, activity_type: str, description: str,
    ...
    async def get_user_activity_log(self, user_id: int, limit: int = 50) -> List[UserActivity]:
    ...
    def get_accessible_locations(self, user: User) -> List[int]:
        ...
        if user.role in ["super_admin", "admin"]:
            ...
            locations = self.db.query(Location).filter(Location.mentor_id == user.id).all()
            return [loc.id for loc in locations]
    ...
    def filter_locations_by_access(self, user: User, locations: List[Location]) -> List[Location]:
