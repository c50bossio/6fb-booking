...
class PerformanceAlertsService:
    ...
    async def _calculate_barber_metrics(self, barber: Barber) -> Dict[str, Any]:
        ...
        appointment_change = self._calculate_percent_change(
            current_week_appointments, previous_week_appointments
        )
        ...
        return {
            ...
            "revenue": {
                ...
                "target_achievement": self._calculate_target_achievement(
                    barber.id, current_week_revenue
                ),
            },
            "appointments": {
            ...
            "booking_rate": {"weekly_average": weekly_metrics.get("booking_rate", 0)},
            "average_ticket": {
                "weekly": weekly_metrics.get("average_ticket", 0),
                "percent_change": self._calculate_avg_ticket_change(barber.id),
            },
            "no_show_rate": {"weekly": no_show_rate},
            "retention": {"monthly_rate": retention_rate},
        }
        ...
        try:
            ...
            if is_breached:
                ...
                alert = await self._create_alert(
                    threshold, current_value, barber_id, metrics
                )
    ...
    def _is_in_cooldown(
        self, threshold: AlertThreshold, barber_id: Optional[int]
    ) -> bool:
        ...
        recent_alerts = [
            ...
            if (
                ...
                and alert.barber_id == barber_id
                and alert.triggered_at > cooldown_cutoff
            )
        ]
    ...
    def _generate_alert_message(
        self, threshold: AlertThreshold, current_value: float, metrics: Dict[str, Any]
    ) -> str:
        ...
        base_message = messages.get(
            threshold.alert_type, f"Metric {threshold.metric_path} breached threshold"
        )

        # Add recommendations
        recommendations = self._get_recommendations(
            threshold.alert_type, current_value, metrics
        )
        if recommendations:
            base_message += f"\n\nRecommendations:\n{recommendations}"
    ...
    def _get_recommendations(
        self, alert_type: AlertType, current_value: float, metrics: Dict[str, Any]
    ) -> str:
        ...
        return recommendations.get(
            alert_type, "• Review performance metrics\n• Consider process improvements"
        )
    ...
    def _calculate_retention_rate(self, barber_id: int) -> float:
        ...
        return (
            (recent_clients / previous_clients * 100) if previous_clients > 0 else 100.0
        )

    def _calculate_target_achievement(
        self, barber_id: int, current_revenue: float
    ) -> float:
    ...
    def _calculate_avg_ticket_change(self, barber_id: int) -> float:
        ...
        current_avg = self.calculator.get_weekly_metrics(barber_id).get(
            "average_ticket", 0
        )
    ...
    def get_active_alerts(
        self, barber_id: Optional[int] = None
