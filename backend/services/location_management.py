...
class LocationManagementService:
    ...
    async def create_location(self, location_data: Dict[str, Any]) -> Location:
        ...
        try:
            ...
            if "location_code" not in location_data:
                location_data["location_code"] = self._generate_location_code(
    ...
    def get_user_accessible_locations(self, user: User) -> List[Location]:
        ...
        if user.role in ["super_admin", "admin"]:
            ...
            if accessible_location_ids:
                return (
                    self.db.query(Location)
                    .filter(Location.id.in_(accessible_location_ids))
                    .all()
                )
    ...
    async def update_location(
        self, location_id: int, update_data: Dict[str, Any]
    ) -> Location:
    ...
    async def calculate_location_analytics(
        self,
        location_id: int,
        ...
        try:
            ...
            if not barber_ids:
                return self._empty_analytics_response(
                    location, period_type, start_date, end_date
                )
            ...
            await self._save_location_analytics(
                location_id, period_type, start_date, end_date, analytics
            )
    ...
    async def _calculate_comprehensive_metrics(
        ...
        # Client metrics
        unique_clients = len(
            set(a.client_id for a in completed_appointments if a.client_id)
        )
        new_clients = len(
        ...
        for barber in barbers:
            ...
            barber_score = calculator.calculate_sixfb_score(
                barber.id, "weekly", start_date, end_date
            )
        ...
        appointments_per_barber = (
            completed_count / active_barbers if active_barbers > 0 else 0
        )
        ...
        revenue_target_achievement = (
            (total_revenue / period_revenue_target * 100)
            if period_revenue_target > 0
            else 0
        )
        appointment_target_achievement = (
            (completed_count / period_appointment_target * 100)
            if period_appointment_target > 0
            else 0
        )
    ...
    def _empty_analytics_response(
        self, location: Location, period_type: str, start_date: date, end_date: date
    ) -> Dict[str, Any]:
    ...
    async def _save_location_analytics(
        self,
        location_id: int,
        period_type: str,
        start_date: date,
        ...
        try:
            ...
            existing = (
                self.db.query(LocationAnalytics)
                .filter(
                    and_(
            ...
            analytics_data = {
                ...
                "revenue_target_achievement": analytics["revenue"][
                    "target_achievement"
                ],
    ...
    async def get_network_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for location in locations:
                ...
                analytics = await self.calculate_location_analytics(
                    location.id, "weekly"
                )

                location_performance.append(
                    {"location": location, "analytics": analytics}
                )

                # Aggregate network stats
                network_stats["total_revenue"] += analytics["revenue"]["total"]
                network_stats["total_appointments"] += analytics["appointments"][
                    "completed"
                ]
            ...
            if location_performance:
                network_stats["average_6fb_score"] = sum(
                    lp["analytics"]["performance"]["average_6fb_score"]
                ...
                network_stats["top_performing_locations"] = [
                    {
                        ...
                        "sixfb_score": lp["analytics"]["performance"][
                            "average_6fb_score"
                        ],
                ...
                network_stats["locations_needing_attention"] = [
                    ...
                    for lp in location_performance
                    if lp["analytics"]["performance"]["average_6fb_score"]
                    < attention_threshold
                ]
    ...
    async def compare_locations(
        self, location_ids: List[int], period_type: str = "weekly"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison_data["comparison_metrics"] = {
                "revenue": {
                    ...
                    "average": sum(a["revenue"]["total"] for a in all_analytics)
                    / len(all_analytics),
                },
                "sixfb_score": {
                    "highest": max(
                        a["performance"]["average_6fb_score"] for a in all_analytics
                    ),
                    "lowest": min(
                        a["performance"]["average_6fb_score"] for a in all_analytics
                    ),
                ...
                "appointments": {
                    "highest": max(
                        a["appointments"]["completed"] for a in all_analytics
                    ),
                    "lowest": min(
                        a["appointments"]["completed"] for a in all_analytics
                    ),
                ...
                rankings["revenue"].append(
                    {
                        ...
                        "target_achievement": analytics["revenue"][
                            "target_achievement"
                        ],
                    }
                )
                ...
                rankings["sixfb_score"].append(
                    {
                        ...
                        "target_achievement": (
                            analytics["performance"]["average_6fb_score"] / 85.0
                        )
                        * 100,
                    }
                )
