...
class PaymentMonitoringService:
        ...
        with sentry_sdk.push_scope() as scope:
            ...
            if success:
                sentry_sdk.capture_message(
                    f"Webhook processed: {event_type}", level="info"
                )
            else:
                sentry_sdk.capture_message(
                    f"Webhook failed: {event_type} - {error_message}", level="error"
                )

    def track_payment_attempt(
        self,
        payment_id: str,
        amount: float,
        currency: str,
        ...
        # Record metric
        self.record_metric(f"payment_{status.value}", processing_time, payment_data)
        ...
        with sentry_sdk.push_scope() as scope:
            ...
            if status == PaymentStatus.SUCCESS:
                sentry_sdk.capture_message(
                    f"Payment successful: {payment_id}", level="info"
                )
            elif status == PaymentStatus.FAILED:
                sentry_sdk.capture_message(
                    f"Payment failed: {payment_id} - {error_code}", level="warning"
                )
    ...
    def _check_webhook_health(self):
        ...
        avg_processing_time = sum(e["processing_time"] for e in recent_events) / len(
            recent_events
        )
    ...
    def _check_payment_health(self):
        ...
        refund_rate = (
            ...
            / max(self.payment_counts["succeeded"], 1)
            * 100
        )
    ...
    def _create_alert(
        ...
        with sentry_sdk.push_scope() as scope:
            ...
            scope.set_context("alert", {"id": alert_id, "title": title, "data": data})
            ...
            if severity == AlertSeverity.CRITICAL:
                ...
                sentry_sdk.capture_message(
                    f"{severity.value.upper()}: {title} - {message}", level="warning"
                )
    ...
    def get_payment_health_summary(self) -> Dict[str, Any]:
        ...
        refund_rate = (
            self.payment_counts["refunded"] / max(self.payment_counts["succeeded"], 1)
        ) * 100
        ...
        if failure_rate > self.thresholds[
            "payment_failure_rate"
        ] or webhook_success_rate < (100 - self.thresholds["webhook_failure_rate"]):
            status = "unhealthy"
        ...
        return {
            ...
            "webhook_stats": {
                ...
                "recent_failures": len(
                    [e for e in recent_webhooks if not e["success"]]
                ),
            },
            "active_alerts": len(
    ...
    def get_payment_metrics(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        relevant_metrics = [
            m for m in self.payment_metrics if m.timestamp >= cutoff_time
        ]
