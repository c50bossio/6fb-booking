...
class StripeService:
    ...
    async def create_or_get_customer(self, user: User) -> str:
        ...
        try:
            ...
            customer = stripe.Customer.create(
                ...
                name=user.full_name,
                phone=user.phone,
                metadata={"user_id": str(user.id), "platform": "6FB"},
            )
    ...
    async def add_payment_method(
        self, user: User, payment_method_id: str, set_as_default: bool = False
    ) -> PaymentMethod:
        ...
        try:
            ...
            if set_as_default:
                ...
                self.db.query(PaymentMethod).filter(
                    PaymentMethod.user_id == user.id, PaymentMethod.is_default == True
    ...
    async def remove_payment_method(self, user: User, payment_method_id: int) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Removed payment method {payment_method_id} for user {user.id}"
            )
            return True
    ...
    async def create_payment_intent(
        ...
        try:
            ...
            logger.info(
                f"Created payment intent {intent.id} for appointment {appointment.id}"
            )
    ...
    async def handle_webhook_event(self, event: stripe.Event) -> bool:
        ...
        try:
            ...
            existing_event = (
                ...
                .filter(PaymentWebhookEvent.stripe_event_id == event.id)
                .first()
            )
    ...
    async def _handle_charge_refunded(self, charge: Dict[str, Any]):
        ...
        payment = (
            self.db.query(Payment)
            .filter(Payment.stripe_charge_id == charge["id"])
            .first()
        )
        ...
        if payment:
            ...
            for stripe_refund in charge.get("refunds", {}).get("data", []):
                existing_refund = (
                    ...
                    .filter(Refund.stripe_refund_id == stripe_refund["id"])
                    .first()
                )
    ...
    async def _handle_payment_method_detached(self, payment_method: Dict[str, Any]):
        ...
        pm = (
            self.db.query(PaymentMethod)
            .filter(PaymentMethod.stripe_payment_method_id == payment_method["id"])
            .first()
        )
