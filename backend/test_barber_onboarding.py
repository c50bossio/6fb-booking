...
class BarberOnboardingTest:
    ...
    def list_barbers(self):
        ...
        if response.status_code == 200:
            ...
            for barber in barbers:
                stripe_status = "✅ Connected" if barber.get("stripe_account_id") else "❌ Not Connected"
    ...
    def create_test_barber(self):
        ...
        if response.status_code in [200, 201]:
            ...
            self.print_success(f"Created test barber: {barber['full_name']} (ID: {barber['id']})")
            return barber["id"]
        else:
    ...
    def check_barber_status(self, barber_id):
        ...
        if response.status_code == 200:
            ...
            if status.get('connected'):
                ...
                print(f"Charges Enabled: {'✅' if status.get('charges_enabled') else '❌'}")
                print(f"Payouts Enabled: {'✅' if status.get('payouts_enabled') else '❌'}")
    ...
    def generate_connect_url(self, barber_id):
        ...
        if response.status_code == 200:
            ...
            if "already connected" in str(error.get("detail", "")).lower():
                ...
                self.print_error(f"Failed to generate URL: {error.get('detail', response.text)}")
            return None
    ...
    def simulate_callback(self, barber_id, state_token):
        ...
        self.print_warning("Note: This is a simulation. In real flow, Stripe redirects with auth code.")
        ...
        response = self.session.post(
            f"{API_URL}/stripe-connect/callback",
            json=callback_data
        )
    ...
    def test_payout_calculation(self, barber_id):
        ...
        scenarios = [
            {"service": 50.00, "tip": 10.00, "description": "Standard haircut with tip"},
    ...
    def run_complete_test(self):
        ...
        if not status or not status.get('connected'):
            ...
            if connect_data:
                ...
                print(f"\n{Fore.YELLOW}Would you like to simulate the OAuth callback? (y/n):{Style.RESET_ALL} ", end="")
