...
class TestResult:
    ...
    def add_step(self, step: str, success: bool, details: str = ""):
        self.steps.append({"step": step, "success": success, "details": details})
        if not success:
            self.passed = False
...
class EndToEndTester:
    ...
    def print_step(self, step: str, status: str = "TESTING"):
        ...
        color = (
            Colors.YELLOW
            if status == "TESTING"
            else Colors.GREEN if status == "PASSED" else Colors.RED
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result.add_step(
                    "Client registration",
                    True,
                    f"User ID: {self.client_data['user_id']}",
                )
                self.print_step("Client registration", "PASSED")
                self.print_detail(
                    f"Successfully created client with ID: {self.client_data['user_id']}"
                )
            else:
                result.add_step(
                    "Client registration",
            ...
            if self.client_token:
                ...
                if login_response.status_code == 200:
                    ...
                    result.add_step(
                        "Client login", False, f"Status: {login_response.status_code}"
                    )
    ...
    def test_2_browse_services(self) -> TestResult:
        ...
        try:
            ...
            categories_response = requests.get(
                f"{API_V1}/booking/categories", headers=headers
            )

            if categories_response.status_code == 200:
                categories = categories_response.json()
                result.add_step(
                    "Fetch categories", True, f"Found {len(categories)} categories"
                )
            ...
            services_response = requests.get(
                f"{API_V1}/booking/services", headers=headers
            )

            if services_response.status_code == 200:
                self.services = services_response.json()
                result.add_step(
                    "Fetch services", True, f"Found {len(self.services)} services"
                )
                ...
                if self.services:
                    ...
                    self.print_detail(
                        f"Selected service: {self.selected_service.get('name')} - ${self.selected_service.get('price')}"
                    )

                    # Show first few services
                    for service in self.services[:3]:
                        self.print_detail(
                            f"  - {service.get('name')} (${service.get('price')}, {service.get('duration_minutes')} min)"
                        )
            else:
                result.add_step(
                    "Fetch services", False, f"Status: {services_response.status_code}"
                )
    ...
    def test_3_check_availability(self) -> TestResult:
        ...
        try:
            ...
            if barbers_response.status_code == 200:
                ...
                result.add_step(
                    "Fetch barbers", True, f"Found {len(self.barbers)} barbers"
                )
                ...
                if self.barbers:
                    ...
                    self.print_detail(
                        f"Selected barber: {self.selected_barber.get('name', 'Unknown')}"
                    )

                    # Show barber details
                    for barber in self.barbers[:3]:
                        self.print_detail(
                            f"  - {barber.get('name')} at {barber.get('location', {}).get('name', 'Unknown location')}"
                        )
            else:
                result.add_step(
                    "Fetch barbers", False, f"Status: {barbers_response.status_code}"
                )
            ...
            if self.selected_barber and self.selected_service:
                ...
                if availability_response.status_code == 200:
                    ...
                    result.add_step(
                        "Check availability", True, f"Found availability data"
                    )
                    ...
                    if availability.get("available_slots"):
                        self.print_detail(
                            f"Found {len(availability['available_slots'])} available time slots"
                        )
                    ...
                    result.add_step(
                        "Check availability",
                        False,
                        f"Status: {availability_response.status_code}",
                    )
    ...
    def test_4_create_booking(self) -> TestResult:
        ...
        try:
            ...
            if not self.selected_service or not self.selected_barber:
                result.add_step(
                    "Create booking", False, "No service or barber selected"
                )
                self.print_step("Create booking", "FAILED")
            ...
            booking_response = requests.post(
                f"{API_V1}/booking/appointments", json=booking_data, headers=headers
            )
            ...
            if booking_response.status_code == 200:
                ...
                result.add_step(
                    "Create booking", True, f"Booking ID: {self.appointment_id}"
                )
                self.print_step("Create booking", "PASSED")
                self.print_detail(
                    f"Successfully created booking with ID: {self.appointment_id}"
                )
                self.print_detail(
                    f"Status: {self.booking_data.get('status', 'unknown')}"
                )
            else:
                result.add_step(
                    "Create booking",
                    False,
                    f"Status: {booking_response.status_code}, Error: {booking_response.text}",
                )
                self.print_step("Create booking", "FAILED")
    ...
    def test_5_process_payment(self) -> TestResult:
        ...
        try:
            ...
            payment_data = {
                ...
                "amount": self.selected_service.get("price", 0)
                * 100,  # Convert to cents
                "currency": "usd",
            }

            payment_response = requests.post(
                f"{API_V1}/payments/create-intent", json=payment_data, headers=headers
            )
            ...
            if payment_response.status_code == 200:
                ...
                result.add_step(
                    ...
                    True,
                    f"Intent ID: {payment_intent.get('client_secret', 'N/A')[:20]}...",
                )
                ...
                self.print_detail(
                    "Note: In production, payment would be confirmed via Stripe.js"
                )
                result.add_step("Payment simulation", True, "Payment flow available")

            elif (
                payment_response.status_code == 500
                and "Stripe" in payment_response.text
            ):
                result.add_step("Payment configuration", False, "Stripe not configured")
                self.print_step("Payment configuration", "SKIPPED")
                self.print_detail(
                    "Stripe is not configured - payment processing would fail in production"
                )
                self.print_detail(
                    "To enable payments, configure STRIPE_SECRET_KEY in .env"
                )
            else:
                result.add_step(
                    "Create payment intent",
    ...
    def test_6_check_appointment(self) -> TestResult:
        ...
        try:
            if not self.appointment_id:
                result.add_step(
                    "Check appointment", False, "No appointment ID available"
                )
            ...
            appointment_response = requests.get(
                f"{API_V1}/appointments/{self.appointment_id}", headers=headers
            )

            if appointment_response.status_code == 200:
                appointment = appointment_response.json()
                result.add_step(
                    "Fetch appointment", True, f"Status: {appointment.get('status')}"
                )
                ...
                self.print_detail(
                    f"Service: {appointment.get('service', {}).get('name', 'Unknown')}"
                )
                self.print_detail(
                    f"Barber: {appointment.get('barber', {}).get('name', 'Unknown')}"
                )
                self.print_detail(f"Date/Time: {appointment.get('start_time')}")
                self.print_detail(
                    f"Total Cost: ${appointment.get('total_cost', 0):.2f}"
                )
            else:
                result.add_step(
                    "Fetch appointment",
                    False,
                    f"Status: {appointment_response.status_code}",
                )
            ...
            all_appointments_response = requests.get(
                f"{API_V1}/appointments", headers=headers
            )
            ...
            if all_appointments_response.status_code == 200:
                ...
                result.add_step(
                    "Fetch all appointments",
                    False,
                    f"Status: {all_appointments_response.status_code}",
                )
    ...
    def test_7_leave_review(self) -> TestResult:
        ...
        try:
            ...
            review_response = requests.post(
                f"{API_V1}/booking/reviews", json=review_data, headers=headers
            )
            ...
            if review_response.status_code == 200:
                ...
                self.print_detail(
                    f"Successfully submitted {review_data['rating']}-star review"
                )
                self.print_detail(f"Review ID: {review.get('id')}")
            else:
                result.add_step(
                    "Submit review", False, f"Status: {review_response.status_code}"
                )
            ...
            if self.selected_barber:
                ...
                if reviews_response.status_code == 200:
                    ...
                    result.add_step(
                        "Fetch reviews", True, f"Found {len(reviews)} reviews"
                    )
                    ...
                    if reviews:
                        avg_rating = sum(r.get("rating", 0) for r in reviews) / len(
                            reviews
                        )
                    ...
                    result.add_step(
                        "Fetch reviews",
                        False,
                        f"Status: {reviews_response.status_code}",
                    )
    ...
    def run_all_tests(self):
        ...
        print(
            f"{Colors.CYAN}Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.RESET}\n"
        )
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"{Colors.RED}ERROR: Server is not responding at {BASE_URL}{Colors.RESET}"
                )
                print(
                    f"{Colors.YELLOW}Please ensure the backend server is running:{Colors.RESET}"
                )
                print(
                    f"{Colors.CYAN}  cd /Users/bossio/6fb-booking/backend{Colors.RESET}"
                )
                print(f"{Colors.CYAN}  uvicorn main:app --reload{Colors.RESET}")
                return
        except requests.exceptions.ConnectionError:
            print(
                f"{Colors.RED}ERROR: Cannot connect to server at {BASE_URL}{Colors.RESET}"
            )
            print(
                f"{Colors.YELLOW}Please start the backend server first:{Colors.RESET}"
            )
    ...
    def print_summary(self):
        ...
        payment_test = next(
            (r for r in self.results if r.name == "Process Payment"), None
        )
