            ...
            try:
                ...
                for pattern_name, pattern_info in self.performance_patterns.items():
                    if 'ast_check' in pattern_info:
                        ast_issues = pattern_info['ast_check'](tree, file_info['filename'])
            ...
            def visit_For(self, node):
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Attribute):
                        if hasattr(child, 'attr') and child.attr in ['query', 'filter', 'get']:
...
class PreDeploymentValidator:
    ...
    async def validate_deployment(self, branch: str = 'main') -> Tuple[bool, Dict]:
        ...
        if critical_issues:
            ...
            results['issues'].extend([f"{i.file}:{i.line} - {i.message}" for i in critical_issues])
        
        if high_issues:
            results['warnings'].extend([f"{i.file}:{i.line} - {i.message}" for i in high_issues])
        
        # Security scan
        security_issues = await self.quality_gate.run_security_scan('.')
        if security_issues:
            results['warnings'].extend([f"Security: {i.message}" for i in security_issues])
        
        # Test coverage check
        coverage_ok, coverage_pct = await self._check_test_coverage()
        if not coverage_ok:
            results['can_deploy'] = False
            results['issues'].append(f"Test coverage too low: {coverage_pct}% (minimum 80%)")
