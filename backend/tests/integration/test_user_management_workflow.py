...
class TestUserManagementWorkflow:
    ...
    def test_user_lifecycle_workflow(self):
        ...
        login_response = client.post(
            "/api/v1/auth/token",
            data={"username": user_email, "password": "InitialP@ssw0rd!"},
        )
        ...
        new_login_response = client.post(
            "/api/v1/auth/token",
            data={"username": user_email, "password": "NewSecureP@ssw0rd!"},
        )
    ...
    def test_role_based_access_workflow(self):
        ...
        for role in ["barber", "receptionist", "mentor"]:
            ...
            login_response = client.post(
                "/api/v1/auth/token",
                data={"username": email, "password": "SecureP@ssw0rd!"},
            )
            assert login_response.status_code == 200
        ...
        for user in users:
            analytics_response = client.get(
                "/api/v1/analytics/dashboard", headers=user["headers"]
            )
        ...
        for user in users:
            users_response = client.get("/api/v1/users", headers=user["headers"])
