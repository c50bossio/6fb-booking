...
class TestAppointmentWorkflow:
    ...
    def test_complete_appointment_workflow(self):
        ...
        login_response = client.post(
            "/api/v1/auth/token",
            data={"username": barber_email, "password": "SecureP@ssw0rd!"},
        )
        ...
        get_appointment_response = client.get(
            f"/api/v1/appointments/{appointment_id}", headers=barber_headers
        )

        # If individual appointment endpoint doesn't exist, check list
        if get_appointment_response.status_code == 404:
            list_response = client.get("/api/v1/appointments", headers=barber_headers)
        ...
        analytics_response = client.get(
            f"/api/v1/analytics/barber/{barber_id}", headers=barber_headers
        )
