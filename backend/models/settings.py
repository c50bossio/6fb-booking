...
class SettingsConfig(BaseModel):
    ...
    )  # ID of the location/user/barber if applicable
    ...
    inherits_from_scope = Column(
        SQLEnum(SettingsScope)
    )  # Which scope this inherits from
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "scope_id IS NOT NULL OR scope = 'global'",
...
class UserPreferences(BaseModel):
    ...
    information_density = Column(
        String(20), default="comfortable"
    ...
    panel_layout = Column(
        JSON, default=lambda: {"left": True, "right": True, "bottom": False}
    )
...
class NotificationSettings(BaseModel):
    ...
    cancellation_notifications = Column(
        JSON,
        default=lambda: {
            "enabled": True,
            "methods": ["email", "sms"],
    ...
    __table_args__ = (
        UniqueConstraint(
            "scope", "scope_id", name="unique_notification_settings_per_scope"
        ),
    )
...
class IntegrationSettings(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "scope", "scope_id", name="unique_integration_settings_per_scope"
        ),
    )
...
DEFAULT_SETTINGS_TEMPLATES = [
    ...
    },
    {
        ...
        "template_data": {
            ...
            "payment_configuration": {"require_deposit": True, "deposit_amount": 50},
            "cancellation_policy": {
