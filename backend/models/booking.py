...
class Service(BaseModel):
    ...
    buffer_minutes = Column(Integer, default=0)  # Clean-up/prep time between appointments
    
    # Deposit Settings
    requires_deposit = Column(Boolean, default=False)
    deposit_type = Column(String(20))  # 'percentage' or 'fixed'
    deposit_amount = Column(Float, default=0.0)  # Either percentage (0-100) or fixed amount
...
class BarberAvailability(BaseModel):
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('barber_id', 'location_id', 'day_of_week', 'start_time', 
                        name='unique_barber_schedule'),
        CheckConstraint('end_time > start_time', name='check_valid_hours'),
        CheckConstraint('break_end > break_start OR break_start IS NULL', 
                       name='check_valid_break'),
    )
...
class BookingRule(BaseModel):
    ...
    # Rule Type
    rule_type = Column(String(50), nullable=False)  # 'cancellation', 'reschedule', 'booking_window', etc.
...
class BookingSlot(BaseModel):
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('barber_id', 'location_id', 'slot_date', 'start_time',
