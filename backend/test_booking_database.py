...
def test_service_categories(db):
    ...
    # Get all categories
    categories = (
        db.query(ServiceCategory).filter(ServiceCategory.is_active == True).all()
    )
    ...
    if categories:
        for cat in categories:
            ...
            service_count = (
                db.query(Service)
                .filter(and_(Service.category_id == cat.id, Service.is_active == True))
                .count()
            )
...
def test_services(db):
    ...
    services = (
        db.query(Service, ServiceCategory)
    ...
    if services:
        ...
        for cat_name, cat_services in by_category.items():
            ...
            for svc in cat_services[:3]:  # Show first 3
                print(
                    f"    - {svc.name} (${svc.base_price:.2f}, {svc.duration_minutes} min)"
                )
                if svc.requires_deposit:
                    print(
                        f"      Deposit: {svc.deposit_amount}{'%' if svc.deposit_type == 'percentage' else '$'}"
                    )
    else:
...
def test_barber_availability(db):
    ...
    availability = (
        db.query(BarberAvailability, Barber)
        .join(Barber, BarberAvailability.barber_id == Barber.id)
        .filter(BarberAvailability.is_available == True)
        .all()
    )
    ...
    if availability:
        ...
        for barber_name, patterns in by_barber.items():
            ...
            for pattern in patterns:
                print(
                    f"    - {pattern.day_of_week.name}: {pattern.start_time} - {pattern.end_time}"
                )
...
def test_appointments_and_reviews(db):
    ...
    recent = (
        db.query(Appointment).order_by(Appointment.created_at.desc()).limit(10).all()
    )
    ...
    if recent:
        for appt in recent[:5]:
            print(f"\n  Appointment {appt.id}:")
            print(
                f"    Date: {appt.appointment_date} at {appt.appointment_time.time()}"
            )
    ...
    if total_reviews > 0:
        ...
        avg_ratings = (
            ...
            .join(Review, Review.barber_id == Barber.id)
            .group_by(Barber.id)
            .all()
        )
...
def test_booking_flow_simulation(db):
    ...
    service = services[0]
    print(
        f"✅ Selected service: {service.name} (${service.base_price:.2f}, {service.duration_minutes} min)"
    )
    ...
    if availability:
        print(
            f"✅ Barber is available on {day_of_week.name}: {availability.start_time} - {availability.end_time}"
        )
    else:
