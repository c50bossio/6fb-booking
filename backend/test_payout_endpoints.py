...
from models.barber_payment import BarberPaymentModel, CommissionPayment, PaymentStatus, PaymentType
...
def setup_test_data():
    ...
    try:
        ...
        db.execute(text("DELETE FROM commission_payments WHERE barber_id IN (SELECT id FROM barbers WHERE name LIKE 'Test Barber%')"))
        db.execute(text("DELETE FROM barber_payment_models WHERE barber_id IN (SELECT id FROM barbers WHERE name LIKE 'Test Barber%')"))
        db.execute(text("DELETE FROM barbers WHERE name LIKE 'Test Barber%'"))
        db.execute(text("DELETE FROM users WHERE email IN (:admin_email, :barber_email)"), 
        ...
        for i, barber in enumerate(barbers):
            ...
            if i == 0:  # First barber - pending payouts
                ...
                statuses = [PaymentStatus.PENDING, PaymentStatus.OVERDUE, PaymentStatus.CANCELLED]
...
def test_get_payouts_list(token: str):
    ...
    if response.status_code == 200:
        ...
        if data["payouts"]:
            ...
            required_fields = ["id", "barber_id", "barber_name", "amount", "fee", 
                             "net_amount", "status", "payment_method", "payout_date", 
                             "created_at"]
    ...
    response = requests.get(f"{BASE_URL}/api/v1/payouts?status=pending", headers=headers)
    if response.status_code == 200:
        data = response.json()
    ...
    if test_data and test_data["barbers"]:
        ...
        response = requests.get(f"{BASE_URL}/api/v1/payouts?barber_id={barber_id}", headers=headers)
        if response.status_code == 200:
            data = response.json()
            print_success(f"Filter by barber_id={barber_id} returned {len(data['payouts'])} payouts")
        else:
...
def test_process_payout(token: str, payout_id: str):
    ...
    response = requests.post(f"{BASE_URL}/api/v1/payouts/{payout_id}/process", headers=headers)
    ...
    if response.status_code == 200:
        ...
        print_info(f"Expected error (no real Stripe account): {response.json()['detail']}")
...
def test_cancel_payout(token: str, payout_id: str):
    ...
    response = requests.post(f"{BASE_URL}/api/v1/payouts/{payout_id}/cancel", headers=headers)
...
def test_unauthorized_access():
    ...
    if response.status_code == 401:
        ...
        print_error(f"Unexpected response for unauthorized access: {response.status_code}")
...
def test_invalid_payout_operations(token: str):
    ...
    response = requests.post(f"{BASE_URL}/api/v1/payouts/99999/process", headers=headers)
    ...
    response = requests.get(f"{BASE_URL}/api/v1/payouts?status=completed", headers=headers)
    if response.status_code == 200 and response.json()["payouts"]:
        completed_payout_id = response.json()["payouts"][0]["id"]
        response = requests.post(f"{BASE_URL}/api/v1/payouts/{completed_payout_id}/cancel", headers=headers)
...
def run_all_tests():
    ...
    try:
        ...
        if response.status_code == 200 and response.json()["payouts"]:
            ...
            test_results.append(("Process Payout", test_process_payout(admin_token, pending_payout_id)))
            ...
            if len(response.json()["payouts"]) > 1:
                ...
                test_results.append(("Cancel Payout", test_cancel_payout(admin_token, cancel_payout_id)))
        ...
        test_results.append(("Invalid Operations", test_invalid_payout_operations(admin_token)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if response.status_code != 200:
            print_error("Server is not running! Start it with: uvicorn main:app --reload")
            sys.exit(1)
    except requests.exceptions.ConnectionError:
        print_error("Cannot connect to server! Start it with: uvicorn main:app --reload")
        sys.exit(1)
