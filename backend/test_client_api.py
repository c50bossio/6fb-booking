...
def login():
    ...
    response = requests.post(
        f"{BASE_URL}/auth/login", json={"username": "admin", "password": "Test123!"}
    )
...
def test_get_clients():
    ...
    response = requests.get(
        f"{BASE_URL}/clients", headers=headers, params={"search": "john"}
    )
    print_response(response, "Search Clients (john)")

    # Test with filter
    response = requests.get(
        f"{BASE_URL}/clients", headers=headers, params={"customer_type": "vip"}
    )
...
def test_update_client(client_id):
    ...
    response = requests.put(
        f"{BASE_URL}/clients/{client_id}", headers=headers, json=update_data
    )
...
def test_vip_status(client_id):
    ...
    response = requests.post(
        f"{BASE_URL}/clients/{client_id}/vip-status", headers=headers, json=vip_data
    )
...
def test_send_message(client_id):
    ...
    response = requests.post(
        f"{BASE_URL}/clients/{client_id}/message", headers=headers, json=message_data
    )
...
def test_export_clients():
    ...
    # Test CSV export
    response = requests.post(
        f"{BASE_URL}/clients/export", headers=headers, params={"format": "csv"}
    )
