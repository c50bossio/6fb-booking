...
def upgrade():
    ...
    op.create_index(
        "idx_appointments_status_date", "appointments", ["status", "appointment_date"]
    )
    op.create_index(
    ...
    op.create_index(
        "idx_appointments_barber_status", "appointments", ["barber_id", "status"]
    )
    ...
    try:
        op.create_index(
            "idx_booking_slots_barber_date", "booking_slots", ["barber_id", "slot_date"]
        )
        op.create_index(
            "idx_booking_slots_available", "booking_slots", ["is_available"]
        )
        op.create_index("idx_booking_slots_blocked", "booking_slots", ["is_blocked"])
        op.create_index(
            "idx_booking_slots_time_range", "booking_slots", ["start_time", "end_time"]
        )
    except Exception:
        # Table might not exist yet
        pass
