...
def upgrade():
    ...
    payment_status_enum = postgresql.ENUM('pending', 'processing', 'succeeded', 'failed', 'canceled', 'refunded', 'partially_refunded', name='payment_status')
    ...
    refund_status_enum = postgresql.ENUM('pending', 'succeeded', 'failed', 'canceled', name='refund_status')
    ...
    op.create_table('refunds',
        ...
        sa.ForeignKeyConstraint(['refunded_by_user_id'], ['users.id'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index(op.f('ix_payment_webhook_events_type'), 'payment_webhook_events', ['type'], unique=False)
    ...
    op.create_index(op.f('ix_payment_reports_date_from'), 'payment_reports', ['date_from'], unique=False)
    op.create_index(op.f('ix_payment_reports_date_to'), 'payment_reports', ['date_to'], unique=False)

    # Add payment-related columns to appointments table
    op.add_column('appointments', sa.Column('payment_required', sa.Boolean(), default=True, nullable=False))
    op.add_column('appointments', sa.Column('payment_status', sa.String(50), nullable=True))
    op.add_column('appointments', sa.Column('payment_id', sa.Integer(), nullable=True))
    op.create_foreign_key('fk_appointments_payment_id', 'appointments', 'payments', ['payment_id'], ['id'], ondelete='SET NULL')
    
    # Add payment-related columns to services table
    op.add_column('services', sa.Column('requires_deposit', sa.Boolean(), default=False, nullable=False))
    op.add_column('services', sa.Column('deposit_amount', sa.Integer(), nullable=True))  # Amount in cents
...
def downgrade():
    ...
    op.drop_index(op.f('ix_payment_webhook_events_type'), table_name='payment_webhook_events')
