...
def downgrade() -> None:
    ...
    with op.batch_alter_table("appointments", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("trafft_location_name", sa.String(length=200), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_employee_id", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_service_id", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_booking_uuid", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_appointment_id", sa.String(length=100), nullable=True)
        )

    # Add Trafft columns back to clients table
    with op.batch_alter_table("clients", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("trafft_customer_id", sa.String(length=100), nullable=True)
        )

    # Add Trafft columns back to locations table
    with op.batch_alter_table("locations", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("trafft_api_key", sa.String(length=500), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_location_id", sa.String(length=100), nullable=True)
        )

    # Add Trafft columns back to barbers table
    with op.batch_alter_table("barbers", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("trafft_last_sync", sa.String(length=50), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_employee_email", sa.String(length=255), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_employee_id", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_subdomain", sa.String(length=100), nullable=True)
        )
        batch_op.add_column(
            sa.Column("trafft_api_key", sa.String(length=500), nullable=True)
        )
