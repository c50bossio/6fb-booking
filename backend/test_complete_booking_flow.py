...
def test_complete_flow():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%service%' OR name LIKE '%booking%'")
        tables = cursor.fetchall()
        print(f"✅ Found {len(tables)} booking-related tables: {[t[0] for t in tables]}")
        ...
        print(f"✅ Database has {categories_count} categories and {services_count} services")
        conn.close()
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/api/v1/booking-public/barbers/1/availability?date={today}&service_id=1&duration=30")
        ...
        try:
            response = requests.post(f"{BASE_URL}/api/v1/booking-public/bookings/create", json=booking_data)
        ...
        try:
            frontend_response = requests.get("http://localhost:3000/booking-demo", timeout=5)
