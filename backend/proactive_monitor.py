...
class ProactiveMonitor:
        ...
        for check in checks:
            metric = HealthMetric(
                ...
                threshold_warning=self.config.get(
                    f"{check.name.lower().replace(' ', '_')}_threshold_warning",
                    check.threshold,
                ),
                threshold_critical=self.config.get(
                    f"{check.name.lower().replace(' ', '_')}_threshold_critical",
                    check.threshold * 1.2,
                ),
            )
            self.db.add(metric)
    ...
    async def predict_issues(self) -> List[PredictedIssue]:
            ...
            if cpu_trend > 0.5:  # Rising trend
                ...
                if predicted_value > self.config["cpu_threshold_critical"]:
                    prediction = PredictedIssue(
                        ...
                        + timedelta(minutes=self.prediction_window),
        ...
        if len(self.metrics_history["disk"]) > 10:
            ...
            if disk_trend > 0:
                days_until_full = (100 - self.metrics_history["disk"][-1]) / (
                    disk_trend * 48
            ...
            for endpoint in endpoints:
                try:
                    ...
                    async with session.get(endpoint, timeout=5) as response:
                        ...
                        # Get baseline
                        baseline = (
                            self.db.query(PerformanceBaseline)
                        ...
                        if response.status >= 500:
                            ...
                            baseline and response_time > baseline.p95_response_time * 2
                        ):
                            status = "degraded"
                        ...
                        result = {
                            ...
                            "baseline_p95": (
                                baseline.p95_response_time if baseline else None
                            ),
                        }
    ...
    async def run_pre_deployment_checks(
        ...
        critical_predictions = [
            p for p in predictions if p.impact == "critical" and p.probability > 0.7
        ]
        ...
        if critical_predictions:
            ...
            issues.extend(
                [
                    f"Predicted: {p.issue_type} (probability: {p.probability:.0%})"
    ...
    async def continuous_monitoring_loop(self):
        ...
        while True:
            try:
                ...
                critical = [
                    c for c in health_checks if c.severity == AlertSeverity.CRITICAL
                ]
                if critical:
                ...
                if high_prob_predictions:
                    await self.notification_service.send_alert(
                        ...
                        "\n".join(
                            [
                                f"- {p.issue_type}: {p.prevention_action}"
                                for p in high_prob_predictions
                            ]
                        ),
...
class NotificationService:
    ...
    async def send_alert(self, title: str, message: str, severity: AlertSeverity):
        ...
        if (
            ...
            and self.email_config["alert_emails"]
        ):
            try:
...
async def check_deployment(deployment_config: Dict):
    ...
    return {
        ...
        "issues": issues,
        "recommendation": (
            "Safe to deploy" if can_deploy else "Deployment not recommended"
        ),
    }
