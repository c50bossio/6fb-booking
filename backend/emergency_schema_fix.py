...
def emergency_fix():
    ...
    try:
        with engine.connect() as conn:
            ...
            missing_columns = [
                (
                    "ALTER TABLE barbers ADD COLUMN IF NOT EXISTS trafft_employee_id VARCHAR(255)",
                    "trafft_employee_id",
                ),
                (
                    "ALTER TABLE barbers ADD COLUMN IF NOT EXISTS trafft_employee_email VARCHAR(255)",
                    "trafft_employee_email",
                ),
                (
                    "ALTER TABLE barbers ADD COLUMN IF NOT EXISTS hourly_rate FLOAT",
                    "hourly_rate",
                ),
                (
                    "ALTER TABLE barbers ADD COLUMN IF NOT EXISTS average_service_duration INTEGER",
