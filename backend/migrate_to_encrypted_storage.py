...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data_migration.log"), logging.StreamHandler()],
)
...
class DataMigrationManager:
    ...
    def create_backup(self):
        ...
        try:
            backup_filename = (
                f"6fb_booking_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql"
            )
    ...
    def check_environment(self):
        ...
        if not os.getenv("DATA_ENCRYPTION_KEY") and not os.getenv("MASTER_PASSWORD"):
            logger.error(
                "No encryption key found. Set DATA_ENCRYPTION_KEY or MASTER_PASSWORD"
            )
            return False
        ...
        try:
            ...
            if sample_client and sample_client.email:
                ...
                if len(sample_client.email) > 100 and "=" in sample_client.email:
                    logger.warning(
                        "Data appears already encrypted. Migration may have been run before."
                    )
    ...
    def _is_encrypted(self, data: str) -> bool:
        ...
        return (
            ...
            and not "@" in data  # emails contain @
            and not "-" in data[:10]
        )  # phone numbers often start with area codes
            ...
            for client in sample_clients:
                if client.email:
                    ...
                    try:
                        ...
                        if "@" in decrypted:
                            logger.info(
                                f"✓ Client {client.id} email encryption verified"
                            )
                        else:
                            logger.warning(
                                f"⚠ Client {client.id} email decryption suspicious"
                            )
                    except Exception:
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate 6FB customer data to encrypted storage"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
