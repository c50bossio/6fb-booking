    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"❌ Calendar events failed: {response.status_code} - {response.text}"
                )
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"❌ Availability failed: {response.status_code} - {response.text}"
                )
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"❌ Google Calendar OAuth failed: {response.status_code} - {response.text}"
                )
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"❌ Calendar stats failed: {response.status_code} - {response.text}"
                )
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"✅ Appointments endpoint working: {len(appointments)} appointments"
                )
            else:
                print(
                    f"❌ Appointments failed: {response.status_code} - {response.text}"
                )
        ...
        try:
            ...
            response = await client.get(
                f"{BASE_URL}/api/v1/booking-public/shops/1/barbers"
            )

            if response.status_code == 200:
                barbers = response.json()
                print(
                    f"✅ Public booking endpoint working: {len(barbers) if isinstance(barbers, list) else 'data'} barbers"
                )
            elif response.status_code == 404:
                print("ℹ️  No barbers found for shop 1 (expected if no data)")
            else:
                print(
                    f"❌ Public booking failed: {response.status_code} - {response.text}"
                )
