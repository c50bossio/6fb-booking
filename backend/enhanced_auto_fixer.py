...
class NotificationService:
    ...
    async def notify_fix_applied(self, error_type: str, project: str, success: bool, details: str):
...
class RateLimiter:
    ...
    def allow_fix(self) -> bool:
        ...
        self.hour_window = deque(
            [t for t in self.hour_window if t > hour_ago],
...
class HealthChecker:
    ...
    async def get_current_error_rate(self, project: str) -> float:
        ...
        try:
            ...
            url = f"https://sentry.io/api/0/projects/bossio-solution-inc/{project}/stats/"
            params = {
                "stat": "received",
                "resolution": "1m",
    ...
    async def check_health_after_fix(self, project: str, fix_id: str) -> bool:
        ...
        if self.baseline_error_rate and current_rate > (self.baseline_error_rate * self.error_spike_threshold):
            logger.error(f"Error rate spike detected after fix {fix_id}: {current_rate} > {self.baseline_error_rate * self.error_spike_threshold}")
            return False
...
class DatabaseMigrationHelper:
    ...
    def generate_migration_for_missing_column(self, table: str, column: str, error_context: Dict) -> Optional[str]:
...
class EnhancedAutoFixer:
    ...
    def __init__(self):
        ...
        self.sentry_token = os.getenv("SENTRY_INTEGRATION_TOKEN", "490de1a139e58fc7d8f5eae81c60ca2ea5d6a50accc247ecce4976c80d9c356a")
        self.sentry_client_secret = os.getenv("SENTRY_CLIENT_SECRET", "2f231593da004229854fa5fb98104402e3cc4ebf8d4f1cad95dde4c36bf4c818")
    ...
    def verify_signature(self, payload: bytes, signature: str) -> bool:
        ...
        try:
            expected = hmac.new(
                self.sentry_client_secret.encode(),
                payload,
                hashlib.sha256
            ).hexdigest()
    ...
    async def learn_from_fix(self, error_pattern: str, error_type: str, success: bool, fix_time: float):
        ...
        pattern.avg_fix_time = (
            (pattern.avg_fix_time * (total_fixes - 1) + fix_time) / total_fixes
        )
        
        # Update confidence score (Bayesian approach)
        success_rate = pattern.success_count / total_fixes if total_fixes > 0 else 0.5
        confidence = (pattern.success_count + 1) / (total_fixes + 2)  # Laplace smoothing
    ...
    async def analyze_error_with_claude(self, error_data: Dict, pattern_history: Optional[ErrorPattern] = None) -> Dict:
...
"""
    ...
    async def apply_fix_with_rollback(self, fix_data: Dict, error_data: Dict) -> Tuple[bool, str]:
        ...
        project = error_data.get('project', 'unknown')
        self.health_checker.baseline_error_rate = await self.health_checker.get_current_error_rate(project)
        ...
        try:
            ...
            # Check system health
            health_ok = await self.health_checker.check_health_after_fix(project, fix_id)
    ...
    async def process_sentry_error(self, webhook_data: Dict) -> Dict:
        ...
        try:
            ...
            # Get pattern history
            pattern = self.db.query(ErrorPattern).filter_by(
            ...
            if confidence > 75 and risk_level == "LOW":
                # Apply fix
                success, message = await self.apply_fix_with_rollback(fix_analysis, error_data)
...
async def get_status():
    ...
    # Get recent fixes
    recent_fixes = db.query(FixHistory).order_by(
        FixHistory.timestamp.desc()
    ).limit(10).all()
    ...
    return {
        ...
        "stats": {
            ...
            "success_rate": db.query(FixHistory).filter_by(success=True).count() / max(db.query(FixHistory).count(), 1) * 100,
...
async def get_patterns():
    ...
    db = SessionLocal()
    patterns = db.query(ErrorPattern).order_by(
        ErrorPattern.confidence_score.desc()
    ).all()
        ...
        "patterns": [
            {
                ...
                "success_rate": p.success_count / (p.success_count + p.failure_count) * 100 if (p.success_count + p.failure_count) > 0 else 0,
