...
class PaymentSetupVerifier:
    ...
    def check_environment_variables(self):
        ...
        for var_name, description in required_vars.items():
            ...
            is_set = bool(
                ...
                and not value.startswith("sk_test_placeholder")
            )
            ...
            self.results["environment_variables"][var_name] = {
                ...
                "masked_value": (
                    f"{value[:8]}..." if is_set and len(value) > 8 else "Not set"
                ),
            }
    ...
    def check_stripe_connection(self):
        ...
        try:
            ...
            self.results["api_connectivity"]["stripe"] = {
                ...
                "currency": (
                    balance.available[0].currency if balance.available else "usd"
                ),
            }

            print("  ✅ Stripe API connection successful")
            print(
                f"  💰 Available balance: ${balance.available[0].amount / 100:.2f}"
        ...
        try:
            ...
            if connection_test:
                ...
                self.results["api_connectivity"]["square"] = {
                    "connected": True,
                    "merchant_id": (
                        merchant_info.get("merchant", {}).get("id", "Unknown")
                        if merchant_info
                        else "Unknown"
                    ),
                }
    ...
    def check_webhook_configuration(self):
        ...
        if webhook_secret and webhook_secret.startswith("whsec_"):
            try:
                ...
                print(
                    f"  ⚠️  Stripe webhook secret configured but validation failed: {str(e)}"
                )
        else:
            self.results["webhook_configuration"]["stripe"] = {"configured": False}
    ...
    def check_frontend_configuration(self):
        ...
        if os.path.exists(frontend_env_path):
            ...
            print(
                f"  {'✅' if has_stripe_key else '❌'} Stripe publishable key configured"
            )
            print(f"  {'✅' if has_api_url else '❌'} API URL configured")

        else:
            self.results["frontend_configuration"] = {"env_file_exists": False}
            print("  ❌ Frontend .env.local file not found")
            print(
                "  ⚠️  Create frontend/.env.local with NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"
            )
    ...
    def generate_report(self):
        ...
        # Count successes
        env_vars_ok = sum(
            1 for v in self.results["environment_variables"].values() if v["set"]
        )
        total_env_vars = len(self.results["environment_variables"])

        api_connections = [
            v for k, v in self.results["api_connectivity"].items() if "connected" in v
        ]
        api_ok = sum(1 for v in api_connections if v.get("connected", False))

        webhooks_ok = sum(
            1
            for v in self.results["webhook_configuration"].values()
            if v.get("configured", False)
        )
        ...
        if (
            not self.results["webhook_configuration"]
            .get("stripe", {})
