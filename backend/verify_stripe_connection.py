...
def main():
    try:
        ...
        print(f"   Service Commission Rate: {payment_model.service_commission_rate * 100}%")
        print(f"   Product Commission Rate: {payment_model.product_commission_rate * 100}%")
        ...
        try:
            # Get account details
            account_info = stripe_service.get_account_details(payment_model.stripe_connect_account_id)
            ...
            print(f"   Details Submitted: {'✅ Yes' if account_info.get('details_submitted') else '❌ No'}")
            print(f"   Payouts Enabled: {'✅ Yes' if account_info.get('payouts_enabled') else '❌ No'}")
            print(f"   Charges Enabled: {'✅ Yes' if account_info.get('charges_enabled') else '❌ No'}")
            ...
            # Get balance
            try:
                balance = stripe_service.get_balance(payment_model.stripe_connect_account_id)
                print(f"\n💰 Account Balance:")
            ...
            # Get dashboard link
            try:
                dashboard_url = stripe_service.create_login_link(payment_model.stripe_connect_account_id)
            ...
            print("   This could mean the account was disconnected or there's an API issue")
