...
async def get_dashboard_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating dashboard metrics: {str(e)}"
        )
...
async def get_sixfb_score(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating 6FB score: {str(e)}"
        )
...
async def get_daily_metrics(
    ...
    target_date: Optional[date] = Query(
        None, description="Date for metrics, defaults to today"
    ),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating daily metrics: {str(e)}"
        )
...
async def get_weekly_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating weekly metrics: {str(e)}"
        )
...
async def get_revenue_breakdown(
    ...
    try:
        ...
        breakdown = {
            ...
            "percentages": {
                "service_percentage": round(
                    (
                        (
                            weekly_data["total_service_revenue"]
                            / weekly_data["total_revenue"]
                            * 100
                        )
                        if weekly_data["total_revenue"] > 0
                        else 0
                    ),
                    1,
                ),
                "tips_percentage": round(
                    (
                        (weekly_data["total_tips"] / weekly_data["total_revenue"] * 100)
                        if weekly_data["total_revenue"] > 0
                        else 0
                    ),
                    1,
                ...
                "product_percentage": round(
                    (
                        (
                            weekly_data["total_product_revenue"]
                            / weekly_data["total_revenue"]
                            * 100
                        )
                        if weekly_data["total_revenue"] > 0
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating revenue breakdown: {str(e)}"
        )
...
async def get_customer_analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating customer analytics: {str(e)}"
        )
...
async def refresh_metrics(
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Refreshed metrics for the past {days_back} days",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error refreshing metrics: {str(e)}"
        )
