    ...
    try:
        ...
        for rule in rules:
            workflow_data.append({
                ...
                "last_triggered": rule.last_triggered.isoformat() if rule.last_triggered else "Never",
...
@router.get("/alerts")
async def get_performance_alerts(barber_id: Optional[int] = None, db: Session = Depends(get_db)):
...
async def generate_on_demand_report(
    ...
    try:
        ...
        # Generate report
        report_data = await service.generate_on_demand_report(report_type_enum, barber_id)
...
async def get_scheduling_recommendations(
    ...
    try:
        ...
        recommendations = await service.generate_recommendations(barber_id, target_date_obj)
...
async def get_schedule_analysis(
    ...
    try:
        service = SmartSchedulingService(db)
        analysis = await service.get_schedule_optimization_analysis(barber_id, analysis_days)
