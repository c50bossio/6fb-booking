...
async def get_clients(
    ...
    customer_type: Optional[str] = Query(
        None, description="new, returning, vip, or at_risk"
    ),
    ...
    try:
        ...
        if not barber:
            raise HTTPException(
                status_code=403, detail="User is not authorized as a barber"
            )
        ...
        clients = (
            query.order_by(Client.last_visit_date.desc().nullslast())
            .offset(skip)
            .limit(limit)
            .all()
        )
...
async def create_client(
    ...
    barber_id: int = Query(
        1, description="Barber ID - will be from auth in production"
    ),
    db: Session = Depends(get_db),
):
    """Create a new client"""
    try:
        ...
        if client_data.email:
            existing_client = (
                ...
                .filter(
                    and_(
                        Client.barber_id == barber_id, Client.email == client_data.email
                    )
                )
                .first()
            )
        if not existing_client and client_data.phone:
            existing_client = (
                ...
                .filter(
                    and_(
                        Client.barber_id == barber_id, Client.phone == client_data.phone
                    )
                )
                .first()
            )
        ...
        return {
            "success": True,
            "data": {"id": new_client.id, "message": "Client created successfully"},
        }
...
async def get_client_details(
    ...
    try:
        ...
        client = (
            db.query(Client)
            .filter(and_(Client.id == client_id, Client.barber_id == barber_id))
            .first()
        )
        ...
        for apt in completed_appointments:
            if apt.appointment_date:
                month_key = (
                    f"{apt.appointment_date.year}-{apt.appointment_date.month:02d}"
                )
                monthly_spending[month_key] = (
                    monthly_spending.get(month_key, 0) + apt.total_revenue
                )
        ...
        client_details = {
            ...
            "visit_stats": {
                ...
                "average_ticket": (
                    sum(apt.total_revenue for apt in completed_appointments)
            ...
            "preferences": {
                "favorite_services": dict(
                    sorted(service_counts.items(), key=lambda x: x[1], reverse=True)
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching client details: {str(e)}"
        )
...
async def update_client(
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"id": client.id, "message": "Client updated successfully"},
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def get_client_visits(
    ...
    try:
        ...
        total_appointments = (
            db.query(Appointment).filter(Appointment.client_id == client_id).count()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching client visits: {str(e)}"
        )
