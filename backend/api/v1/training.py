...
async def get_certifications(
    ...
    certifications = (
        db.query(Certification).filter(Certification.is_active == True).all()
    )
...
async def get_mentor_training_overview(
    ...
    mentor_locations = (
        db.query(Location).filter(Location.mentor_id == current_user.id).all()
    )
            ...
            if barber.user_id:
                ...
                if user:
                    ...
                    enrollments = (
                        ...
                        .filter(TrainingEnrollment.user_id == user.id)
                        .all()
                    )
                    ...
                    mentee_data.append(
                        {
                            ...
                            "last_activity": (
                                max([e.enrolled_at for e in enrollments]).isoformat()
                                if enrollments
                                else None
                            ),
                        }
    ...
    return {"mentees": mentee_data, "total_mentees": len(mentee_data)}
