...
async def get_payouts(
    ...
    query = db.query(CommissionPayment).join(
        BarberPaymentModel,
    ...
    for cp in commission_payments:
        ...
        fee = float(cp.total_paid) * 0.029 if cp.payment_method == "stripe_connect" else 0
        
        payouts.append(PayoutResponse(
            id=str(cp.id),
            barber_id=cp.barber_id,
            barber_name=f"{barber_user.first_name} {barber_user.last_name}" if barber_user else "Unknown",
            amount=float(cp.total_paid + fee),
            fee=fee,
            net_amount=float(cp.total_paid),
            status=payout_status,
            payment_method=cp.payment_method or "stripe",
            payout_date=cp.paid_date.isoformat() if cp.paid_date else cp.period_end.isoformat(),
    ...
    total_completed_amount = sum(p.net_amount for p in payouts if p.status == "completed")
...
async def process_payout(
    ...
    commission_payment = db.query(CommissionPayment).filter(
        CommissionPayment.id == int(payout_id)
    ).first()
    ...
    payment_model = db.query(BarberPaymentModel).filter(
        BarberPaymentModel.id == commission_payment.payment_model_id
    ).first()
