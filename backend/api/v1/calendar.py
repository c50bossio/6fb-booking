...
router.include_router(
    google_calendar_oauth.router, prefix="/oauth", tags=["Google Calendar OAuth"]
)
...
def convert_appointment_to_event(appointment: Appointment) -> CalendarEvent:
    ...
    return CalendarEvent(
        ...
        location_name=(
            appointment.barber.location.name
            if appointment.barber and appointment.barber.location
            else None
        ),
...
async def sync_appointment_to_google_calendar(
    appointment: Appointment, action: str, db: Session
):
    """Background task to sync appointment with Google Calendar"""
    try:
        if (
            hasattr(appointment, "sync_to_google_calendar")
            and not appointment.sync_to_google_calendar
        ):
            return
        ...
        logger.error(
            f"Failed to sync appointment {appointment.id} to Google Calendar: {str(e)}"
        )
...
async def get_calendar_events(
    ...
    location_ids: Optional[str] = Query(
        None, description="Comma-separated location IDs"
    ),
    service_ids: Optional[str] = Query(None, description="Comma-separated service IDs"),
    statuses: Optional[str] = Query(
        None, description="Comma-separated appointment statuses"
    ),
    types: Optional[str] = Query(None, description="Comma-separated event types"),
    client_search: Optional[str] = Query(
        None, description="Search client name or email"
    ),
    include_google_calendar: bool = Query(
        False, description="Include Google Calendar events"
    ),
    ...
    filters = CalendarFilters(
        barber_ids=(
            [int(x.strip()) for x in barber_ids.split(",")] if barber_ids else None
        ),
        location_ids=(
            [int(x.strip()) for x in location_ids.split(",")] if location_ids else None
        ),
        service_ids=(
            [int(x.strip()) for x in service_ids.split(",")] if service_ids else None
        ),
        ...
        if rbac.has_permission(current_user, Permission.MANAGE_LOCATION_APPOINTMENTS):
            ...
            if filters.location_ids:
                ...
                allowed_locations = list(
                    set(filters.location_ids) & set(accessible_locations)
                )
    ...
    if filters.location_ids:
        barber_ids_in_locations = (
            db.query(Barber.id)
            .filter(Barber.location_id.in_(filters.location_ids))
            .subquery()
        )
            ...
            if barber and google_calendar_service.is_connected(barber.id):
                ...
                for g_event in google_events:
                    if "start" in g_event and "end" in g_event:
                        ...
                        start_str = g_event["start"].get(
                            "dateTime", g_event["start"].get("date")
                        )
                        end_str = g_event["end"].get(
                            "dateTime", g_event["end"].get("date")
                        )
                        ...
                        if start_str and end_str:
                            try:
                                ...
                                if not any(
                                    event_summary in event.title for event in events
                                ):
                                    events.append(
                                        CalendarEvent(
                                ...
                                logger.warning(
                                    f"Could not parse Google Calendar event time: {e}"
                                )
...
async def get_barber_availability(
    ...
    if availability.break_start and availability.break_end:
        break_start_minutes = (
            availability.break_start.hour * 60 + availability.break_start.minute
        )
        break_end_minutes = (
            availability.break_end.hour * 60 + availability.break_end.minute
        )
    ...
    while current_minutes + service_duration <= end_minutes:
        ...
        if break_start_minutes and break_end_minutes:
            if (
                ...
                and current_minutes + service_duration > break_start_minutes
            ):
        ...
        for appointment in existing_appointments:
            ...
            # Check for overlap
            if (
                current_minutes < apt_end_minutes
                and current_minutes + service_duration > apt_start_minutes
            ):
                is_available = False
...
async def create_appointment(
    ...
    if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
        if rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS):
            ...
            if not barber or appointment_data.barber_id != barber.id:
                raise HTTPException(
                    status_code=403, detail="Can only create appointments for yourself"
                )
        else:
            raise HTTPException(
                status_code=403, detail="No permission to create appointments"
            )
    ...
    if appointment_data.sync_to_google_calendar:
        background_tasks.add_task(
            sync_appointment_to_google_calendar, appointment, "create", db
        )
...
async def update_appointment(
    ...
    if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
        if rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS):
            ...
            if not barber or appointment.barber_id != barber.id:
                raise HTTPException(
                    status_code=403, detail="Can only update your own appointments"
                )
        else:
            raise HTTPException(
                status_code=403, detail="No permission to update appointments"
            )
    ...
    if "appointment_date" in update_data and "appointment_time" in update_data:
        ...
        appointment.appointment_time = datetime.combine(
            update_data["appointment_date"], current_time
        )
        del update_data["appointment_date"]
    elif "appointment_time" in update_data:
        current_date = appointment.appointment_date
        appointment.appointment_time = datetime.combine(
            current_date, update_data["appointment_time"]
        )
        del update_data["appointment_time"]
    ...
    if sync_to_google and appointment.google_calendar_event_id:
        background_tasks.add_task(
            sync_appointment_to_google_calendar, appointment, "update", db
        )
    ...
    log_user_action(
        ...
        user_id=current_user.id,
        details={"appointment_id": appointment.id, "changes": list(update_data.keys())},
    )
...
async def delete_appointment(
    ...
    if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
        if rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS):
            ...
            if not barber or appointment.barber_id != barber.id:
                raise HTTPException(
                    status_code=403, detail="Can only delete your own appointments"
                )
        else:
            raise HTTPException(
                status_code=403, detail="No permission to delete appointments"
            )
    ...
    if google_event_id:
        background_tasks.add_task(
            sync_appointment_to_google_calendar, appointment, "delete", db
        )
...
async def get_calendar_stats(
    ...
    if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
        if rbac.has_permission(current_user, Permission.MANAGE_LOCATION_APPOINTMENTS):
            ...
            if location_id_list:
                allowed_locations = list(
                    set(location_id_list) & set(accessible_locations)
                )
                if not allowed_locations:
    ...
    if location_id_list:
        barber_ids_in_locations = (
            db.query(Barber.id)
            .filter(Barber.location_id.in_(location_id_list))
            .subquery()
        )
        query = query.filter(Appointment.barber_id.in_(barber_ids_in_locations))
    ...
    if barber_id_list:
        total_possible_hours = (
            len(barber_id_list) * working_days * working_hours_per_day
        )
    else:
    ...
    utilization_rate = (
        (total_booked_hours / total_possible_hours * 100)
