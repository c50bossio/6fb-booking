...
async def get_location_revenue_breakdown(
    ...
    if location_id not in accessible_locations and not rbac.has_permission(current_user, Permission.VIEW_ALL_ANALYTICS):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    service = RevenueManagementService(db)
    breakdown = await service.calculate_location_revenue_breakdown(location_id, start_date, end_date)
...
async def process_bulk_payouts(
    ...
    if location_id:
        accessible_locations = rbac.get_accessible_locations(current_user)
        if location_id not in accessible_locations and not rbac.has_permission(current_user, Permission.MANAGE_ALL_LOCATIONS):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def get_commission_structures(
    ...
    if not rbac.has_permission(current_user, Permission.VIEW_REVENUE_DATA):
        ...
        structure = service.commission_structures.get(cert_level, service.commission_structures['no_certification'])
...
async def get_revenue_trends(
    ...
    for i in range(period_months):
        ...
        trends.append({
            ...
            "revenue": revenue,
            "growth_percentage": (revenue / (revenue - 2000) - 1) * 100 if i > 0 else 0
        })
