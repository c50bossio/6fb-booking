...
from models.booking import (
    Service, ServiceCategory, BarberAvailability, 
    BookingRule, DayOfWeek, BookingSlot
)
...
async def create_appointment(
    ...
    await send_webhook_notification(
        "appointment.created",
        {
            ...
            "time": new_appointment.appointment_time.isoformat() if new_appointment.appointment_time else None,
...
async def get_calendar_appointments(
    ...
    if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
        if rbac.has_permission(current_user, Permission.MANAGE_LOCATION_APPOINTMENTS):
            ...
            barber_ids = db.query(Barber.id).filter(
                Barber.location_id.in_(accessible_locations)
            ).subquery()
            query = query.filter(Appointment.barber_id.in_(barber_ids))
        elif rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS):
            barber = db.query(Barber).filter(Barber.user_id == current_user.id).first()
    ...
    for apt in appointments:
        ...
        can_modify = (
            rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS) or
            (rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS) and 
             apt.barber.user_id == current_user.id)
        )
        
        calendar_appointments.append(CalendarAppointmentResponse(
            id=apt.id,
...
async def get_available_slots(
    ...
    blocked_slots = db.query(BookingSlot).filter(
    ...
    while current_slot + timedelta(minutes=service_duration) <= end_dt:
        ...
        if is_available:
            ...
            if service_id:
                ...
                rules = db.query(BookingRule).filter(
...
async def get_appointment_details(
    ...
    if appointment.client_id:
        ...
        client_appointments = db.query(Appointment).filter(
        ...
        if client_appointments:
            ...
            no_shows = len([apt for apt in client_appointments if apt.status == "no_show"])
            completed = [apt for apt in client_appointments if apt.status == "completed"]
            ...
            for apt in completed:
                if apt.service_name:
                    service_counts[apt.service_name] = service_counts.get(apt.service_name, 0) + 1
            ...
            # Last visit
            last_completed = max(
                [apt.appointment_date for apt in completed],
                default=None
            )
    ...
    return AppointmentDetailResponse(
        ...
        previous_appointments=client_history.get("total_appointments") if client_history else None,
        ...
        average_tip_percentage=client_history.get("average_tip_percentage") if client_history else None,
        preferred_services=client_history.get("preferred_services") if client_history else None,
...
async def create_recurring_appointments(
    ...
    barber = db.query(Barber).filter(
        Barber.id == recurring_request.appointment_data.barber_id
    ).first()
    if not barber:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if recurring_request.appointment_data.client_id:
        client = db.query(Client).filter(
            Client.id == recurring_request.appointment_data.client_id
        ).first()
        if not client:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if recurring_request.appointment_data.client_email:
            client = db.query(Client).filter(
                Client.email == recurring_request.appointment_data.client_email
            ).first()
        elif recurring_request.appointment_data.client_phone:
            client = db.query(Client).filter(
                Client.phone == recurring_request.appointment_data.client_phone
            ).first()
        else:
            client = None
    ...
    for appointment_date in dates:
        ...
        existing = db.query(Appointment).filter(
            ...
            Appointment.appointment_time == recurring_request.appointment_data.appointment_time,
            Appointment.status.in_(["scheduled", "confirmed"])
        ).first()
...
async def get_barber_schedule(
    ...
    regular_schedule = db.query(BarberAvailability).filter(
        BarberAvailability.barber_id == barber_id,
        BarberAvailability.is_available == True
    ).order_by(BarberAvailability.day_of_week).all()
    ...
    for special in special_availability:
        special_list.append({
            ...
            "effective_until": special.effective_until.isoformat() if special.effective_until else None
        })
        ...
        if "day_of_week" in update_data:
            ...
            if "effective_from" in update_data:
                existing = existing.filter(
                    or_(
                        BarberAvailability.effective_from == update_data["effective_from"],
                        BarberAvailability.effective_from == None
                    )
                )
