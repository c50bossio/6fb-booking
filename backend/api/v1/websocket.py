...
async def handle_websocket_message(
    connection_id: str, message: dict, user: User, barber: Barber = None
):
...
async def public_websocket_endpoint(websocket: WebSocket):
    ...
    connection_id = await connection_manager.connect(
        websocket, connection_type="public"
    )
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.warning(
                    f"Invalid JSON received from public connection {connection_id}"
                )
            except Exception as e:
...
async def websocket_status():
    ...
    return {"status": "active", "realtime_service": "enabled", **stats}


@router.post("/ws/broadcast")
async def broadcast_message(message: dict, user: User = Depends(get_current_user_ws)):
    """Broadcast message to all connected clients (admin only)"""
    if not user or user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")

    event = RealtimeEvent(event_type=EventType.SYSTEM_ALERT, data=message)
...
async def send_to_location(
    location_id: str, message: dict, user: User = Depends(get_current_user_ws)
):
    ...
    event = RealtimeEvent(
        event_type=EventType.CUSTOM, data=message, location_id=location_id
    )
