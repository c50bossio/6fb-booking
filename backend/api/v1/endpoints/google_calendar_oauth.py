...
async def connect_google_calendar(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to start Google Calendar connection")
        ...
        if not rbac.has_permission(current_user, Permission.MANAGE_ALL_APPOINTMENTS):
            if rbac.has_permission(current_user, Permission.MANAGE_OWN_APPOINTMENTS):
                ...
                if not barber or appointment.barber_id != barber.id:
                    raise HTTPException(
                        status_code=403,
                        detail="Can only sync own appointments"
                    )
            else:
                raise HTTPException(
                    status_code=403,
                    detail="No permission to sync appointments"
                )
        
        # Sync appointment
        event_id = google_calendar_service.sync_appointment(appointment, action='create')
