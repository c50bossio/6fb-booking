...
def sanitize_webhook_data(data):
    ...
    sensitive_fields = [
        'card_number', 'card', 'payment_method', 'token', 'secret',
        'private_key', 'api_key', 'password', 'ssn', 'account_number',
        'routing_number', 'bank_account', 'credit_card'
    ]
    ...
    for key, value in sanitized.items():
        if isinstance(value, dict):
            ...
            sanitized[key] = [sanitize_webhook_data(item) if isinstance(item, dict) else item for item in value]
...
async def handle_stripe_webhook(
    ...
    sanitized_data = sanitize_webhook_data(event.data.object if hasattr(event, 'data') else {})
    logger.info(f"Received Stripe webhook event: {event.type} (ID: {event.id})")
    logger.debug(f"Webhook event data (sanitized): {json.dumps(sanitized_data, default=str)}")
    ...
    try:
        ...
        return {"status": "success", "event_id": event.id, "processing_time_ms": round(processing_time, 2)}
        ...
        return {"status": "error_logged", "event_id": event.id if event else "unknown", "error": "Logged for investigation"}
