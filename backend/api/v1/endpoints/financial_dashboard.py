...
def check_demo_mode(current_user: User) -> bool:
    ...
    return (
        current_user.email.endswith("@demo.com") or current_user.email == "demo@6fb.com"
    )
...
def generate_demo_barber_revenue(
...
def generate_demo_payout_summary(start_date: date, end_date: date) -> Dict[str, Any]:
    ...
    return {
        ...
        "shop_net": {
            "gross_shop_revenue": round(
                total_revenue - commission_payouts + booth_rent_collected, 2
            ),
            "after_fees": round(
                total_revenue
                - commission_payouts
                + booth_rent_collected
                - processing_fees,
                2,
            ),
            "profit_margin": 42.5,
...
async def get_shop_metrics(
    period: str = Query(
        "month", description="Time period: week, month, quarter, year, custom"
    ),
    ...
    try:
        ...
        if location_id:
            current_query = current_query.join(Barber).filter(
                Barber.location_id == location_id
            )
            prev_query = prev_query.join(Barber).filter(
                Barber.location_id == location_id
            )
        ...
        barber_query = db.query(
            ...
            func.avg(case((Appointment.status == "completed", 1), else_=0)).label(
                "avg_utilization"
            ),
        ).filter(Appointment.appointment_date.between(start, end))

        if location_id:
            barber_query = barber_query.join(Barber).filter(
                Barber.location_id == location_id
            )
        ...
        appointment_change = (
            ((total_appointments - prev_appointments) / prev_appointments * 100)
        ...
        prev_avg_ticket = (
            prev_revenue / prev_appointments if prev_appointments > 0 else 0
        )
        avg_ticket_change = (
            ((avg_ticket - prev_avg_ticket) / prev_avg_ticket * 100)
            if prev_avg_ticket > 0
            else 0
        )
        ...
        return {
            ...
            "barber_metrics": {
                ...
                "average_utilization": round(
                    float(barber_metrics.avg_utilization or 0) * 100, 1
                ),
            },
        }
        ...
        for barber in barbers:
            ...
            if payment_model:
                if payment_model.payment_type == PaymentModelType.COMMISSION:
                    barber_earnings = (
                        total_revenue * payment_model.service_commission_rate
                    )
                    shop_earnings = total_revenue * (
                        1 - payment_model.service_commission_rate
                    )
                    booth_rent = None
                elif payment_model.payment_type == PaymentModelType.BOOTH_RENT:
                    weeks_in_period = (end - start).days / 7
                    booth_rent = (
                        float(payment_model.booth_rent_amount) * weeks_in_period
                    )
                    ...
                    barber_earnings = commission_earnings
                    shop_earnings = (
                        total_revenue * (1 - payment_model.service_commission_rate)
                    ) + booth_rent
            else:
            ...
            utilization_rate = min(
                daily_appointments / 8 * 100, 100
            )  # Assume 8 appointments is 100%
            ...
            barber_data.append(
                {
                    ...
                    "earnings": {
                        ...
                        "shop_earnings": round(shop_earnings, 2),
                        "commission_rate": (
                            (payment_model.service_commission_rate * 100)
                            if payment_model
                            and payment_model.payment_type
                    ...
                    "metrics": {
                        ...
                        "average_ticket": (
                            round(total_revenue / barber.total_appointments, 2)
                            if barber.total_appointments > 0
                            else 0
                        ),
...
@router.get("/barber-dashboard/{barber_id}")
async def get_barber_dashboard(
    ...
    """
    Get comprehensive dashboard data for individual barber
    Access: barber (own data), shop_owner, admin, super_admin
    """
    # Check authorization
    if current_user.role == "barber":
        # Barbers can only access their own dashboard
    ...
    # Check demo mode
    if check_demo_mode(current_user):
        return generate_demo_barber_dashboard_data(barber_id)
    ...
    try:
        ...
        week_earnings = float(week_metrics.earnings or 0)
        month_earnings = float(month_metrics.earnings or 0)

        return {
            "today": {
                "earnings": today_earnings,
                "appointments": today_metrics.appointments or 0,
                "tips": float(today_metrics.tips or 0),
                "services": float(today_metrics.services or 0),
            ...
            "week": {
                ...
                "appointments": week_metrics.appointments or 0,
                "tips": float(week_metrics.tips or 0),
                "goal": 1500,  # Mock goal
            ...
            "month": {
                ...
                "appointments": month_metrics.appointments or 0,
                "goal": 6000,  # Mock goal
                "progress": min((month_earnings / 6000) * 100, 100),
...
def generate_demo_barber_dashboard_data(barber_id: int) -> Dict[str, Any]:
    """Generate demo data for barber dashboard"""
    return {
        "today": {
            "earnings": 285.50,
            "appointments": 6,
...
async def get_payout_summary(
    ...
    try:
        ...
        if location_id:
            revenue_query = revenue_query.join(Barber).filter(
                Barber.location_id == location_id
            )
        ...
        commission_query = db.query(
            ...
            CommissionPayment.period_start >= start, CommissionPayment.period_end <= end
        )

        if location_id:
            commission_query = (
                commission_query.join(BarberPaymentModel)
                .join(Barber)
        ...
        rent_query = db.query(
            ...
            BoothRentPayment.period_start >= start, BoothRentPayment.period_end <= end
        )

        if location_id:
            rent_query = rent_query.join(Barber).filter(
                Barber.location_id == location_id
            )
        ...
        for payment in commission_summary:
            if payment.status == BarberPaymentStatus.PENDING:
                ...
                pending_barbers = (
                    db.query(Barber.first_name, Barber.last_name)
                    .join(CommissionPayment)
                    .filter(
                ...
                last_payout = (
                    db.query(CommissionPayment.paid_date)
                    .filter(
                ...
                if last_payout and last_payout.paid_date:
                    payouts["completed"][
                        "last_payout_date"
                    ] = last_payout.paid_date.date().isoformat()
        ...
        for rent in rent_summary:
            ...
            if rent.status == BarberPaymentStatus.PAID:
                ...
                booth_rent["pending"] += float(rent.amount_due or 0) - float(
                    rent.amount_paid or 0
                )
            elif rent.status == BarberPaymentStatus.OVERDUE:
                booth_rent["overdue"] += float(rent.amount_due or 0) - float(
                    rent.amount_paid or 0
                )

        # Calculate shop net
        total_barber_payments = (
            payouts["completed"]["amount"] + payouts["pending"]["amount"]
        )
        gross_shop_revenue = (
            total_revenue - total_barber_payments + booth_rent["collected"]
        )
        net_shop_revenue = gross_shop_revenue - processing_fees
        profit_margin = (
            (net_shop_revenue / total_revenue * 100) if total_revenue > 0 else 0
        )
