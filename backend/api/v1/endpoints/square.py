...
async def init_square_oauth(
    ...
    if (
        ...
        and current_user.id != request.barber_id
    ):
        # Check if user is the barber
        barber = (
            db.query(Barber)
            .filter(Barber.id == request.barber_id, Barber.user_id == current_user.id)
            .first()
        )

        if not barber:
            raise HTTPException(
                status_code=403, detail="Not authorized to connect Square account"
            )
...
async def handle_square_oauth_callback(
    request: SquareOAuthCallbackRequest, db: Session = Depends(get_db)
):
...
async def get_square_account(
    ...
    if current_user.role not in ["super_admin", "admin"]:
        barber = (
            db.query(Barber)
            .filter(Barber.id == barber_id, Barber.user_id == current_user.id)
            .first()
        )

        if not barber:
            raise HTTPException(
                status_code=403, detail="Not authorized to view Square account"
            )
    ...
    return SquareAccountResponse(
        ...
        connected_at=(
            square_account.connected_at.isoformat()
            if square_account.connected_at
            else ""
        ),
    )
...
async def create_square_payment(
    request: SquarePaymentRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if not location_id:
            raise HTTPException(
                status_code=400, detail="Square location not configured"
            )
        ...
        await square_service.create_payout(
            db=db, payment_id=payment.id, commission_rate=0.7  # Default 70% commission
        )
...
async def list_square_payments(
    ...
    if barber_id:
        ...
        if current_user.role not in ["super_admin", "admin"]:
            ...
            if not barber:
                raise HTTPException(
                    status_code=403, detail="Not authorized to view payments"
                )
...
async def list_square_payouts(
    ...
    if barber_id:
        ...
        if current_user.role not in ["super_admin", "admin"]:
            ...
            if not barber:
                raise HTTPException(
                    status_code=403, detail="Not authorized to view payouts"
                )
    ...
    payouts = (
        query.order_by(SquarePayout.created_at.desc()).offset(offset).limit(limit).all()
    )
...
async def process_square_payout(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Payout processing failed: {str(e)}"
        )
...
async def handle_square_webhook(
    ...
    try:
        ...
        background_tasks.add_task(square_service.handle_webhook, db, webhook_data)
...
async def disconnect_square_account(
    ...
    if current_user.role not in ["super_admin", "admin"]:
        ...
        if not barber:
            raise HTTPException(
                status_code=403, detail="Not authorized to disconnect Square account"
            )
