...
from models.settings import (
    SettingsConfig, SettingsTemplate, UserPreferences, LocationSettings,
    NotificationSettings, AccessibilitySettings, IntegrationSettings,
    SettingsHistory, SettingsScope, SettingsCategory, ThemeMode,
    CalendarView, TimeFormat, DateFormat, DEFAULT_SETTINGS_TEMPLATES
)
...
async def apply_settings_template(
    ...
    # Verify permission
    if scope == SettingsScope.LOCATION and not check_permission(current_user, "manage_locations"):
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    
    template = db.query(SettingsTemplate).filter(
    ...
    for category_key, category_settings in template.template_data.items():
        if isinstance(category_settings, dict):
            for setting_key, setting_value in category_settings.items():
                ...
                setting = db.query(SettingsConfig).filter(
...
async def update_settings_config(
    ...
    for update in updates.updates:
        try:
            ...
            if not setting:
                errors.append(f"Setting '{update.setting_key}' not found or not configurable")
                continue
...
async def get_location_settings(
    ...
    if not check_permission(current_user, "view_location_data") and location_id not in current_user.accessible_locations:
...
async def get_accessibility_settings(
    ...
    settings = db.query(AccessibilitySettings).filter(
        AccessibilitySettings.user_id == current_user.id
    ).first()
...
async def get_integration_settings(
    ...
    settings = db.query(IntegrationSettings).filter(
        IntegrationSettings.scope == scope,
        IntegrationSettings.scope_id == scope_id
    ).first()
    ...
    if not include_sensitive:
        ...
        for key, value in settings_dict.items():
            if isinstance(value, dict):
                for subkey in value:
                    if any(sensitive in subkey.lower() for sensitive in ['secret', 'key', 'token', 'password']):
                        if value[subkey]:
...
async def export_settings(
    ...
    settings = db.query(SettingsConfig).filter(
        SettingsConfig.scope == scope,
        SettingsConfig.scope_id == scope_id,
        SettingsConfig.is_active == True
    ).all()
...
async def import_settings(
    ...
    for setting_data in import_data.get("settings", []):
        try:
            existing_setting = db.query(SettingsConfig).filter(
                SettingsConfig.scope == scope,
                SettingsConfig.scope_id == scope_id,
            ...
            errors.append(f"Error importing '{setting_data.get('setting_key', 'unknown')}': {str(e)}")
...
async def initialize_default_settings(
    ...
    for template_data in DEFAULT_SETTINGS_TEMPLATES:
        existing = db.query(SettingsTemplate).filter(
            SettingsTemplate.name == template_data["name"]
        ).first()
    ...
    if scope == SettingsScope.USER and scope_id:
        ...
        existing_prefs = db.query(UserPreferences).filter(
            UserPreferences.user_id == scope_id
        ).first()
        ...
        existing_settings = db.query(LocationSettings).filter(
            LocationSettings.location_id == scope_id
        ).first()
