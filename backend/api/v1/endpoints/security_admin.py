...
async def get_security_events(
    ...
    try:
        ...
        for event in security_monitor.recent_events:
            ...
            filtered_events.append(SecurityEventResponse(
...
async def analyze_ip_address(
    ...
    try:
        ...
        # Get session data
        session_data = security_monitor.threat_detector.user_sessions.get(ip_address, {})
...
async def get_threat_intelligence(
    ...
    try:
        ...
        events_24h = sum(1 for e in security_monitor.recent_events if e.timestamp >= last_24h)
        events_7d = sum(1 for e in security_monitor.recent_events if e.timestamp >= last_7d)
        
        # Count by severity
        critical_24h = sum(1 for e in security_monitor.recent_events 
                          if e.timestamp >= last_24h and e.severity == 'CRITICAL')
        high_24h = sum(1 for e in security_monitor.recent_events 
        ...
        intelligence = {
            'summary': {
                ...
                'threat_trend': 'increasing' if events_24h > events_7d / 7 else 'stable'
            },
            'top_attack_types': top_attacks,
            'geographic_distribution': geographic_threats,
            'threat_indicators': {
                'sql_injection_attempts': sum(1 for event in security_monitor.recent_events 
                                            if 'SQL_INJECTION' in event.event_type and event.timestamp >= last_24h),
                'xss_attempts': sum(1 for event in security_monitor.recent_events 
                                  if 'XSS' in event.event_type and event.timestamp >= last_24h),
                'brute_force_attempts': sum(1 for event in security_monitor.recent_events 
...
async def security_system_health(
    ...
    try:
        ...
        critical_events = sum(1 for e in security_monitor.recent_events 
                            if e.timestamp >= last_hour and e.severity == 'CRITICAL')
        
        if critical_events > 10:
            health_status['threat_detection'] = 'warning'
            health_status['overall_status'] = 'warning'
            health_status['warnings'] = [f'High number of critical security events: {critical_events} in last hour']
