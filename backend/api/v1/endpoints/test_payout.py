...
async def trigger_test_payout(
    ...
    try:
        ...
        if not plan.payout_settings or not plan.payout_settings.get("enabled"):
            raise HTTPException(
                status_code=400, detail="Automated payouts not enabled for this plan"
            )
...
async def get_payout_status(
    ...
    try:
        ...
        return {
            ...
            "payout_enabled": (
                plan.payout_settings.get("enabled", False)
                if plan.payout_settings
                else False
            ),
            "payout_method": (
                plan.payout_settings.get("method") if plan.payout_settings else None
            ),
            "payout_frequency": (
                plan.payout_settings.get("frequency") if plan.payout_settings else None
            ),
            "unpaid_commissions_count": len(unpaid_commissions),
            "total_unpaid_amount": total_unpaid,
            "stripe_connected": bool(barber.stripe_account_id) if barber else False,
            "ready_for_payout": total_unpaid > 0
            and plan.payout_settings.get("enabled", False),
        }
...
async def get_scheduler_status(current_user=Depends(get_current_user)):
    ...
    try:
        is_running = (
            payout_scheduler.scheduler.running if payout_scheduler.scheduler else False
        )
...
async def get_configuration_status(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        config_status = {
            "database": {
                ...
                "url": (
                    os.getenv("DATABASE_URL", "Not configured")[:50] + "..."
            ...
            "stripe": {
                ...
                "environment": (
                    "live"
                    if os.getenv("STRIPE_SECRET_KEY", "").startswith("sk_live_")
                    else "test"
                ),
            },
            "email": {
            ...
            "scheduler": {
                "running": (
                    payout_scheduler.scheduler.running
                    if payout_scheduler.scheduler
                    else False
                ),
                "jobs_count": (
                    len(payout_scheduler.scheduler.get_jobs())
                    if payout_scheduler.scheduler
                    else 0
                ),
            },
        ...
        readiness = {
            ...
            "missing_optional": [
                name
                for name, status in zip(
                    ["Email SMTP", "Stripe Webhooks"], optional_configs
                )
                if not status
            ],
        }

        return {"configuration": config_status, "readiness": readiness}
