...
from models import (
    ...
    Location, Client, Appointment, BookingRule, DayOfWeek,
    Review, ReviewRating
)
    ...
    def validate_break_times(cls, v, values):
        if v and values.get('break_start'):
            ...
            if values.get('start_time') and values.get('end_time'):
                if values['break_start'] < values['start_time'] or v > values['end_time']:
...
async def get_barber_schedule(
    ...
    for avail in availability:
        schedule["availability_patterns"].append({
            ...
            "effective_from": str(avail.effective_from) if avail.effective_from else None,
            "effective_until": str(avail.effective_until) if avail.effective_until else None
        })
...
async def set_barber_availability(
    ...
    if current_user.role != "admin":
        user_barber = db.query(Barber).filter(Barber.user_id == current_user.id).first()
        if not user_barber or user_barber.id != barber_id:
            raise HTTPException(status_code=403, detail="Not authorized to set this availability")
...
async def get_booking_calendar(
    ...
    if current_user.role == "barber":
        ...
        if current_user.primary_location_id:
            query = query.join(Barber).filter(Barber.location_id == current_user.primary_location_id)
    ...
    return {
        ...
        "summary": {
            ...
            "average_revenue": total_revenue / total_appointments if total_appointments > 0 else 0
        },
        "calendar": calendar
    }
...
async def create_review(
    ...
    review = Review(
        ...
        service_rating=ReviewRating(review_data.service_rating) if review_data.service_rating else None,
        cleanliness_rating=ReviewRating(review_data.cleanliness_rating) if review_data.cleanliness_rating else None,
        punctuality_rating=ReviewRating(review_data.punctuality_rating) if review_data.punctuality_rating else None,
        value_rating=ReviewRating(review_data.value_rating) if review_data.value_rating else None,
    ...
    avg_rating = db.query(func.avg(Review.overall_rating)).filter(
        Review.barber_id == appointment.barber_id
    ).scalar()
