...
async def check_external_services() -> Dict[str, Any]:
    ...
    services = {
        "stripe": {
            ...
            "headers": {"Authorization": f"Bearer {settings.STRIPE_SECRET_KEY}"} if settings.STRIPE_SECRET_KEY else None,
            "timeout": 5
        },
...
def check_environment_config() -> Dict[str, Any]:
    ...
    config_checks = {
        ...
        "JWT_SECRET_KEY": bool(settings.JWT_SECRET_KEY and len(settings.JWT_SECRET_KEY) > 32),
    ...
    return {
        ...
        "missing_optional": missing_optional,
        "configured_services": [key for key, value in optional_configs.items() if value]
    }
...
async def health_metrics():
    ...
    try:
        ...
        # Get metrics summary
        metrics_summary = await monitoring_service.get_metrics_summary(time_range_minutes=10)
...
async def reset_payment_alerts(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "error": str(e),
                "message": "Failed to reset payment alerts"
            }
        )
...
async def security_health_check():
    ...
    try:
        ...
        return {
            "security_health": {
                "status": "healthy" if security_summary["summary"]["critical_events"] == 0 else "warning",
