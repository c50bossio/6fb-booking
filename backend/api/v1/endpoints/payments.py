...
from models.payment import (
    ...
    Refund, RefundStatus, PaymentReport
)
...
class PaymentIntentCreate(BaseModel):
    ...
    payment_method_id: Optional[int] = Field(None, description="ID of saved payment method")
...
class PaymentConfirm(BaseModel):
    ...
    payment_method_id: Optional[str] = Field(None, description="Payment method ID if not already attached")


class RefundCreate(BaseModel):
    """Schema for creating a refund."""
    payment_id: int
    amount: Optional[int] = Field(None, gt=0, description="Amount to refund in cents (None for full refund)")
...
async def create_payment_intent(
    ...
    appointment = db.query(Appointment).filter(
        Appointment.id == payment_intent.appointment_id
    ).first()
...
async def get_refund(
    ...
    if current_user.id != payment.user_id and current_user.role not in ["super_admin", "admin", "mentor"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def create_payment_report(
    ...
    for payment in payments:
        # By barber
        barber_name = payment.appointment.barber.full_name if payment.appointment else "Unknown"
        ...
        # By service
        service_name = payment.appointment.service_name if payment.appointment else "Unknown"
