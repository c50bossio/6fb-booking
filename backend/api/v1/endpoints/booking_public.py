...
from models import (
    Barber, Service, ServiceCategory, BarberAvailability,
...
def get_barber_availability_for_date(
    ...
    slot_duration = timedelta(minutes=service.duration_minutes + (service.buffer_minutes or 0))
    
    # Account for break times
    break_start = datetime.combine(target_date, availability.break_start) if availability.break_start else None
    break_end = datetime.combine(target_date, availability.break_end) if availability.break_end else None
    ...
    while current_time + timedelta(minutes=service.duration_minutes) <= end_time:
        ...
        for appt in existing_appointments:
            appt_start = datetime.combine(appt.appointment_date, appt.appointment_time.time())
...
async def get_shop_barbers(
    ...
    for barber, avg_rating, review_count in barbers:
        result.append(BarberPublicProfile(
            id=barber.id,
...
async def get_barber_services(
    ...
    # Build query
    query = db.query(Service, ServiceCategory.name.label('category_name')).join(
...
async def create_booking(
    ...
    appointment_datetime = datetime.combine(booking.appointment_date, booking.appointment_time)
...
async def confirm_booking(
    ...
    appointment = db.query(Appointment).filter(
        Appointment.reference_source == booking_token
    ).first()
    
    if not appointment:
        raise HTTPException(
            status_code=404,
            detail="Invalid booking token"
        )
