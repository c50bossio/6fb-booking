...
async def get_clients(
    ...
    sort_by: str = Query("last_visit", regex="^(last_visit|total_spent|total_visits|created_at)$"),
    ...
    if sort_by == "last_visit":
        query = query.order_by(Client.last_visit_date.desc() if order == "desc" else Client.last_visit_date.asc())
    elif sort_by == "total_spent":
        query = query.order_by(Client.total_spent.desc() if order == "desc" else Client.total_spent.asc())
    elif sort_by == "total_visits":
        query = query.order_by(Client.total_visits.desc() if order == "desc" else Client.total_visits.asc())
    else:
        query = query.order_by(Client.created_at.desc() if order == "desc" else Client.created_at.asc())
...
async def create_client(
    ...
    if current_user.get("role") == "barber":
        ...
        raise HTTPException(status_code=400, detail="Barber ID required for creating clients")
    
    if not barber_id:
        raise HTTPException(status_code=400, detail="No barber associated with current user")
    ...
    if client_data.date_of_birth:
        new_client.date_of_birth = datetime.fromisoformat(client_data.date_of_birth).date()
...
async def get_client_history(
    ...
    for appointment in client.appointments:
        appointment_data = {
            ...
            "barber": appointment.barber.user.full_name if appointment.barber and appointment.barber.user else "Unknown",
        ...
        if appointment.service:
            service_name = appointment.service.name
            services_breakdown[service_name] = services_breakdown.get(service_name, 0) + 1
...
async def update_vip_status(
    ...
    return {"message": "VIP status updated successfully", "client_id": client_id, "is_vip": vip_data.is_vip}
...
async def send_client_message(
    ...
    if current_user.get("role") == "barber":
        barber = db.query(Barber).filter(Barber.user_id == current_user["id"]).first()
        if not barber or client.barber_id != barber.id:
            raise HTTPException(status_code=403, detail="Not authorized to message this client")
...
async def export_clients(
    ...
    if format == "csv":
        ...
        writer.writerow([
            ...
            "Customer Type", "Total Visits", "Total Spent", "Average Ticket",
            "Last Visit", "No Shows", "Cancellations", "Tags", "Created At"
        ])
        ...
        for client in clients:
            writer.writerow([
                ...
                client.last_visit_date.isoformat() if client.last_visit_date else "",
