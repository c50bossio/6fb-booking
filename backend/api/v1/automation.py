...
async def get_automation_rules(
    ...
    rules = query.order_by(AutomationRule.priority.desc()).offset(skip).limit(limit).all()
...
async def get_automation_logs(
    ...
    logs = query.order_by(WorkflowLog.execution_time.desc()).offset(skip).limit(limit).all()
...
async def get_automation_stats(
    ...
    active_rules = db.query(AutomationRule).filter(AutomationRule.is_active == True).count()
    
    total_executions = db.query(WorkflowLog).filter(
        WorkflowLog.execution_time >= start_date
    ).count()
    
    successful_executions = db.query(WorkflowLog).filter(
        WorkflowLog.execution_time >= start_date,
        WorkflowLog.status == 'success'
    ).count()
    
    failed_executions = db.query(WorkflowLog).filter(
        WorkflowLog.execution_time >= start_date,
    ...
    most_active = db.query(
        WorkflowLog.rule_id,
        func.count(WorkflowLog.id).label('execution_count')
    ).filter(
        WorkflowLog.execution_time >= start_date
    ).group_by(WorkflowLog.rule_id).order_by(desc('execution_count')).limit(5).all()
