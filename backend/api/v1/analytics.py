...
async def get_barber_sixfb_score(
    ...
    if barber.user_id != current_user.id:  # Not viewing own score
        if not rbac.has_permission(current_user, Permission.VIEW_ALL_ANALYTICS):
            if not rbac.has_permission(
                current_user, Permission.VIEW_LOCATION_ANALYTICS, barber.location_id
            ):
                raise HTTPException(
    ...
    for component, score in components.items():
        if score < 70:
            improvements.append(
                f"Improve {component.replace('_', ' ').title()} (current: {score})"
            )
...
async def get_dashboard_summary(
    ...
    if rbac.has_permission(current_user, Permission.VIEW_ALL_ANALYTICS):
        ...
        # Get all locations with optimized query
        ...
        return {
            ...
            "metrics": {
                ...
                "avg_revenue_per_location": (
                    total_revenue / len(locations) if locations else 0
                ),
            },
        }
        ...
        return {
            ...
            "metrics": {
                ...
                "appointments_this_week": 0,  # Calculate from appointments
                "revenue_this_month": 0,  # Calculate from appointments
                "client_retention": score.get("components", {}).get(
        ...
        barber_count = (
            db.query(Barber)
            .filter(and_(Barber.location_id == location_id, Barber.is_active == True))
            .count()
        )
    ...
    return [
        {
            "date": str(d),
            "revenue": float(
                data_map.get(str(d), type("", (), {"revenue": 0})).revenue or 0
            ),
            "services": float(
                data_map.get(str(d), type("", (), {"services": 0})).services or 0
            ),
            "products": float(
                data_map.get(str(d), type("", (), {"products": 0})).products or 0
            ),
...
def get_booking_analytics(
    ...
    query = db.query(
        ...
        func.sum(case((Appointment.status == "scheduled", 1), else_=0)).label(
            "pending"
        ),
    ).filter(
        and_(
...
def get_performance_metrics(
    ...
    if location_id:
        revenue_data = revenue_data.join(Barber).filter(
            Barber.location_id == location_id
        )
...
def get_retention_analytics(
    ...
    while current_date <= end_date:
        ...
        retention_rate = (
            (returning_clients / current_clients * 100) if current_clients > 0 else 0
        )
    ...
    # Mock cohort data
    cohort_analysis = [
        {"name": "Jan 2024", "retention": [100, 85, 78, 72, 68, 65]},
        {"name": "Feb 2024", "retention": [100, 88, 82, 75, 70, None]},
        {"name": "Mar 2024", "retention": [100, 90, 85, 78, None, None]},
    ]
...
def get_barber_comparison(
    ...
    # Query barber stats with optimized joins
    query = (
        db.query(
            Barber.id,
...
def export_analytics(
    ...
    revenue_data = get_revenue_analytics(
        start_date, end_date, location_id, db, current_user
    )
    booking_data = get_booking_analytics(
        start_date, end_date, location_id, db, current_user
    )
