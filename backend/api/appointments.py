...
async def get_appointments(
    ...
    status: Optional[str] = Query(
        None, description="scheduled, completed, cancelled, no_show"
    ),
                ...
                "appointments": [
                    {
                        ...
                        "client_name": (
                            apt.client.full_name if apt.client else "Unknown"
                        ),
...
async def update_appointment(
    ...
    try:
        ...
        appointment = (
            ...
            .filter(
                and_(
                    Appointment.id == appointment_id, Appointment.barber_id == barber_id
                )
            )
            .first()
        )
...
async def get_daily_summary(
    ...
    try:
        ...
        summary = {
            ...
            "cancelled": len(
                [apt for apt in appointments if apt.status == "cancelled"]
            ),
            ...
            "appointments": [
                ...
                for apt in sorted(
                    appointments, key=lambda x: x.appointment_time or datetime.min
                )
            ],
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating daily summary: {str(e)}"
        )


@router.post("/trafft-webhook")
async def handle_trafft_webhook(webhook_data: dict, db: Session = Depends(get_db)):
