...
def check_configuration():
    ...
    test_results["configuration"]["webhook_secret_configured"] = bool(
        ...
        and settings.STRIPE_WEBHOOK_SECRET != "your-webhook-secret"
    )

    test_results["configuration"]["connect_client_id_configured"] = bool(
        os.getenv("STRIPE_CONNECT_CLIENT_ID")
        and os.getenv("STRIPE_CONNECT_CLIENT_ID") != "your-connect-client-id"
    )

    # Check Square configuration
    test_results["configuration"]["square_configured"] = bool(
        os.getenv("SQUARE_ACCESS_TOKEN") and os.getenv("SQUARE_APPLICATION_ID")
    )

    # Check Tremendous configuration
    test_results["configuration"]["tremendous_configured"] = bool(
        os.getenv("TREMENDOUS_API_KEY")
        and os.getenv("TREMENDOUS_API_KEY") != "your-tremendous-api-key"
    )
...
def test_stripe_connection():
    ...
    try:
        ...
        log_test("Stripe API Connection", "failed", error="Invalid API key")
        return False
    except Exception as e:
        log_test("Stripe API Connection", "failed", error=str(e))
        return False
...
def test_payment_intent_creation(db):
    ...
    try:
        ...
        if not test_barber:
            ...
            test_barber = Barber(user_id=barber_user.id, commission_rate=0.7)
            db.add(test_barber)
            db.commit()
        ...
        log_test("Payment Intent Creation", "failed", error=str(e))
...
def test_payment_split_calculation(db):
    ...
    try:
        ...
        if (
            float(split["barber_amount"]) == expected_barber
            and float(split["shop_amount"]) == expected_shop
        ):
            log_test(
                "Payment Split Calculation (Commission)",
                "passed",
                details={
                    "total": "$100.00",
        ...
        if float(split_rent["barber_amount"]) == 100.00:
            log_test(
                "Payment Split Calculation (Booth Rent)",
                "passed",
                details={
                    "total": "$100.00",
        ...
        log_test("Payment Split Calculation", "failed", error=str(e))
...
def test_stripe_connect_oauth():
    ...
    try:
        ...
        log_test("Stripe Connect OAuth URL", "failed", error=str(e))
...
def test_webhook_signature_verification():
    ...
    try:
        ...
        log_test("Webhook Signature Verification", "failed", error=str(e))
...
def test_compensation_plan_calculations(db):
    ...
    try:
        ...
        if barber:
            ...
            log_test(
                "Compensation Plan - Daily Metrics",
                "passed",
                details={
        ...
        log_test("Compensation Plan Calculations", "failed", error=str(e))
...
def test_barber_payout_methods(db):
    ...
    try:
        ...
        log_test("Barber Payout Methods", "failed", error=str(e))
...
def test_payment_security_measures(db):
    ...
    try:
        ...
        log_test("Payment Security Measures", "failed", error=str(e))
