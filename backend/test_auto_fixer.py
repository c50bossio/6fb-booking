...
class TestAutoFixer:
    ...
    def __init__(self):
        ...
        logger.info(f"Integration token: {self.sentry_token[:20]}..." if self.sentry_token else "No token found")
    ...
    async def process_sentry_error(self, webhook_data: Dict) -> Dict:
        ...
        try:
            ...
            fix_confidence = 85 if error_type in ["database_schema", "api_rate_limit"] else 65
            auto_fixable = error_type in ["database_schema", "api_rate_limit", "import_error"]
...
async def handle_sentry_webhook(
    ...
    try:
        ...
        logger.info(f"Issue: {payload.get('data', {}).get('issue', {}).get('title', 'Unknown')}")
